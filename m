Return-Path: <devicetree+bounces-39-lists+devicetree=lfdr.de@vger.kernel.org>
X-Original-To: lists+devicetree@lfdr.de
Delivered-To: lists+devicetree@lfdr.de
Received: from sy.mirrors.kernel.org (sy.mirrors.kernel.org [IPv6:2604:1380:40f1:3f00::1])
	by mail.lfdr.de (Postfix) with ESMTPS id C3F3879F8CB
	for <lists+devicetree@lfdr.de>; Thu, 14 Sep 2023 05:16:59 +0200 (CEST)
Received: from smtp.subspace.kernel.org (wormhole.subspace.kernel.org [52.25.139.140])
	(using TLSv1.2 with cipher ECDHE-RSA-AES256-GCM-SHA384 (256/256 bits))
	(No client certificate requested)
	by sy.mirrors.kernel.org (Postfix) with ESMTPS id DF0FCB209DF
	for <lists+devicetree@lfdr.de>; Thu, 14 Sep 2023 03:16:56 +0000 (UTC)
Received: from localhost.localdomain (localhost.localdomain [127.0.0.1])
	by smtp.subspace.kernel.org (Postfix) with ESMTP id 3662E1391;
	Thu, 14 Sep 2023 03:16:15 +0000 (UTC)
X-Original-To: devicetree@vger.kernel.org
Received: from lindbergh.monkeyblade.net (lindbergh.monkeyblade.net [23.128.96.19])
	(using TLSv1.2 with cipher ECDHE-RSA-AES256-GCM-SHA384 (256/256 bits))
	(No client certificate requested)
	by smtp.subspace.kernel.org (Postfix) with ESMTPS id 19CD580B
	for <devicetree@vger.kernel.org>; Thu, 14 Sep 2023 03:16:14 +0000 (UTC)
Received: from ex01.ufhost.com (ex01.ufhost.com [61.152.239.75])
	by lindbergh.monkeyblade.net (Postfix) with ESMTPS id 850D01BDC;
	Wed, 13 Sep 2023 20:16:13 -0700 (PDT)
Received: from EXMBX166.cuchost.com (unknown [175.102.18.54])
	(using TLSv1 with cipher DHE-RSA-AES256-SHA (256/256 bits))
	(Client CN "EXMBX166", Issuer "EXMBX166" (not verified))
	by ex01.ufhost.com (Postfix) with ESMTP id 58CD024E2D6;
	Thu, 14 Sep 2023 11:16:11 +0800 (CST)
...
Subject: [PATCH v9 5/8] media: staging: media: starfive: camss: Add ISP driver
Date: Thu, 14 Sep 2023 11:16:04 +0800
Message-ID: <20230914031607.34877-6-jack.zhu@starfivetech.com>
...
;
+
+	reg_val =3D GAIN_D_POINT(0x40) | GAIN_C_POINT(0x40) |
+		  GAIN_B_POINT(0x40) | GAIN_A_POINT(0x40);
+	for (reg_add =3D ISP_REG_OBCG_CFG_0; reg_add <=3D ISP_REG_OBCG_CFG_3;) =
{
+		stf_isp_reg_write(stfcamss, reg_add, reg_val);
+		reg_add +=3D 4;
+	}
+
+	reg_val =3D OFFSET_D_POINT(0) | OFFSET_C_POINT(0) |
+		  OFFSET_B_POINT(0) | OFFSET_A_POINT(0);
+	for (reg_add =3D ISP_REG_OBCO_CFG_0; reg_add <=3D ISP_REG_OBCO_CFG_3;) =
{
+		stf_isp_reg_write(stfcamss, reg_add, reg_val);
+		reg_add +=3D 4;
+	}
...
+	u16 par_h, par_l;
+
+	par_h =3D 0x10; par_l =3D 0;
+	par_val =3D OCEF_PAR_H(par_h) | OCEF_PAR_L(par_l);
+	for (reg_add =3D ISP_REG_OECF_X0_CFG0; reg_add <=3D ISP_REG_OECF_Y3_CFG=
0;) {
+		stf_isp_reg_write(stfcamss, reg_add, par_val);
+		reg_add +=3D 0x20;
+	}
+
+	par_h =3D 0x40; par_l =3D 0x20;
+	par_val =3D OCEF_PAR_H(par_h) | OCEF_PAR_L(par_l);
+	for (reg_add =3D ISP_REG_OECF_X0_CFG1; reg_add <=3D ISP_REG_OECF_Y3_CFG=
1;) {
+		stf_isp_reg_write(stfcamss, reg_add, par_val);
+		reg_add +=3D 0x20;
+	}
+
+	par_h =3D 0x80; par_l =3D 0x60;
+	par_val =3D OCEF_PAR_H(par_h) | OCEF_PAR_L(par_l);
+	for (reg_add =3D ISP_REG_OECF_X0_CFG2; reg_add <=3D ISP_REG_OECF_Y3_CFG=
2;) {
+		stf_isp_reg_write(stfcamss, reg_add, par_val);
+		reg_add +=3D 0x20;
+	}
+
+	par_h =3D 0xc0; par_l =3D 0xa0;
+	par_val =3D OCEF_PAR_H(par_h) | OCEF_PAR_L(par_l);
+	for (reg_add =3D ISP_REG_OECF_X0_CFG3; reg_add <=3D ISP_REG_OECF_Y3_CFG=
3;) {
+		stf_isp_reg_write(stfcamss, reg_add, par_val);
+		reg_add +=3D 0x20;
+	}
+
+	par_h =3D 0x100; par_l =3D 0xe0;
+	par_val =3D OCEF_PAR_H(par_h) | OCEF_PAR_L(par_l);
+	for (reg_add =3D ISP_REG_OECF_X0_CFG4; reg_add <=3D ISP_REG_OECF_Y3_CFG=
4;) {
+		stf_isp_reg_write(stfcamss, reg_add, par_val);
+		reg_add +=3D 0x20;
+	}
+
+	par_h =3D 0x200; par_l =3D 0x180;
+	par_val =3D OCEF_PAR_H(par_h) | OCEF_PAR_L(par_l);
+	for (reg_add =3D ISP_REG_OECF_X0_CFG5; reg_add <=3D ISP_REG_OECF_Y3_CFG=
5;) {
+		stf_isp_reg_write(stfcamss, reg_add, par_val);
+		reg_add +=3D 0x20;
+	}
+
+	par_h =3D 0x300; par_l =3D 0x280;
+	par_val =3D OCEF_PAR_H(par_h) | OCEF_PAR_L(par_l);
+	for (reg_add =3D ISP_REG_OECF_X0_CFG6; reg_add <=3D ISP_REG_OECF_Y3_CFG=
6;) {
+		stf_isp_reg_write(stfcamss, reg_add, par_val);
+		reg_add +=3D 0x20;
+	}
+
+	par_h =3D 0x3fe; par_l =3D 0x380;
+	par_val =3D OCEF_PAR_H(par_h) | OCEF_PAR_L(par_l);
+	for (reg_add =3D ISP_REG_OECF_X0_CFG7; reg_add <=3D ISP_REG_OECF_Y3_CFG=
7;) {
+		stf_isp_reg_write(stfcamss, reg_add, par_val);
+		reg_add +=3D 0x20;
+	}
+
+	par_h =3D 0x80; par_l =3D 0x80;
+	par_val =3D OCEF_PAR_H(par_h) | OCEF_PAR_L(par_l);
+	for (reg_add =3D ISP_REG_OECF_S0_CFG0; reg_add <=3D ISP_REG_OECF_S3_CFG=
7;) {
+		stf_isp_reg_write(stfcamss, reg_add, par_val);
+		reg_add +=3D 4;
+	}
...
+
+	for (reg_add =3D ISP_REG_LCCF_CFG_2; reg_add <=3D ISP_REG_LCCF_CFG_5;) =
{
...
+
+	symbol_h =3D 0x0; symbol_l =3D 0x0;
+	reg_val =3D AWB_X_SYMBOL_H(symbol_h) | AWB_X_SYMBOL_L(symbol_l);
+
+	for (reg_add =3D ISP_REG_AWB_X0_CFG_0; reg_add <=3D ISP_REG_AWB_X3_CFG_=
1;) {
+		stf_isp_reg_write(stfcamss, reg_add, reg_val);
+		reg_add +=3D 4;
+	}
+
+	symbol_h =3D 0x0, symbol_l =3D 0x0;
+	reg_val =3D AWB_Y_SYMBOL_H(symbol_h) | AWB_Y_SYMBOL_L(symbol_l);
+
+	for (reg_add =3D ISP_REG_AWB_Y0_CFG_0; reg_add <=3D ISP_REG_AWB_Y3_CFG_=
1;) {
+		stf_isp_reg_write(stfcamss, reg_add, reg_val);
+		reg_add +=3D 4;
+	}
+
+	symbol_h =3D 0x80, symbol_l =3D 0x80;
+	reg_val =3D AWB_S_SYMBOL_H(symbol_h) | AWB_S_SYMBOL_L(symbol_l);
+
+	for (reg_add =3D ISP_REG_AWB_S0_CFG_0; reg_add <=3D ISP_REG_AWB_S3_CFG_=
1;) {
+		stf_isp_reg_write(stfcamss, reg_add, reg_val);
+		reg_add +=3D 4;
+	}
...
+
+	for (reg_add =3D ISP_REG_ICAMD_12; reg_add <=3D ISP_REG_ICAMD_20;) {
...
+
+	gamma_slope_v =3D 0x2400; gamma_v =3D 0x0;
+	reg_val =3D GAMMA_S_VAL(gamma_slope_v) | GAMMA_VAL(gamma_v);
+	stf_isp_reg_write(stfcamss, ISP_REG_GAMMA_VAL0, reg_val);
+
+	gamma_slope_v =3D 0x800; gamma_v =3D 0x20;
+	for (reg_add =3D ISP_REG_GAMMA_VAL1; reg_add <=3D ISP_REG_GAMMA_VAL7;) =
{
+		reg_val =3D GAMMA_S_VAL(gamma_slope_v) | GAMMA_VAL(gamma_v);
+		stf_isp_reg_write(stfcamss, reg_add, reg_val);
+		reg_add +=3D 4;
...
+
+	gamma_v =3D 0x100;
+	for (reg_add =3D ISP_REG_GAMMA_VAL8; reg_add <=3D ISP_REG_GAMMA_VAL13;)=
 {
+		reg_val =3D GAMMA_S_VAL(gamma_slope_v) | GAMMA_VAL(gamma_v);
+		stf_isp_reg_write(stfcamss, reg_add, reg_val);
+		reg_add +=3D 4;
...
+
+	y_curve =3D 0x0;
+	for (reg_add =3D ISP_REG_YCURVE_0; reg_add <=3D ISP_REG_YCURVE_63;) {
...
+
+	for (reg_add =3D ISP_REG_SHARPEN9; reg_add <=3D ISP_REG_SHARPEN14;) {
+		stf_isp_reg_write(sc, reg_add, S_WEIGHT(0xf));
+		reg_add +=3D 4;
+	}
+
+	for (reg_add =3D ISP_REG_SHARPEN_FS0; reg_add <=3D ISP_REG_SHARPEN_FS5;=
) {
...
+{
+	u32 bpp =3D stfcamss->isp_dev.current_fmt->bpp;
+	u32 val;
+
+	val =3D VSTART_CAP(crop->top) | HSTART_CAP(crop->left);
+	stf_isp_reg_write(stfcamss, ISP_REG_PIC_CAPTURE_START_CFG, val);
+
+	val =3D VEND_CAP(crop->height + crop->top - 1) |
+	      HEND_CAP(crop->width + crop->left - 1);
+	stf_isp_reg_write(stfcamss, ISP_REG_PIC_CAPTURE_END_CFG, val);
+
+	val =3D H_ACT_CAP(crop->height) | W_ACT_CAP(crop->width);
+	stf_isp_reg_write(stfcamss, ISP_REG_PIPELINE_XY_SIZE, val);
+
+	val =3D ALIGN(crop->width * bpp / 8, STFCAMSS_FRAME_WIDTH_ALIGN_8);
...
+
+static const struct stf_isp_format isp_formats_sink[] =3D {
...
+};
+
+static const struct stf_isp_format isp_formats_source[] =3D {
...
+stf_g_fmt_by_mcode(const struct stf_isp_format_table *fmt_table, u32 mco=
de)
+{
+	unsigned int i;
+
+	for (i =3D 0; i < fmt_table->nfmts; i++) {
+		if (fmt_table->fmts[i].code =3D=3D mcode)
...
+
+	isp_dev->stfcamss =3D stfcamss;
+	isp_dev->formats =3D isp_formats_st7110;
+	isp_dev->nformats =3D ARRAY_SIZE(isp_formats_st7110);
+	isp_dev->current_fmt =3D &isp_formats_source[0];
+
+	return 0;
+}
+
...
+	fmt =3D v4l2_subdev_get_pad_format(sd, sd_state, STF_ISP_PAD_SINK);
+	crop =3D v4l2_subdev_get_pad_crop(sd, sd_state, STF_ISP_PAD_SRC);
+
+	if (enable) {
...
+		fmt->code =3D formats->fmts[0].code;
+}
+
...
+	const struct stf_isp_format_table *formats;
+
+	if (code->pad =3D=3D STF_ISP_PAD_SINK) {
+		formats =3D &isp_dev->formats[SINK_FORMATS_INDEX];
+		code->code =3D formats->fmts[code->index].code;
+	} else {
+		struct v4l2_mbus_framefmt *sink_fmt;
+
+		sink_fmt =3D v4l2_subdev_get_pad_format(sd, state,
+						      STF_ISP_PAD_SRC);
+
...
+		return -EINVAL;
+
+	format.code =3D fse->code;
+	format.width =3D 1;
+	format.height =3D 1;
+	isp_try_format(isp_dev, state, fse->pad, &format);
+	fse->min_width =3D STFCAMSS_FRAME_MIN_WIDTH;
+	fse->min_height =3D STFCAMSS_FRAME_MIN_HEIGHT;
+
+	if (format.code !=3D fse->code)
+		return -EINVAL;
+
+	format.code =3D fse->code;
+	format.width =3D -1;
+	format.height =3D -1;
+	isp_try_format(isp_dev, state, fse->pad, &format);
+	fse->max_width =3D STFCAMSS_FRAME_MAX_WIDTH;
+	fse->max_height =3D STFCAMSS_FRAME_MAX_HEIGHT;
+
+	return 0;
+}
+
...
+	struct v4l2_mbus_framefmt *format;
+
+	format =3D v4l2_subdev_get_pad_format(sd, state, fmt->pad);
+	if (!format)
+		return -EINVAL;
+
...
+	if (fmt->pad =3D=3D STF_ISP_PAD_SINK) {
+		struct v4l2_subdev_selection sel =3D { 0 };
+
+		/* Reset sink pad compose selection */
+		sel.which =3D fmt->which;
+		sel.pad =3D STF_ISP_PAD_SINK;
+		sel.target =3D V4L2_SEL_TGT_CROP;
+		sel.r.width =3D fmt->format.width;
+		sel.r.height =3D fmt->format.height;
...
+}
+
+static const struct v4l2_rect stf_frame_min_crop =3D {
+	.width =3D STFCAMSS_FRAME_MIN_WIDTH,
+	.height =3D STFCAMSS_FRAME_MIN_HEIGHT,
+	.top =3D 0,
+	.left =3D 0,
+};
+
...
+					   STF_ISP_PAD_SINK);
+
+	const struct v4l2_rect bounds =3D {
+		.width =3D fmt->width,
...
+			fmt.format =3D *v4l2_subdev_get_pad_format(sd, state,
+								 sel->pad);
+			sel->r.left =3D 0;
+			sel->r.top =3D 0;
+			sel->r.width =3D fmt.format.width;
+			sel->r.height =3D fmt.format.height;
+		} else if (sel->pad =3D=3D STF_ISP_PAD_SRC) {
+			rect =3D v4l2_subdev_get_pad_crop(sd, state, sel->pad);
+			sel->r =3D *rect;
+		}
+		break;
+
...
+		return -EINVAL;
+
+	if (sel->target =3D=3D V4L2_SEL_TGT_CROP &&
+	    sel->pad =3D=3D STF_ISP_PAD_SINK) {
+		struct v4l2_subdev_selection crop =3D { 0 };
+
+		rect =3D v4l2_subdev_get_pad_crop(sd, state, sel->pad);
+		if (!rect)
+			return -EINVAL;
...
+		crop.which =3D sel->which;
+		crop.pad =3D STF_ISP_PAD_SRC;
+		crop.target =3D V4L2_SEL_TGT_CROP;
+		crop.r =3D *rect;
+		isp_set_selection(sd, state, &crop);
+	} else if (sel->target =3D=3D V4L2_SEL_TGT_CROP &&
+		   sel->pad =3D=3D STF_ISP_PAD_SRC) {
+		struct v4l2_subdev_format fmt =3D { 0 };
+
+		rect =3D v4l2_subdev_get_pad_crop(sd, state, sel->pad);
+		if (!rect)
...
+		fmt.which =3D sel->which;
+		fmt.pad =3D STF_ISP_PAD_SRC;
+		fmt.format.width =3D rect->width;
+		fmt.format.height =3D rect->height;
...
+	.init_cfg =3D isp_init_formats,
+	.enum_mbus_code =3D isp_enum_mbus_code,
+	.enum_frame_size =3D isp_enum_frame_size,
...
+};
+
+static const struct media_entity_operations isp_media_ops =3D {
...
+	struct media_pad *pads =3D isp_dev->pads;
+	int ret;
+
...
+	ret =3D media_entity_pads_init(&sd->entity, STF_ISP_PAD_MAX, pads);
+	if (ret) {
