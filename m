...
Received: from vger.kernel.org (vger.kernel.org [209.132.180.67])
	by mail.lfdr.de (Postfix) with ESMTP id E705F454B8
	for <lists+devicetree@lfdr.de>; Fri, 14 Jun 2019 08:31:40 +0200 (CEST)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S1725789AbfFNGbk (ORCPT <rfc822;lists+devicetree@lfdr.de>);
        Fri, 14 Jun 2019 02:31:40 -0400
Received: from mail-ua1-f65.google.com ([209.85.222.65]:44563 "EHLO
        mail-ua1-f65.google.com" rhost-flags-OK-OK-OK-OK) by vger.kernel.org
        with ESMTP id S1725775AbfFNGbj (ORCPT
        <rfc822;devicetree@vger.kernel.org>); Fri, 14 Jun 2019 02:31:39 -0400
Received: by mail-ua1-f65.google.com with SMTP id 8so498886uaz.11;
        Thu, 13 Jun 2019 23:31:30 -0700 (PDT)
DKIM-Signature: v=1; a=rsa-sha256; c=relaxed/relaxed;
        d=gmail.com; s=20161025;
        h=mime-version:references:in-reply-to:from:date:message-id:subject:to
         :cc:content-transfer-encoding;
        bh=A0AfqkRXKcAIT2Myg1QwZkKUS91pYM/hWplqbIIUHQQ=;
        b=
X-Google-DKIM-Signature: v=1; a=rsa-sha256; c=relaxed/relaxed;
        ...
        bh=A0AfqkRXKcAIT2Myg1QwZkKUS91pYM/hWplqbIIUHQQ=;
        b=
X-Gm-Message-State: APjAAAXGYxlu+/UdL2REC0NYs05IseLzCyYAHVHn3E07I1ocNkr1evyD
        AwxRmVINo3vb9LimlkpAYl7S4E0vS2BbfkVouls=
X-Google-Smtp-Source: APXvYqw8vHsznMFdLojuERr9WpXi92rX5eVh0Mfs1jIJEJyKYPR3xHT6YvLy7SRvAwazjNYKo1XEsuveunWSy6mCjdU=
X-Received: by 2002:ab0:678d:: with SMTP id v13mr13443627uar.120.1560493889599;
 Thu, 13 Jun 2019 23:31:29 -0700 (PDT)
MIME-Version: 1.0
References: <1559729030-16390-1-git-send-email-pawell@cadence.com> <1559729030-16390-6-git-send-email-pawell@cadence.com>
In-Reply-To: <1559729030-16390-6-git-send-email-pawell@cadence.com>
From:   Jun Li <lijun.kernel@gmail.com>
Date:   Fri, 14 Jun 2019 14:31:07 +0800
Message-ID: <CAKgpwJVq5ZLAW+3zi4bGkKmL8Xi7HBmrnFfEPtwXHy9zMcJRAw@mail.gmail.com>
...
Pawel Laszczak <pawell@cadence.com> =E4=BA=8E2019=E5=B9=B46=E6=9C=885=E6=97=
=A5=E5=91=A8=E4=B8=89 =E4=B8=8B=E5=8D=886:07=E5=86=99=E9=81=93=EF=BC=9A
>
...
> +endif
> +
> +cdns3-$(CONFIG_USB_CDNS3_GADGET)       +=3D gadget.o ep0.o trace.o
...
> +
> +       /*
> +        * If driver can't read mode by means of usb_get_dr_mode function=
 then
> +        * chooses mode according with Kernel configuration. This setting

Only consider kernel configuration in case of can't read from  usb_get_dr_m=
ode?

> +        * can be restricted later depending on strap pin configuration.

So I think you'd better have one central place to get dr mode which conside=
r:
- usb_get_dr_mode
- kernel config
- strap pin
...
> +        * Driver try update this setting considering kernel configuratio=
n
> +        */
...
> +
> +       switch (role) {
> +       case CDNS3_ROLE_IDLE: /* from IDLE, we can change to HOST or GADG=
ET */
> +               if (!id)
...
> +               break;
> +       case CDNS3_ROLE_GADGET: /* from GADGET, we can only change to IDL=
E */
> +               if (!vbus)
> +                       role =3D CDNS3_ROLE_IDLE;
> +               break;
...
seems this "disable" file is only for debug purpose, need keep it in
formal patch?
...
> +
> +       cdns->phy =3D devm_phy_optional_get(dev, "cdns3,usbphy");
> +       if (IS_ERR(cdns->phy))
> +               return PTR_ERR(cdns->phy);
> +
> +       phy_init(cdns->phy);
> +       ret =3D phy_init(cdns->phy);
> +       if (ret) {
> +               dev_err(dev, "phy_init error\n");
> +               return ret;
> +       }
> +
> +       ret =3D phy_power_on(cdns->phy);
> +       if (ret) {
...
> +       { .compatible =3D "cdns,usb3-1.0.0" },
> +       { .compatible =3D "cdns,usb3-1.0.1" },

 Can't this version infor be read out from register?
...
> +
> +               if (ret) {
> +                       dev_err(cdns->dev, "timeout waiting for xhci_read=
y\n");
> +                       return ret;
> +               }
> +       } else {
...
> +
> +               dev_info(cdns->dev, "Waiting till Device mode is turned o=
n\n");
> +
...
> +               cdns3_handshake(&cdns->otg_regs->state, OTGSTATE_DEV_STAT=
E_MASK,
> +                               0, 2000000);

Don't care handshake result? the same comments for many other ones.
...
> +        * in register maps.
> +        * The first register in old version is command register and it's=
 read
> +        * only, so driver should read 0 from it. On the other hand, in v=
1
> +        * the first register contains device ID number which is not set =
to 0.
...
> new file mode 100644
> index 000000000000..dcc7a1b91e1b
> --- /dev/null
...
> +
> +       if (zlp) {
> +               priv_ep->trb_pool[0].control =3D TRB_CYCLE | TRB_TYPE(TRB=
_NORMAL);
...
It's better to use the ep num, not ctrl->wIndex.
...
Still need select ep before return ?
...
> +               if (request) {
> +                       cdns3_dbg(priv_ep->cdns3_dev, "Resume transfer fo=
r %s\n",
> +                                 priv_ep->name);
> +
...
request->complete() be called with spin lock hold?
...
> new file mode 100644
> index 000000000000..f3b22ec43c0a
> --- /dev/null
...
> +       int dma_index;
> +
> +       dma_index =3D readl(&priv_dev->regs->ep_traddr) - priv_ep->trb_po=
ol_dma;
> +
...
Maybe direct get cdns3_request, seems more often you need cdns3_request.
...
> +{
> +       return list_first_entry_or_null(list, struct cdns3_aligned_buf, l=
ist);
> +}
> +
> +/**
...
> +{
> +       priv_ep->free_trbs--;
> +       cdns3_ep_inc_trb(&priv_ep->enqueue, &priv_ep->pcs, priv_ep->num_t=
rbs);
> +}
> +
> +/**
...
> +{
> +       priv_ep->free_trbs++;
> +       cdns3_ep_inc_trb(&priv_ep->dequeue, &priv_ep->ccs, priv_ep->num_t=
rbs);
> +}
> +
...
> +               request =3D cdns3_next_request(&priv_ep->deferred_req_lis=
t);
...
> +                       priv_dev->run_garbage_colector =3D 1;
> +               }
> +
> +               buf->in_use =3D 1;
...
> +               u32 doorbell;
> +
> +               doorbell =3D !!(readl(&priv_dev->regs->ep_cmd) & EP_CMD_D=
RDY);
> +
> +               if (doorbell) {
> +                       priv_ep->wa1_cycle_bit =3D priv_ep->pcs ? TRB_CYC=
LE : 0;
...
> +               dma_index =3D cdns3_get_dma_pos(priv_dev, priv_ep);
> +
> +               /* Driver can't update LINK TRB if it is current processe=
d. */
...
> +               link_trb =3D priv_ep->trb_pool + (priv_ep->num_trbs - 1);
> +               /*
> +                * For TRs size equal 2 enabling TRB_CHAIN for epXin caus=
es
> +                * that DMA stuck at the LINK TRB.
> +                * On the other hand, removing TRB_CHAIN for longer TRs f=
or
> +                * epXout cause that DMA stuck after handling LINK TRB.
> +                * To eliminate this strange behavioral driver set TRB_CH=
AIN
...
> +                                   TRB_TYPE(TRB_LINK) | TRB_TOGGLE | ch_=
bit;
> +       }
> +
...
> +                       control |=3D pcs;
> +
> +               if (priv_ep->type =3D=3D USB_ENDPOINT_XFER_ISOC  && !priv=
_ep->dir) {
> +                       control |=3D TRB_IOC | TRB_ISP;
> +               } else {
> +                       /* for last element in TD or in SG list */
> +                       if (sg_iter =3D=3D (num_trb - 1) && sg_iter !=3D =
0)
...
> +       /*
> +        * Memory barrier - cycle bit must be set before other filds in t=
rb.
> +        */
> +       wmb();
> +
...
> +               /*
> +                * Until SW is not ready to handle the OUT transfer the I=
SO OUT
...
> +
> +               cdns3_dbg(priv_ep->cdns3_dev, "Update ep_trbaddr for %s t=
o %08x\n",
...
> +       if (TRBS_PER_SEGMENT =3D=3D 2 && priv_ep->type !=3D USB_ENDPOINT_=
XFER_ISOC) {
> +               handled =3D 1;
> +               goto finish;
> +       }
...
> +               priv_req =3D to_cdns3_request(request);
> +
> +               /* Re-select endpoint. It could be changed by other CPU d=
uring
...
> +                * OUT data packet. If host disable stream or lost some p=
acket
> +                * then the only way to finish all queued transfer is to =
do it
> +                * on TRBERR event.
> +                */
> +               if (priv_ep->type =3D=3D USB_ENDPOINT_XFER_ISOC &&
> +                   !priv_ep->wa1_set) {
> +                       if (!priv_ep->dir) {
> +                               u32 ep_cfg =3D readl(&priv_dev->regs->ep_=
cfg);
> +
...
> +                       } else {
> +                               cdns3_rearm_transfer(priv_ep, priv_ep->wa=
1_set);
> +                       }
> +               }
> +       }
...
> +       if (usb_ists & USB_ISTS_L1ENTI) {
> +               /*
> +                * WORKAROUND: CDNS3 controller has issue with hardware r=
esuming
> +                * from L1. To fix it, if any DMA transfer is pending dri=
ver
...
> +                       spin_unlock(&priv_dev->lock);
> +                       priv_dev->gadget_driver->suspend(&priv_dev->gadge=
t);
> +                       spin_lock(&priv_dev->lock);
> +               }
> +       }
...
> +
> +               list_for_each_entry_safe(buf, tmp, &priv_dev->aligned_buf=
_list,
> +                                        list) {
> +                       if (!buf->in_use) {
> +                               list_del(&buf->list);
> +
> +                               spin_unlock_irqrestore(&priv_dev->lock, f=
lags);
...
> +               }
> +
> +               priv_dev->run_garbage_colector =3D 0;
> +       }
> +
> +irqend:
> +       ep_ien =3D readl(&priv_dev->regs->ep_ien) | priv_dev->shadow_ep_e=
n;
...
> +                       priv_dev->onchip_mem_allocated_size -=3D priv_dev=
->out_mem_is_allocated;
> +                       priv_dev->onchip_mem_allocated_size +=3D size;
> +                       priv_dev->out_mem_is_allocated =3D size;
> +               }
> +       }
> +
...
> +               }
> +
> +               if (priv_ep->type =3D=3D USB_ENDPOINT_XFER_ISOC && !priv_=
ep->dir)
> +                       cdns3_set_register_bit(&regs->tdl_from_trb, mask)=
;
> +
...
> +
> +               if ((priv_dev->dev_ver =3D=3D DEV_VER_V2  && !priv_ep->di=
r) ||
> +                   priv_dev->dev_ver > DEV_VER_V2)
> +                       ep_cfg |=3D EP_CFG_TDL_CHK;
> +               break;
...
> +       priv_ep->interval =3D desc->bInterval ? BIT(desc->bInterval - 1) =
: 0;
> +
> +       spin_unlock_irqrestore(&priv_dev->lock, flags);
...
> +                              BIT(cdns3_ep_addr_to_index(bEndpointAddres=
s)));
> +
> +       writel(reg, &priv_dev->regs->ep_sts_en);
> +
> +       /*
> +        * For some versions of controller at some point during ISO OUT t=
raffic
> +        * DMA reads Transfer Ring for the EP which has never got doorbel=
l.
> +        * This issue was detected only on simulation, but to avoid this =
issue
> +        * driver add protection against it. To fix it driver enable ISO =
OUT
> +        * endpoint before setting DRBL. This special treatment of ISO OU=
T
...
> +
> +       if (enable)
> +               cdns3_set_register_bit(&priv_dev->regs->ep_cfg, EP_CFG_EN=
ABLE);
> +
> +       ep->desc =3D desc;
...
> +        * just queue request in deferred list. Transfer will be started =
in
...
> +       cdns3_select_ep(priv_dev, ep->desc->bEndpointAddress);
> +
> +       list_for_each_entry_safe(req, req_temp, &priv_ep->pending_req_lis=
t,
> +                                list) {
...
> +       }
> +
> +       list_for_each_entry_safe(req, req_temp, &priv_ep->deferred_req_li=
st,
> +                                list) {
> +               if (request =3D=3D req)
> +                       goto found;
...
> +
> +               link_trb->buffer =3D TRB_BUFFER(priv_ep->trb_pool_dma +
> +                                             (priv_req->start_trb * TRB_=
SIZE));
> +               link_trb->control =3D (link_trb->control & TRB_CYCLE) |
> +                                   TRB_TYPE(TRB_LINK) | TRB_CHAIN | TRB_=
TOGGLE;
> +       } else {
> +               priv_ep->flags |=3D EP_UPDATE_EP_TRBADDR;
> +       }
...
> +
> +       /*
> +        *Driver need modify LFPS minimal U1 Exit time for 0x00024505 rev=
ision
> +        * of controller
> +        */
> +       if (priv_dev->dev_ver =3D=3D DEV_VER_TI_V1) {
> +               u32 reg =3D readl(&regs->dbg_link1);
> +
...
Do you need spin lock for above?
...
> +                       if (ret) {
> +                               dev_err(priv_dev->dev, "Failed to init ep=
0\n");
> +                               goto err;
> +                       }
> +               } else {
> +                       snprintf(priv_ep->name, sizeof(priv_ep->name), "e=
p%d%s",
> +                                ep_number, !!ep_dir ? "in" : "out");
> +                       priv_ep->endpoint.name =3D priv_ep->name;
> +
> +                       usb_ep_set_maxpacket_limit(&priv_ep->endpoint,
> +                                                  CDNS3_EP_MAX_PACKET_LI=
MIT);
> +                       priv_ep->endpoint.max_streams =3D CDNS3_EP_MAX_ST=
REAMS;
...
> new file mode 100644
> index 000000000000..bab0cbbe13f4
> --- /dev/null
...
R_ISOC ? \
> +                                     TRBS_PER_ISOC_SEGMENT : TRBS_PER_SE=
GMENT)
> +/**
...
iver
> + * @run_garbage_colector: infroms that at least one element of aligned_b=
uf_list
> + *                        can be freed
...
> new file mode 100644
> index 000000000000..cc18deca7eec
> --- /dev/null
...
> +               vsnprintf(__get_str(msg), CDNS3_MSG_MAX, vaf->fmt, *vaf->=
va);
> +       ),
...
> +       ),
> +       TP_printk("%s", cdns3_decode_usb_irq(__get_str(str), __entry->spe=
ed,
> +                                            __entry->usb_ists))
> +);
> +
...
> +       TP_PROTO(struct cdns3_device *priv_dev, struct cdns3_endpoint *pr=
iv_ep),
...
> +               __get_str(name), __entry->req, __entry->buf, __entry->act=
ual,
> +               __entry->length,
> +               __entry->zero ? "zero | " : "",
...
;
> +       ),
> +       TP_printk("%s: req: %p, req buf %p, dma %08llx a_buf %p a_dma %08=
llx, size %d",
...
c);
> +       ),
> +       TP_printk("%s: trb %p, dma buf: 0x%08x, size: %ld, burst: %d ctrl=
...
> +
> +       TP_printk("%s",
> +                 cdns3_dbg_ring((struct cdns3_endpoint *)__get_str(priv_=
ep),
