...
Received: from vger.kernel.org (vger.kernel.org [209.132.180.67])
	by mail.lfdr.de (Postfix) with ESMTP id 6B8C7E0229
	for <lists+devicetree@lfdr.de>; Tue, 22 Oct 2019 12:34:51 +0200 (CEST)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S2388314AbfJVKet (ORCPT <rfc822;lists+devicetree@lfdr.de>);
        Tue, 22 Oct 2019 06:34:49 -0400
Received: from mail.kernel.org ([198.145.29.99]:48394 "EHLO mail.kernel.org"
        rhost-flags-OK-OK-OK-OK) by vger.kernel.org with ESMTP
        id S2388310AbfJVKet (ORCPT <rfc822;devicetree@vger.kernel.org>);
        Tue, 22 Oct 2019 06:34:49 -0400
Received: from archlinux (cpc149474-cmbg20-2-0-cust94.5-4.cable.virginm.net [82.4.196.95])
        (using TLSv1.2 with cipher ECDHE-RSA-AES256-GCM-SHA384 (256/256 bits))
        (No client certificate requested)
        by mail.kernel.org (Postfix) with ESMTPSA id 84BA020B7C;
        Tue, 22 Oct 2019 10:34:44 +0000 (UTC)
DKIM-Signature: v=1; a=rsa-sha256; c=relaxed/simple; d=kernel.org;
        s=default; t=1571740486;
        bh=1uPUuIVHNyq5BGT+Os55NkurAWa6ZwKmXcpXnVh++fI=;
        h=Date:From:To:Cc:Subject:In-Reply-To:References:From;
        b=2FPnTK3RFUOIsfUoUiqdJ+7XQcvTKRYolA0/RNAT989+sJqLRlhK4grUha7aHpsYr
         fLdPiMGHRBxmqTOZ9urSuX+Cn4jKra/wjyGkaVmklB5LuNouzwp/7mRrNepMBm8z+q
         ENsPYo9hOLQQbifFrtU5Jx8+y5T6bsgk9kdp5GTo=
Date:   Tue, 22 Oct 2019 11:34:41 +0100
...
Message-ID: <20191022113441.345654a1@archlinux>
In-Reply-To: <20191012120426.58e6679a@archlinux>
References: <20191011084038.45829-1-nuno.sa@analog.com>
        <20191012120426.58e6679a@archlinux>
...
On Sat, 12 Oct 2019 12:04:26 +0100
...
> > Measurement System. It measures a wide variety of temperature sensors a=
nd
> > digitally outputs the result, in =C2=B0C or =C2=B0F, with 0.1=C2=B0C ac=
curacy and
...
There is an issue with the of table that I've just fixed up which came
up in build tests.
...
> >  * For custom thermistors, excitation-current cannot be set to Auto ran=
ge.
> >=20
> > Changes in v3:
> >  * Use normal `devm_request_irq`;
> >  * Handle and decode the new devicetree properties for sensor configura=
tion.
> >=20
> > Changes in v4:
...
> >  * __ltc2983_custom_sensor_new(): new argument to identify the property=
 name;
...
> > =20
> > +config LTC2983
> > +	tristate "Analog Devices Multi-Sensor Digital Temperature Measurement=
 System"
> > +	depends on SPI
> > +	select REGMAP_SPI
> > +	help
...
> > diff --git a/drivers/iio/temperature/ltc2983.c b/drivers/iio/temperatur=
e/ltc2983.c
...
> > +#define	LTC2983_STATUS_START(x)		FIELD_PREP(LTC2983_STATUS_START_MASK,=
 x)
> > +
...
> > + * the number of integer and fractional bits are specified. The resolu=
tion
...
> > +	/*
> > +	 * Check if the offset was assigned already. It should be for steinha=
rt
...
> > +	if (is_steinhart && new_custom->size !=3D LTC2983_CUSTOM_STEINHART_SI=
ZE) {
...
> > +}
> > +
> > +static int ltc2983_thermocouple_fault_handler(const struct ltc2983_dat=
a *st,
> > +					      const u32 result)
> > +{
...
> > +	}
> > +
> > +	ret =3D of_property_read_u32(phandle, "reg", &thermistor->r_sense_cha=
n);
> > +	if (ret) {
...
> > +	}
> > +
> > +	ret =3D of_property_read_u32(child, "adi,rsense-val-milli-ohms", &tem=
p);
> > +	if (ret) {
...
> > +}
> > +
> > +static struct ltc2983_sensor *ltc2983_adc_new(struct device_node *chil=
d,
> > +					 struct ltc2983_data *st,
> > +					 const struct ltc2983_sensor *sensor)
> > +{
...
> > +	ret =3D regmap_bulk_read(st->regmap, LTC2983_CHAN_RES_ADDR(sensor->ch=
an),
...
> > +		return ret;
> > +
> > +	*val =3D sign_extend32((*val) & LTC2983_DATA_MASK, LTC2983_DATA_SIGN_=
BIT);
> > +	return 0;
> > +}
> > +
...
> > +	st->sensors =3D devm_kcalloc(dev, st->num_channels, sizeof(*st->senso=
rs),
> > +				   GFP_KERNEL);
> > +	if (!st->sensors)
> > +		return -ENOMEM;
...
> > +
> > +/*
> > + *  The reg_bits are actually 12 but the device needs the first *compl=
ete*
...
> > +}
> > +
> > +static SIMPLE_DEV_PM_OPS(ltc2983_pm_ops, ltc2983_suspend, ltc2983_resu=
me);
> > +
