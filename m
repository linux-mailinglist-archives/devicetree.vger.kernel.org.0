...
Received: from vger.kernel.org (vger.kernel.org [23.128.96.18])
	by mail.lfdr.de (Postfix) with ESMTP id 5A24F1C9BF7
	for <lists+devicetree@lfdr.de>; Thu,  7 May 2020 22:16:23 +0200 (CEST)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S1728633AbgEGUQU (ORCPT <rfc822;lists+devicetree@lfdr.de>);
        Thu, 7 May 2020 16:16:20 -0400
Received: from relay11.mail.gandi.net ([217.70.178.231]:49507 "EHLO
        relay11.mail.gandi.net" rhost-flags-OK-OK-OK-OK) by vger.kernel.org
        with ESMTP id S1728596AbgEGUQT (ORCPT
        <rfc822;devicetree@vger.kernel.org>); Thu, 7 May 2020 16:16:19 -0400
Received: from aptenodytes (196.109.29.93.rev.sfr.net [93.29.109.196])
        (Authenticated sender: paul.kocialkowski@bootlin.com)
        by relay11.mail.gandi.net (Postfix) with ESMTPSA id 7C119100007;
        Thu,  7 May 2020 20:16:08 +0000 (UTC)
Date:   Thu, 7 May 2020 22:16:08 +0200
...
Subject: Re: [PATCH v5 2/2] drm: Add support for the LogiCVC display
 controller
Message-ID: <20200507201608.GI2422122@aptenodytes>
References: <20200403093617.760278-1-paul.kocialkowski@bootlin.com>
 ...
 <20200504112335.GH10381@phenom.ffwll.local>
MIME-Version: 1.0
Content-Type: multipart/signed; micalg=pgp-sha256;
        protocol="application/pgp-signature"; boundary="Q59ABw34pTSIagmi"
Content-Disposition: inline
In-Reply-To: <20200504112335.GH10381@phenom.ffwll.local>
...
> > > > Introduces a driver for the LogiCVC display controller, a programma=
ble
> > > > logic controller optimized for use in Xilinx Zynq-7000 SoCs and oth=
er
...
> > > > - Panels are only enabled after the first page flip to avoid flashi=
ng a
> > > >   white screen.
> > > > - Depth used in context of the LogiCVC driver only counts color com=
ponents
...
> > > > memory area, with a base address hardcoded for each layer. This req=
uires
> > > > using a dedicated CMA pool registered at the base address and tweak=
ing a
> > > > few offset-related registers to try to use any buffer allocated from
> > > > the pool. This is done on a best-effort basis to have the hardware =
cope
...
> > > > In particular, buffers allocated below the base address for a layer=
 are
> > > > guaranteed not to be configurable for that layer. See the implement=
ation of
...
> > >=20
> > > Just a few comments on stuff I've spotted since I'm working in this a=
rea.
> >=20
> > Thanks for the review (and the devm_drm_dev_alloc series)!
> >=20
> > I'm preparing a new version taking in account your comments. However, I=
 didn't
...
Oh okay, I thought drm_bridge was strictly for "physical" bridges.
Thanks for the hint on that new API, I'll be making use of it in the next
revision. Hopefully with that, the driver will be ready!
...
> > > >  obj-y			+=3D i2c/
> > > > diff --git a/drivers/gpu/drm/logicvc/Kconfig b/drivers/gpu/drm/logi=
cvc/Kconfig
...
> > > > +	help
> > > > +	  DRM display driver for the logiCVC programmable logic block fro=
m Xylon
> > > > diff --git a/drivers/gpu/drm/logicvc/Makefile b/drivers/gpu/drm/log=
icvc/Makefile
> > > > new file mode 100644
...
> > > > @@ -0,0 +1,4 @@
> > > > +logicvc-drm-y +=3D logicvc_crtc.o logicvc_drm.o logicvc_interface.=
o \
> > > > +		 logicvc_layer.o logicvc_mode.o logicvc_of.o
> > > > +
> > > > +obj-$(CONFIG_DRM_LOGICVC) +=3D logicvc-drm.o
> > > > diff --git a/drivers/gpu/drm/logicvc/logicvc_crtc.c b/drivers/gpu/d=
rm/logicvc/logicvc_crtc.c
> > > > new file mode 100644
...
> > > > +
> > > > +	regmap_write(logicvc->regmap, LOGICVC_HSYNC_FRONT_PORCH_REG, hfp =
- 1);
> > > > +	regmap_write(logicvc->regmap, LOGICVC_HSYNC_REG, hsl - 1);
> > > > +	regmap_write(logicvc->regmap, LOGICVC_HSYNC_BACK_PORCH_REG, hbp -=
 1);
> > > > +	regmap_write(logicvc->regmap, LOGICVC_HRES_REG, hact - 1);
> > > > +
> > > > +	regmap_write(logicvc->regmap, LOGICVC_VSYNC_FRONT_PORCH_REG, vfp =
- 1);
> > > > +	regmap_write(logicvc->regmap, LOGICVC_VSYNC_REG, vsl - 1);
> > > > +	regmap_write(logicvc->regmap, LOGICVC_VSYNC_BACK_PORCH_REG, vbp -=
 1);
...
> > > > +}
> > > > +
> > > > +static const struct drm_crtc_helper_funcs logicvc_crtc_helper_func=
s =3D {
...
> > > > +	int ret;
> > > > +
> > > > +	crtc =3D devm_kzalloc(logicvc->drm->dev, sizeof(*crtc), GFP_KERNE=
L);
> > > > +	if (!crtc)
> > > > +		return -ENOMEM;
> > > > +
...
> > > > +	return 0;
> > > > +}
> > > > diff --git a/drivers/gpu/drm/logicvc/logicvc_crtc.h b/drivers/gpu/d=
rm/logicvc/logicvc_crtc.h
> > > > new file mode 100644
...
> > >=20
> > > You have these sprinkled all over, they're redundant imo just upcast =
the
> > > drm_device pointer that all the base class structures have already.
> > > There's a bunch of these all over. If you want to a nice little funct=
ion
> > > to do the pointer chasing&casting so you can go directly from logicvc=
_crtc
> > > to logicvc_drm.
> > >=20
> > > > +
...
> > > > +
> > > > +#endif
> > > > diff --git a/drivers/gpu/drm/logicvc/logicvc_drm.c b/drivers/gpu/dr=
m/logicvc/logicvc_drm.c
> > > > new file mode 100644
...
> > > > +
> > > > +static int logicvc_drm_gem_cma_dumb_create(struct drm_file *file_p=
riv,
...
> > > > +	args->pitch =3D logicvc->config.row_stride * DIV_ROUND_UP(args->b=
pp, 8);
> > > > +
...
> > > > +	logicvc_of_property_parse_bool(of_node, "xylon,layers-configurabl=
e",
> > > > +				       &config->layers_configurable);
> > > > +
> > > > +	ret =3D logicvc_of_property_parse_u32(of_node, "xylon,display-int=
erface",
> > > > +					    &config->display_interface);
> > > > +	if (ret)
> > > > +		return ret;
> > > > +
> > > > +	ret =3D logicvc_of_property_parse_u32(of_node, "xylon,display-col=
orspace",
> > > > +					    &config->display_colorspace);
> > > > +	if (ret)
> > > > +		return ret;
> > > > +
> > > > +	ret =3D logicvc_of_property_parse_u32(of_node, "xylon,display-dep=
th",
...
> > > > +	}
> > > > +
> > > > +	reserved_mem_node =3D of_parse_phandle(of_node, "memory-region", =
0);
...
> > >=20
> > > Uh that's a wrong pattern. Please use devm_drm_dev_init and kzalloc a=
nd
...
> > > > +	}
> > > > +
> > > > +	ret =3D devm_request_irq(drm->dev, irq, logicvc_drm_irq_handler, =
0,
...
> > > > +MODULE_LICENSE("GPL");
> > > > diff --git a/drivers/gpu/drm/logicvc/logicvc_drm.h b/drivers/gpu/dr=
m/logicvc/logicvc_drm.h
> > > > new file mode 100644
...
> > > > +
> > > > +#endif
> > > > diff --git a/drivers/gpu/drm/logicvc/logicvc_interface.c b/drivers/=
gpu/drm/logicvc/logicvc_interface.c
> > > > new file mode 100644
...
> > > > +
> > > > +	regmap_update_bits(interface->logicvc->regmap, LOGICVC_POWER_CTRL=
_REG,
...
> > > > +}
> > > > +
> > > > +static void logicvc_encoder_disable(struct drm_encoder *drm_encode=
r)
> > > > +{
> > > > +	struct logicvc_interface *interface =3D
> > > > +		logicvc_interface_from_drm_encoder(drm_encoder);
...
> > > > +}
> > > > +
> > > > +static const struct drm_encoder_helper_funcs logicvc_encoder_helpe=
r_funcs =3D {
...
> > > > +};
> > > > +
> > > > +static int logicvc_connector_get_modes(struct drm_connector *drm_c=
onnector)
> > > > +{
> > > > +	struct logicvc_interface *interface =3D
...
> > > > +}
> > > > +
> > > > +static const struct drm_connector_helper_funcs logicvc_connector_h=
elper_funcs =3D {
...
> > > > +};
> > > > +
> > > > +static int logicvc_interface_encoder_type(struct logicvc_drm *logi=
cvc)
> > > > +{
...
> > > > +}
> > > > +
> > > > +static int logicvc_interface_connector_type(struct logicvc_drm *lo=
gicvc)
> > > > +{
> > > > +	switch (logicvc->config.display_interface) {
> > > > +	case LOGICVC_DISPLAY_INTERFACE_LVDS_4BITS:
...
> > > > +}
> > > > +
> > > > +static bool logicvc_interface_native_connector(struct logicvc_drm =
*logicvc)
> > > > +{
> > > > +	switch (logicvc->config.display_interface) {
> > > > +	case LOGICVC_DISPLAY_INTERFACE_DVI:
...
> > > > +{
> > > > +	uint32_t possible_crtcs =3D drm_crtc_mask(&logicvc->crtc->drm_crt=
c);
> > > > +
...
> > > > +	bool native_connector =3D logicvc_interface_native_connector(logi=
cvc);
> > > > +	int ret;
> > > > +
...
> > > > +
> > > > +	ret =3D drm_of_find_panel_or_bridge(of_node, 1, 0, &interface->dr=
m_panel,
...
> > > > +	return ret;
> > > > +}
> > > > diff --git a/drivers/gpu/drm/logicvc/logicvc_interface.h b/drivers/=
gpu/drm/logicvc/logicvc_interface.h
> > > > new file mode 100644
...
> > > > +
> > > > +#endif
> > > > diff --git a/drivers/gpu/drm/logicvc/logicvc_layer.c b/drivers/gpu/=
drm/logicvc/logicvc_layer.c
> > > > new file mode 100644
...
> > > > +};
> > > > +
> > > > +/* What we call depth in this driver only counts color components,=
 not alpha.
> > > > + * This allows us to stay compatible with the LogiCVC bistream def=
initions. */
...
> > > > +}
> > > > +
> > > > +static void logicvc_plane_atomic_update(struct drm_plane *drm_plan=
e,
...
> > > > +
> > > > +	/* Vertical position must be set last to sync layer register chan=
ges. */
...
> > > > +			break;
> > > > +		case 16:
> > > > +			if (layer->config.colorspace =3D=3D LOGICVC_LAYER_COLORSPACE_YU=
V)
> > > > +				alpha_bits =3D 8;
> > > > +			else
> > > > +				alpha_bits =3D 6;
...
> > > > +}
> > > > +
> > > > +static void logicvc_plane_atomic_disable(struct drm_plane *drm_pla=
ne,
> > > > +					 struct drm_plane_state *old_state)
> > > > +{
> > > > +	struct logicvc_layer *layer =3D logicvc_layer(drm_plane);
...
> > > > +}
> > > > +
> > > > +static struct logicvc_layer_formats *logicvc_layer_formats_lookup(=
struct logicvc_layer *layer)
> > > > +{
> > > > +	bool alpha;
> > > > +	unsigned int i =3D 0;
> > > > +
> > > > +	alpha =3D (layer->config.alpha_mode =3D=3D LOGICVC_LAYER_ALPHA_PI=
XEL);
> > > > +
> > > > +	while (logicvc_layer_formats[i].formats) {
> > > > +		if (logicvc_layer_formats[i].colorspace =3D=3D layer->config.col=
orspace &&
...
> > > > +}
> > > > +
> > > > +static unsigned int logicvc_layer_formats_count(struct logicvc_lay=
er_formats *formats)
> > > > +{
...
> > > > +				       &config->primary);
> > > > +
> > > > +	ret =3D logicvc_of_property_parse_u32(of_node, "xylon,layer-color=
space",
> > > > +					    &config->colorspace);
> > > > +	if (ret)
> > > > +		return ret;
...
> > > > +		return ret;
> > > > +
> > > > +	ret =3D logicvc_of_property_parse_u32(of_node, "xylon,layer-alpha=
-mode",
> > > > +					    &config->alpha_mode);
> > > > +	if (ret)
> > > > +		return ret;
> > > > +
> > > > +	/* Memory offset is only relevant without layer address configura=
tion. */
> > > > +	if (logicvc->caps->layer_address)
> > > > +		return 0;
> > > > +
> > > > +	ret =3D logicvc_of_property_parse_u32(of_node, "xylon,layer-base-=
offset",
> > > > +					    &config->base_offset);
> > > > +	if (ret)
> > > > +		return ret;
> > > > +
> > > > +	ret =3D logicvc_of_property_parse_u32(of_node, "xylon,layer-buffe=
r-offset",
...
> > > > +}
> > > > +
> > > > +struct logicvc_layer *logicvc_layer_get_from_index(struct logicvc_=
drm *logicvc,
> > > > +						   u32 index)
> > > > +{
> > > > +	struct logicvc_layer *layer;
...
> > > > +}
> > > > +
> > > > +struct logicvc_layer *logicvc_layer_get_from_type(struct logicvc_d=
rm *logicvc,
...
> > > > +}
> > > > +
> > > > +struct logicvc_layer *logicvc_layer_get_primary(struct logicvc_drm=
 *logicvc)
> > > > +{
> > > > +	return logicvc_layer_get_from_type(logicvc, DRM_PLANE_TYPE_PRIMAR=
Y);
> > > > +}
> > > > +
...
> > > > +	int ret;
> > > > +
> > > > +	layer =3D devm_kzalloc(logicvc->drm->dev, sizeof(*layer), GFP_KER=
NEL);
> > > > +	if (!layer) {
> > > > +		ret =3D -ENOMEM;
> > > > +		goto error;
...
> > > > +
> > > > +	ret =3D drm_universal_plane_init(logicvc->drm, &layer->drm_plane,=
 0,
...
> > > > +	}
> > > > +
> > > > +	drm_plane_helper_add(&layer->drm_plane, &logicvc_plane_helper_fun=
cs);
> > > > +
...
> > > > +	return ret;
> > > > +}
> > > > diff --git a/drivers/gpu/drm/logicvc/logicvc_layer.h b/drivers/gpu/=
drm/logicvc/logicvc_layer.h
> > > > new file mode 100644
...
> > > > +
> > > > +#endif
> > > > diff --git a/drivers/gpu/drm/logicvc/logicvc_mode.c b/drivers/gpu/d=
rm/logicvc/logicvc_mode.c
> > > > new file mode 100644
...
> > > > +#include "logicvc_mode.h"
> > > > +
> > > > +static void logicvc_mode_atomic_commit_tail(struct drm_atomic_stat=
e *old_state)
> > > > +{
...
> > > > +
> > > > +	/* Enable the panel after the first commit, which concerns our pa=
nel
...
> > > > +}
> > > > +
> > > > +static const struct drm_mode_config_helper_funcs logicvc_mode_conf=
ig_helper_funcs =3D {
> > > > +	.atomic_commit_tail	=3D logicvc_mode_atomic_commit_tail,
> > > > +};
> > > > +
> > > > +static const struct drm_mode_config_funcs logicvc_mode_config_func=
s =3D {
...
> > > > +{
> > > > +	struct drm_mode_config *mode_config =3D &logicvc->drm->mode_confi=
g;
> > > > +	struct logicvc_layer *layer_primary;
> > > > +	uint32_t preferred_depth;
> > > > +	int ret;
> > > > +
> > > > +	ret =3D drm_vblank_init(logicvc->drm, logicvc->drm->mode_config.n=
um_crtc);
> > > > +	if (ret) {
...
> > > > +}
> > > > diff --git a/drivers/gpu/drm/logicvc/logicvc_mode.h b/drivers/gpu/d=
rm/logicvc/logicvc_mode.h
> > > > new file mode 100644
...
> > > > +
> > > > +#endif
> > > > diff --git a/drivers/gpu/drm/logicvc/logicvc_of.c b/drivers/gpu/drm=
/logicvc/logicvc_of.c
> > > > new file mode 100644
...
> > > > +#include "logicvc_of.h"
> > > > +
> > > > +static struct logicvc_of_property_sv logicvc_of_display_interface_=
sv[] =3D {
...
> > > > +};
> > > > +
> > > > +static struct logicvc_of_property_sv logicvc_of_display_colorspace=
_sv[] =3D {
...
> > > > +};
> > > > +
> > > > +static struct logicvc_of_property_sv logicvc_of_layer_colorspace_s=
v[] =3D {
...
> > > > +};
> > > > +
> > > > +static struct logicvc_of_property_sv logicvc_of_layer_alpha_mode_s=
v[] =3D {
...
> > > > +};
> > > > +
> > > > +static int logicvc_of_property_sv_value(struct logicvc_of_property=
_sv *sv,
...
> > > > +}
> > > > +
> > > > +static struct logicvc_of_property *logicvc_of_property_lookup(cons=
t char *name)
> > > > +{
> > > > +	unsigned int i =3D 0;
> > > > +
...
> > > > +}
> > > > diff --git a/drivers/gpu/drm/logicvc/logicvc_of.h b/drivers/gpu/drm=
/logicvc/logicvc_of.h
> > > > new file mode 100644
...
> > > > +
> > > > +#endif
> > > > diff --git a/drivers/gpu/drm/logicvc/logicvc_regs.h b/drivers/gpu/d=
rm/logicvc/logicvc_regs.h
> > > > new file mode 100644
...

=aj0g
-----END PGP SIGNATURE-----
