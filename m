...
Received: from out1.vger.email (out1.vger.email [IPv6:2620:137:e000::1:20])
	by mail.lfdr.de (Postfix) with ESMTP id 59C1F73403E
	for <lists+devicetree@lfdr.de>; Sat, 17 Jun 2023 12:35:08 +0200 (CEST)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S1346313AbjFQKdd (ORCPT <rfc822;lists+devicetree@lfdr.de>);
        Sat, 17 Jun 2023 06:33:33 -0400
Received: from lindbergh.monkeyblade.net ([23.128.96.19]:55524 "EHLO
        lindbergh.monkeyblade.net" rhost-flags-OK-OK-OK-OK) by vger.kernel.org
        with ESMTP id S1346259AbjFQKdT (ORCPT
        <rfc822;devicetree@vger.kernel.org>); Sat, 17 Jun 2023 06:33:19 -0400
Received: from mailgw02.mediatek.com (unknown [210.61.82.184])
        by lindbergh.monkeyblade.net (Postfix) with ESMTPS id 2D151213C;
        Sat, 17 Jun 2023 03:33:12 -0700 (PDT)
X-UUID: 57dc67180cfa11eeb20a276fd37b9834-20230617
DKIM-Signature: v=1; a=rsa-sha256; q=dns/txt; c=relaxed/relaxed; d=mediatek.com; s=dk;
        h=Content-Type:Content-Transfer-Encoding:MIME-Version:References:In-Reply-To:Message-ID:Date:Subject:CC:To:From; bh=4BY8NFvrUmI5uIFJ0HxWxb4LSXl2kWXyUdHHd5+k+QM=;
        b=T09PvMV7L7xBcMpZ1Gny8MAfjmRzsvbt8Ug2vZ9H1yX2ahQjwtU7gls2qaiIkEdIu8RkmDkWvrTzpjj1Bfdkwl20KocDC7fhm9oT1Zm4fJ9DQapeWgvKr64zB8YUngedblwh22/pmTrXweHgk1Wg10dSM2HGbyni+dvmF3n9+jI=;
X-CID-P-RULE: Release_Ham
X-CID-O-INFO: VERSION:1.1.26,REQID:657e6deb-076a-4875-a6b6-5f349977300c,IP:0,U
        RL:0,TC:0,Content:-25,EDM:0,RT:0,SF:0,FILE:0,BULK:0,RULE:Release_Ham,ACTIO
        N:release,TS:-25
X-CID-META: VersionHash:cb9a4e1,CLOUDID:7e74ed3e-7aa7-41f3-a6bd-0433bee822f3,B
...
Received: from mtkmbs10n2.mediatek.inc [(172.21.101.183)] by mailgw02.mediatek.com
        ...
        with ESMTP id 1212464563; Sat, 17 Jun 2023 18:33:05 +0800
Received: from mtkmbs13n2.mediatek.inc (172.21.101.108) by
 mtkmbs11n1.mediatek.inc (172.21.101.185) with Microsoft SMTP Server
 (version=TLS1_2, cipher=TLS_ECDHE_RSA_WITH_AES_256_GCM_SHA384) id
 15.2.1118.26; Sat, 17 Jun 2023 18:33:03 +0800
Received: from mhfsdcap04.gcn.mediatek.inc (10.17.3.154) by
 mtkmbs13n2.mediatek.inc (172.21.101.73) with Microsoft SMTP Server id
 15.2.1118.26 via Frontend Transport; Sat, 17 Jun 2023 18:33:02 +0800
...
Subject: [PATCH v3,06/11] mediatek: vcodec: replace pr_* with dev_* for v4l2 debug message
Date:   Sat, 17 Jun 2023 18:32:50 +0800
Message-ID: <20230617103255.20239-7-yunfei.dong@mediatek.com>
...
---
 .../platform/mediatek/vcodec/mtk_vcodec_dec.c | 115 +++++-----
 ...
 .../vcodec/mtk_vcodec_dec_stateless.c         |  60 ++---
 ...
 23 files changed, 414 insertions(+), 440 deletions(-)

diff --git a/drivers/media/platform/mediatek/vcodec/mtk_vcodec_dec.c b/drivers/media/platform/mediatek/vcodec/mtk_vcodec_dec.c
index 93fcea821001..d3930e8030de 100644
...
@@ -540,11 +537,12 @@ static int vidioc_vdec_s_fmt(struct file *file, void *priv,
...
@@ -570,14 +568,11 @@ static int vidioc_enum_framesizes(struct file *file, void *priv,
...
@@ -641,7 +636,7 @@ static int vidioc_vdec_g_fmt(struct file *file, void *priv,
...
@@ -712,8 +707,8 @@ static int vidioc_vdec_g_fmt(struct file *file, void *priv,
...
+				  ctx->id, f->type, ctx->state);
 	...
 	return 0;
@@ -730,7 +725,7 @@ int vb2ops_vdec_queue_setup(struct vb2_queue *vq, unsigned int *nbuffers,
...
@@ -756,10 +751,9 @@ int vb2ops_vdec_queue_setup(struct vb2_queue *vq, unsigned int *nbuffers,
...
@@ -770,16 +764,15 @@ int vb2ops_vdec_buf_prepare(struct vb2_buffer *vb)
...
@@ -807,7 +800,7 @@ void vb2ops_vdec_buf_finish(struct vb2_buffer *vb)
...
@@ -843,8 +836,8 @@ void vb2ops_vdec_stop_streaming(struct vb2_queue *q)
...
@@ -870,17 +863,17 @@ void vb2ops_vdec_stop_streaming(struct vb2_queue *q)
 		 */
...
@@ -905,7 +898,7 @@ static int m2mops_vdec_job_ready(void *m2m_priv)
 {
...
+	mtk_v4l2_vdec_dbg(3, ctx, "[%d]", ctx->id);
 	...
 	if (ctx->state == MTK_STATE_ABORT)
 		return 0;
@@ -973,7 +966,7 @@ int mtk_vcodec_dec_queue_init(void *priv, struct vb2_queue *src_vq,
...
@@ -988,7 +981,7 @@ int mtk_vcodec_dec_queue_init(void *priv, struct vb2_queue *src_vq,
...
@@ -1004,7 +997,7 @@ int mtk_vcodec_dec_queue_init(void *priv, struct vb2_queue *src_vq,
...
index 03721eda2769..1056a661f981 100644
...
@@ -71,9 +70,9 @@ static irqreturn_t mtk_vcodec_dec_irq_handler(int irq, void *priv)
...
@@ -96,7 +95,7 @@ static int mtk_vcodec_get_reg_bases(struct mtk_vcodec_dev *dev)
...
+		dev_dbg(&pdev->dev, "reg[%d] base=%p", i, dev->reg_base[i]);
 	...
 	return 0;
@@ -156,7 +155,7 @@ static int fops_vcodec_open(struct file *file)
...
@@ -176,14 +175,14 @@ static int fops_vcodec_open(struct file *file)
...
+		mtk_v4l2_vdec_err(ctx, "Failed to v4l2_m2m_ctx_init() (%d)",
 			ret);
...
@@ -218,8 +217,7 @@ static int fops_vcodec_open(struct file *file)
...
@@ -241,7 +239,7 @@ static int fops_vcodec_release(struct file *file)
...
+	mtk_v4l2_vdec_dbg(0, ctx, "[%d] decoder", ctx->id);
 	...
 	/*
@@ -296,7 +294,7 @@ static int mtk_vcodec_probe(struct platform_device *pdev)
 					 &rproc_phandle)) {
 		fw_type = SCP;
 	} else {
...
@@ -316,7 +314,7 @@ static int mtk_vcodec_probe(struct platform_device *pdev)
...
@@ -332,13 +330,13 @@ static int mtk_vcodec_probe(struct platform_device *pdev)
...
@@ -359,7 +357,7 @@ static int mtk_vcodec_probe(struct platform_device *pdev)
...
@@ -368,7 +366,7 @@ static int mtk_vcodec_probe(struct platform_device *pdev)
...
@@ -377,7 +375,7 @@ static int mtk_vcodec_probe(struct platform_device *pdev)
...
@@ -390,7 +388,7 @@ static int mtk_vcodec_probe(struct platform_device *pdev)
...
@@ -409,21 +407,21 @@ static int mtk_vcodec_probe(struct platform_device *pdev)
...
index 0fbd030026c7..08e76387ad54 100644
...
index db1e14a1bd6c..d70cf4f405b3 100644
...
@@ -317,25 +318,26 @@ static void mtk_vdec_worker(struct work_struct *work)
...
index 7055954eb2af..08cefd87c889 100644
...
index a684a7e4ab6e..f20b8f652d1b 100644
...
index 5a864bcfe7ba..e92d0f05df41 100644
...
index f555341ae708..8d849916634f 100644
...
@@ -172,20 +170,20 @@ void vdec_msg_queue_update_ube_wptr(struct vdec_msg_queue *msg_queue, uint64_t u
 {
 	spin_lock(&msg_queue->lat_ctx.ready_lock);
...
-			       msg_queue->status);
+		mtk_v4l2_vdec_dbg(3, msg_queue->ctx, "wait buf full: list(%d %d) ready_num:%d status:%d",
+				  atomic_read(&msg_queue->lat_list_cnt),
+				  atomic_read(&msg_queue->core_list_cnt),
+				  msg_queue->lat_ctx.ready_num,
+				  msg_queue->status);
...
@@ -193,10 +191,10 @@ bool vdec_msg_queue_wait_lat_buf_full(struct vdec_msg_queue *msg_queue)
...
@@ -305,7 +303,7 @@ int vdec_msg_queue_init(struct vdec_msg_queue *msg_queue,
 					     ctx->picinfo.buf_h);
...
@@ -315,20 +313,21 @@ int vdec_msg_queue_init(struct vdec_msg_queue *msg_queue,
...
@@ -336,14 +335,14 @@ int vdec_msg_queue_init(struct vdec_msg_queue *msg_queue,
...
@@ -359,7 +358,7 @@ int vdec_msg_queue_init(struct vdec_msg_queue *msg_queue,
