...
Received: from out1.vger.email (out1.vger.email [IPv6:2620:137:e000::1:20])
	by mail.lfdr.de (Postfix) with ESMTP id DE41D6DDF69
	for <lists+devicetree@lfdr.de>; Tue, 11 Apr 2023 17:17:53 +0200 (CEST)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S231319AbjDKPRw (ORCPT <rfc822;lists+devicetree@lfdr.de>);
        Tue, 11 Apr 2023 11:17:52 -0400
Received: from lindbergh.monkeyblade.net ([23.128.96.19]:52642 "EHLO
        lindbergh.monkeyblade.net" rhost-flags-OK-OK-OK-OK) by vger.kernel.org
        with ESMTP id S231335AbjDKPRi (ORCPT
        <rfc822;devicetree@vger.kernel.org>); Tue, 11 Apr 2023 11:17:38 -0400
Received: from madras.collabora.co.uk (madras.collabora.co.uk [IPv6:2a00:1098:0:82:1000:25:2eeb:e5ab])
        by lindbergh.monkeyblade.net (Postfix) with ESMTPS id 63005619B;
        Tue, 11 Apr 2023 08:17:07 -0700 (PDT)
Received: from nicolas-tpx395.localdomain (unknown [IPv6:2606:6d00:15:199e::580])
        (using TLSv1.3 with cipher TLS_AES_256_GCM_SHA384 (256/256 bits)
         key-exchange X25519 server-signature RSA-PSS (4096 bits))
        (No client certificate requested)
        (Authenticated sender: nicolas)
        by madras.collabora.co.uk (Postfix) with ESMTPSA id 7AE8966031C2;
        Tue, 11 Apr 2023 16:15:56 +0100 (BST)
DKIM-Signature: v=1; a=rsa-sha256; c=relaxed/simple; d=collabora.com;
        s=mail; t=1681226157;
        bh=hKXlhLCaYvCfTEJz2/jhqxts6aPgvzMaxe40n05eiK4=;
        h=Subject:From:To:Cc:Date:In-Reply-To:References:From;
        b=
Message-ID: <2c18f5fb9c6fbaf8f96a6540e109050bc9bb367d.camel@collabora.com>
...
Date:   Tue, 11 Apr 2023 11:15:46 -0400
...
>=20
>=20
> (with Hans' requested changes of course)
>=20
Reviewed-by: Nicolas Dufresne <nicolas.dufresne@collabora.com>>=20
> ---
...
> index 000000000000..a2c9bcebd8bb
> --- /dev/null
...
> +#define GLOBAL_MODEL_SIZE	ALIGN(GM_GLOBAL_MODELS_PER_FRAME * GLOBAL_MODE=
L_TOTAL_SIZE, 2048)
...
> +
> +#define IS_INTRA(type) ((type =3D=3D V4L2_AV1_KEY_FRAME) || (type =3D=3D=
...
> +#define ALT2_BUF_IDX (V4L2_AV1_REF_ALTREF2_FRAME - V4L2_AV1_REF_LAST_FRA=
ME)
...
> +};
> +
> +static int rockchip_vpu981_get_frame_index(struct hantro_ctx *ctx, int r=
ef)
> +{
...
> +
> +	if (idx >=3D V4L2_AV1_TOTAL_REFS_PER_FRAME || idx < 0)
...
> +}
> +
> +static int rockchip_vpu981_get_order_hint(struct hantro_ctx *ctx, int re=
f)
> +{
> +	struct hantro_av1_dec_hw_ctx *av1_dec =3D &ctx->av1_dec;
...
> +			continue;
> +
> +		av1_dec->frame_refs[i].width =3D frame->frame_width_minus_1 + 1;
> +		av1_dec->frame_refs[i].height =3D frame->frame_height_minus_1 + 1;
> +		av1_dec->frame_refs[i].mi_cols =3D DIV_ROUND_UP(frame->frame_width_min=
us_1 + 1, 8);
> +		av1_dec->frame_refs[i].mi_rows =3D DIV_ROUND_UP(frame->frame_height_mi=
nus_1 + 1, 8);
...
> +		av1_dec->frame_refs[i].used =3D true;
...
> +}
> +
> +static void rockchip_vpu981_av1_dec_frame_unref(struct hantro_ctx *ctx, =
int idx)
> +{
> +	struct hantro_av1_dec_hw_ctx *av1_dec =3D &ctx->av1_dec;
> +
> +	if (idx >=3D 0)
> +		av1_dec->frame_refs[idx].used =3D false;
> +}
> +
...
> +}
> +
> +static int rockchip_vpu981_av1_dec_tiles_reallocate(struct hantro_ctx *c=
tx)
> +{
> +	struct hantro_dev *vpu =3D ctx->dev;
> +	struct hantro_av1_dec_hw_ctx *av1_dec =3D &ctx->av1_dec;
...
> +	size_t size;
> +
> +	if (av1_dec->db_data_col.size >=3D
> +	    ALIGN(height * 12 * ctx->bit_depth / 8, 128) * num_tile_cols)
> +		return 0;
> +
> +	rockchip_vpu981_av1_dec_tiles_free(ctx);
> +
> +	size =3D ALIGN(height * 12 * ctx->bit_depth / 8, 128) * num_tile_cols;
...
> +
> +	size =3D ALIGN(height_in_sb * 44 * ctx->bit_depth * 16 / 8, 128) * num_=
tile_cols;
...
> +
> +	size =3D ALIGN(stripe_num * 1536 * ctx->bit_depth / 8, 128) * num_tile_=
cols;
...
> +
> +	av1_dec->global_model.cpu =3D dma_alloc_coherent(vpu->dev, GLOBAL_MODEL=
_SIZE,
> +						       &av1_dec->global_model.dma,
> +						       GFP_KERNEL);
> +	if (!av1_dec->global_model.cpu)
...
> +	struct hantro_av1_dec_ctrls *ctrls =3D &av1_dec->ctrls;
> +
> +	ctrls->sequence =3D hantro_get_ctrl(ctx, V4L2_CID_STATELESS_AV1_SEQUENC=
E);
...
> +}
> +
> +static short rockchip_vpu981_av1_dec_resolve_divisor_32(u32 d, short *sh=
ift)
> +{
> +	int f;
> +	u64 e;
...
> +		return;
> +
> +	*alpha =3D clamp_val(mat[2] - (1 << WARPEDMODEL_PREC_BITS), S16_MIN, S1=
6_MAX);
> +	*beta =3D clamp_val(mat[3], S16_MIN, S16_MAX);
> +
> +	y =3D rockchip_vpu981_av1_dec_resolve_divisor_32(abs(mat[2]), &shift) *=
 (mat[2] < 0 ? -1 : 1);
> +
> +	gv =3D ((long long)mat[4] * (1 << WARPEDMODEL_PREC_BITS)) * y;
> +
> +	*gamma =3D clamp_val((int)AV1_DIV_ROUND_UP_POW2_SIGNED(gv, shift), S16_=
MIN, S16_MAX);
> +
> +	dv =3D ((long long)mat[3] * mat[4]) * y;
> +	*delta =3D clamp_val(mat[5] -
> +		(int)AV1_DIV_ROUND_UP_POW2_SIGNED(dv, shift) - (1 << WARPEDMODEL_PREC_=
BITS),
> +		S16_MIN, S16_MAX);
> +
> +	*alpha =3D AV1_DIV_ROUND_UP_POW2_SIGNED(*alpha, WARP_PARAM_REDUCE_BITS)
> +		 * (1 << WARP_PARAM_REDUCE_BITS);
> +	*beta =3D AV1_DIV_ROUND_UP_POW2_SIGNED(*beta, WARP_PARAM_REDUCE_BITS)
> +		* (1 << WARP_PARAM_REDUCE_BITS);
> +	*gamma =3D AV1_DIV_ROUND_UP_POW2_SIGNED(*gamma, WARP_PARAM_REDUCE_BITS)
> +		 * (1 << WARP_PARAM_REDUCE_BITS);
> +	*delta =3D AV1_DIV_ROUND_UP_POW2_SIGNED(*delta, WARP_PARAM_REDUCE_BITS)
> +		* (1 << WARP_PARAM_REDUCE_BITS);
> +}
> +
> +static void rockchip_vpu981_av1_dec_set_global_model(struct hantro_ctx *=
ctx)
> +{
> +	struct hantro_av1_dec_hw_ctx *av1_dec =3D &ctx->av1_dec;
> +	struct hantro_av1_dec_ctrls *ctrls =3D &av1_dec->ctrls;
...
> +	u8 *dst =3D av1_dec->global_model.cpu;
> +	struct hantro_dev *vpu =3D ctx->dev;
> +	int ref_frame, i;
> +
> +	memset(dst, 0, GLOBAL_MODEL_SIZE);
> +	for (ref_frame =3D 0; ref_frame < V4L2_AV1_REFS_PER_FRAME; ++ref_frame)=
 {
> +		s64 alpha =3D 0, beta =3D 0, gamma =3D 0, delta =3D 0;
> +
> +		for (i =3D 0; i < 6; ++i) {
> +			if (i =3D=3D 2)
> +				*(s32 *)dst =3D
...
> +		}
> +
> +		if (gm->type[V4L2_AV1_REF_LAST_FRAME + ref_frame] <=3D V4L2_AV1_WARP_M=
ODEL_AFFINE)
> +			rockchip_vpu981_av1_dec_get_shear_params(&gm->params[V4L2_AV1_REF_LAS=
T_FRAME + ref_frame][0],
...
> +	u8 *dst =3D av1_dec->tile_info.cpu;
> +	struct hantro_dev *vpu =3D ctx->dev;
> +	int tile0, tile1;
> +
...
> +			u32 x0 =3D tile_info.width_in_sbs_minus_1[tile0] + 1;
> +
...
> +
> +			// tile start position
> +			start =3D group_entry[tile_id].tile_offset - group_entry[0].tile_offs=
et;
> +			*dst++ =3D start & 255;
> +			*dst++ =3D (start >> 8) & 255;
> +			*dst++ =3D (start >> 16) & 255;
...
> +		hantro_reg_write(vpu, &av1_dec_tile_size_mag, tile_info.tile_size_byte=
s - 1);
> +	else
...
> +}
> +
> +static void rockchip_vpu981_av1_dec_set_frame_sign_bias(struct hantro_ct=
x *ctx)
> +{
> +	struct hantro_av1_dec_hw_ctx *av1_dec =3D &ctx->av1_dec;
> +	struct hantro_av1_dec_ctrls *ctrls =3D &av1_dec->ctrls;
...
> +	for (i =3D 0; i < V4L2_AV1_TOTAL_REFS_PER_FRAME - 1; i++) {
...
> +	}
> +
> +	dst =3D vb2_to_hantro_decoded_buf(&av1_dec->frame_refs[idx].vb2_ref->vb=
2_buf);
...
> +}
> +
> +static void rockchip_vpu981_av1_dec_set_segmentation(struct hantro_ctx *=
ctx)
> +{
> +	struct hantro_av1_dec_hw_ctx *av1_dec =3D &ctx->av1_dec;
> +	struct hantro_av1_dec_ctrls *ctrls =3D &av1_dec->ctrls;
...
> +	u32 segval[V4L2_AV1_MAX_SEGMENTS][V4L2_AV1_SEG_LVL_MAX] =3D { 0 };
> +	struct hantro_dev *vpu =3D ctx->dev;
> +	u8 segsign =3D 0, preskip_segid =3D 0, last_active_seg =3D 0, i, j;
> +
> +	if (!!(seg->flags & V4L2_AV1_SEGMENTATION_FLAG_ENABLED) &&
> +	    frame->primary_ref_frame < V4L2_AV1_REFS_PER_FRAME) {
> +		int idx =3D rockchip_vpu981_get_frame_index(ctx, frame->primary_ref_fr=
ame);
> +
> +		if (idx >=3D 0) {
> +			dma_addr_t luma_addr, mv_addr =3D 0;
> +			struct hantro_decoded_buffer *seg;
> +			size_t mv_offset =3D rockchip_vpu981_av1_dec_chroma_size(ctx);
> +
> +			seg =3D vb2_to_hantro_decoded_buf(&av1_dec->frame_refs[idx].vb2_ref->=
vb2_buf);
> +			luma_addr =3D hantro_get_dec_buf_addr(ctx, &seg->base.vb.vb2_buf);
> +			mv_addr =3D luma_addr + mv_offset;
> +
...
> +	struct hantro_av1_dec_ctrls *ctrls =3D &av1_dec->ctrls;
> +	const struct v4l2_ctrl_av1_frame *frame =3D ctrls->frame;
> +	const struct v4l2_av1_segmentation *segmentation =3D &frame->segmentati=
on;
> +	const struct v4l2_av1_quantization *quantization =3D &frame->quantizati=
on;
> +	int i;
> +
> +	for (i =3D 0; i < V4L2_AV1_MAX_SEGMENTS; i++) {
...
> +}
> +
> +static void rockchip_vpu981_av1_dec_set_loopfilter(struct hantro_ctx *ct=
x)
> +{
> +	struct hantro_av1_dec_hw_ctx *av1_dec =3D &ctx->av1_dec;
> +	struct hantro_av1_dec_ctrls *ctrls =3D &av1_dec->ctrls;
> +	const struct v4l2_ctrl_av1_frame *frame =3D ctrls->frame;
> +	const struct v4l2_av1_loop_filter *loop_filter =3D &frame->loop_filter;
> +	bool filtering_dis =3D (loop_filter->level[0] =3D=3D 0) && (loop_filter=
->level[1] =3D=3D 0);
> +	struct hantro_dev *vpu =3D ctx->dev;
> +
> +	hantro_reg_write(vpu, &av1_filtering_dis, filtering_dis);
> +	hantro_reg_write(vpu, &av1_filt_level_base_gt32, loop_filter->level[0] =
> 32);
...
> +	struct av1cdfs *out_cdfs =3D (struct av1cdfs *)av1_dec->prob_tbl_out.cp=
u;
> +	int i;
> +
...
> +						 av1_dec->cdfs);
> +	} else {
> +		rockchip_av1_get_cdfs(ctx, frame->ref_frame_idx[frame->primary_ref_fra=
me]);
> +	}
...
> +	struct hantro_dev *vpu =3D ctx->dev;
> +	u32 luma_pri_strength =3D 0;
> +	u16 luma_sec_strength =3D 0;
> +	u32 chroma_pri_strength =3D 0;
> +	u16 chroma_sec_strength =3D 0;
> +	int i;
> +
...
> +	struct hantro_dev *vpu =3D ctx->dev;
> +	u16 lr_type =3D 0, lr_unit_size =3D 0;
> +	u8 restoration_unit_size[V4L2_AV1_NUM_PLANES_MAX] =3D { 3, 3, 3 };
> +	int i;
> +
...
> +		    1 + loop_restoration->lr_unit_shift - loop_restoration->lr_uv_shif=
t;
...
> +}
> +
> +static void rockchip_vpu981_av1_dec_set_superres_params(struct hantro_ct=
x *ctx)
> +{
> +	struct hantro_av1_dec_hw_ctx *av1_dec =3D &ctx->av1_dec;
> +	struct hantro_av1_dec_ctrls *ctrls =3D &av1_dec->ctrls;
> +	const struct v4l2_ctrl_av1_frame *frame =3D ctrls->frame;
> +	struct hantro_dev *vpu =3D ctx->dev;
> +	u8 superres_scale_denominator =3D SCALE_NUMERATOR;
...
> +	upscaled_luma =3D frame->upscaled_width;
> +	downscaled_luma =3D width;
> +	downscaled_chroma =3D (downscaled_luma + 1) >> 1;
> +	upscaled_chroma =3D (upscaled_luma + 1) >> 1;
> +	step_luma =3D
...
> +		& RS_SCALE_SUBPEL_MASK;
> +	superres_luma_step =3D step_luma;
> +	superres_chroma_step =3D step_chroma;
...
> +		/ downscaled_chroma;
> +	superres_init_luma_subpel_x =3D initial_luma;
> +	superres_init_chroma_subpel_x =3D initial_chroma;
> +
> +set_regs:
...
> +}
> +
> +static void rockchip_vpu981_av1_dec_set_picture_dimensions(struct hantro=
_ctx *ctx)
> +{
> +	struct hantro_av1_dec_hw_ctx *av1_dec =3D &ctx->av1_dec;
> +	struct hantro_av1_dec_ctrls *ctrls =3D &av1_dec->ctrls;
> +	const struct v4l2_ctrl_av1_frame *frame =3D ctrls->frame;
> +	struct hantro_dev *vpu =3D ctx->dev;
> +	int pic_width_in_cbs =3D DIV_ROUND_UP(frame->frame_width_minus_1 + 1, 8=
);
> +	int pic_height_in_cbs =3D DIV_ROUND_UP(frame->frame_height_minus_1 + 1,=
 8);
...
> +}
> +
> +static void rockchip_vpu981_av1_dec_set_other_frames(struct hantro_ctx *=
ctx)
> +{
> +	struct hantro_av1_dec_hw_ctx *av1_dec =3D &ctx->av1_dec;
> +	struct hantro_av1_dec_ctrls *ctrls =3D &av1_dec->ctrls;
...
> +	int cur_mi_cols =3D DIV_ROUND_UP(frame->frame_width_minus_1 + 1, 8);
> +	int cur_mi_rows =3D DIV_ROUND_UP(frame->frame_height_minus_1 + 1, 8);
...
> +	int rf, idx;
> +
> +	alt_frame_offset =3D rockchip_vpu981_get_order_hint(ctx, ALT_BUF_IDX);
> +	gld_frame_offset =3D rockchip_vpu981_get_order_hint(ctx, GLD_BUF_IDX);
> +	bwd_frame_offset =3D rockchip_vpu981_get_order_hint(ctx, BWD_BUF_IDX);
> +	alt2_frame_offset =3D rockchip_vpu981_get_order_hint(ctx, ALT2_BUF_IDX)=
;
> +
...
> +
> +		if (!is_lst_overlay) {
> +			int lst_mi_cols =3D av1_dec->frame_refs[idx].mi_cols;
> +			int lst_mi_rows =3D av1_dec->frame_refs[idx].mi_rows;
...
> +	if (rockchip_vpu981_av1_dec_get_dist(ctx, bwd_frame_offset, cur_frame_o=
ffset) > 0) {
> +		int bwd_mi_cols =3D av1_dec->frame_refs[idx].mi_cols;
> +		int bwd_mi_rows =3D av1_dec->frame_refs[idx].mi_rows;
> +		bool bwd_intra_only =3D
> +		    IS_INTRA(av1_dec->frame_refs[idx].frame_type);
> +
> +		if (bwd_mi_cols =3D=3D cur_mi_cols && bwd_mi_rows =3D=3D cur_mi_rows &=
&
> +		    !bwd_intra_only) {
...
> +	if (rockchip_vpu981_av1_dec_get_dist(ctx, alt2_frame_offset, cur_frame_=
offset) > 0) {
> +		int alt2_mi_cols =3D av1_dec->frame_refs[idx].mi_cols;
> +		int alt2_mi_rows =3D av1_dec->frame_refs[idx].mi_rows;
> +		bool alt2_intra_only =3D
> +		    IS_INTRA(av1_dec->frame_refs[idx].frame_type);
> +
> +		if (alt2_mi_cols =3D=3D cur_mi_cols && alt2_mi_rows =3D=3D cur_mi_rows=
 &&
> +		    !alt2_intra_only) {
...
> +	if (rockchip_vpu981_av1_dec_get_dist(ctx, alt_frame_offset, cur_frame_o=
ffset) > 0 &&
> +	    ref_stamp >=3D 0) {
> +		int alt_mi_cols =3D av1_dec->frame_refs[idx].mi_cols;
> +		int alt_mi_rows =3D av1_dec->frame_refs[idx].mi_rows;
> +		bool alt_intra_only =3D
> +		    IS_INTRA(av1_dec->frame_refs[idx].frame_type);
> +
> +		if (alt_mi_cols =3D=3D cur_mi_cols && alt_mi_rows =3D=3D cur_mi_rows &=
&
> +		    !alt_intra_only) {
...
> +		int lst2_mi_cols =3D av1_dec->frame_refs[idx].mi_cols;
> +		int lst2_mi_rows =3D av1_dec->frame_refs[idx].mi_rows;
> +		bool lst2_intra_only =3D
> +		    IS_INTRA(av1_dec->frame_refs[idx].frame_type);
> +
> +		if (lst2_mi_cols =3D=3D cur_mi_cols && lst2_mi_rows =3D=3D cur_mi_rows=
 &&
> +		    !lst2_intra_only) {
...
> +	}
> +
> +	for (rf =3D 0; rf < V4L2_AV1_TOTAL_REFS_PER_FRAME - 1; ++rf) {
> +		idx =3D rockchip_vpu981_get_frame_index(ctx, rf);
> +		if (idx >=3D 0) {
> +			int rf_order_hint =3D rockchip_vpu981_get_order_hint(ctx, rf);
> +
> +			cur_offset[rf] =3D
> +			    rockchip_vpu981_av1_dec_get_dist(ctx, cur_frame_offset, rf_order_=
hint);
> +			cur_roffset[rf] =3D
> +			    rockchip_vpu981_av1_dec_get_dist(ctx, rf_order_hint, cur_frame_of=
fset);
> +		} else {
> +			cur_offset[rf] =3D 0;
> +			cur_roffset[rf] =3D 0;
...
> +	    cur_offset[mf_types[0] - V4L2_AV1_REF_LAST_FRAME] <=3D MAX_FRAME_DI=
STANCE &&
> +	    cur_offset[mf_types[0] - V4L2_AV1_REF_LAST_FRAME] >=3D -MAX_FRAME_D=
ISTANCE) {
> +		int rf =3D rockchip_vpu981_get_order_hint(ctx, refs_selected[0]);
> +		int idx =3D rockchip_vpu981_get_frame_index(ctx, refs_selected[0]);
> +		u32 *oh =3D av1_dec->frame_refs[idx].order_hints;
> +		int val;
> +
> +		hantro_reg_write(vpu, &av1_use_temporal0_mvs, 1);
> +
> +		val =3D rockchip_vpu981_av1_dec_get_dist(ctx, rf, oh[V4L2_AV1_REF_LAST=
_FRAME]);
...
> +
> +		val =3D rockchip_vpu981_av1_dec_get_dist(ctx, rf, oh[V4L2_AV1_REF_GOLD=
EN_FRAME]);
> +		hantro_reg_write(vpu, &av1_mf1_golden_offset, val);
> +
> +		val =3D rockchip_vpu981_av1_dec_get_dist(ctx, rf, oh[V4L2_AV1_REF_BWDR=
EF_FRAME]);
> +		hantro_reg_write(vpu, &av1_mf1_bwdref_offset, val);
> +
> +		val =3D rockchip_vpu981_av1_dec_get_dist(ctx, rf, oh[V4L2_AV1_REF_ALTR=
EF2_FRAME]);
...
> +	    cur_offset[mf_types[1] - V4L2_AV1_REF_LAST_FRAME] <=3D MAX_FRAME_DI=
STANCE &&
> +	    cur_offset[mf_types[1] - V4L2_AV1_REF_LAST_FRAME] >=3D -MAX_FRAME_D=
ISTANCE) {
> +		int rf =3D rockchip_vpu981_get_order_hint(ctx, refs_selected[1]);
...
> +	    cur_offset[mf_types[2] - V4L2_AV1_REF_LAST_FRAME] <=3D MAX_FRAME_DI=
STANCE &&
> +	    cur_offset[mf_types[2] - V4L2_AV1_REF_LAST_FRAME] >=3D -MAX_FRAME_D=
ISTANCE) {
> +		int rf =3D rockchip_vpu981_get_order_hint(ctx, refs_selected[2]);
...
> +
> +	hantro_reg_write(vpu, &av1_mf1_type, mf_types[0] - V4L2_AV1_REF_LAST_FR=
AME);
> +	hantro_reg_write(vpu, &av1_mf2_type, mf_types[1] - V4L2_AV1_REF_LAST_FR=
AME);
> +	hantro_reg_write(vpu, &av1_mf3_type, mf_types[2] - V4L2_AV1_REF_LAST_FR=
AME);
> +}
> +
> +static void rockchip_vpu981_av1_dec_set_reference_frames(struct hantro_c=
tx *ctx)
> +{
> +	struct hantro_av1_dec_hw_ctx *av1_dec =3D &ctx->av1_dec;
> +	struct hantro_av1_dec_ctrls *ctrls =3D &av1_dec->ctrls;
...
> +	bool allow_intrabc =3D !!(ctrls->frame->flags & V4L2_AV1_FRAME_FLAG_ALL=
OW_INTRABC);
...
> +
> +	for (i =3D V4L2_AV1_REF_LAST_FRAME; i < V4L2_AV1_TOTAL_REFS_PER_FRAME; =
i++) {
> +		u32 ref =3D i - 1;
> +		int idx =3D 0;
> +		int width, height;
...
> +}
> +
> +static void rockchip_vpu981_av1_dec_set_parameters(struct hantro_ctx *ct=
x)
> +{
> +	struct hantro_dev *vpu =3D ctx->dev;
> +	struct hantro_av1_dec_hw_ctx *av1_dec =3D &ctx->av1_dec;
...
> +			 !!(ctrls->frame->flags & V4L2_AV1_FRAME_FLAG_IS_MOTION_MODE_SWITCHAB=
LE));
...
> +			 !!(ctrls->sequence->flags & V4L2_AV1_SEQUENCE_FLAG_ENABLE_FILTER_INT=
RA));
...
> +			 !!(ctrls->sequence->flags & V4L2_AV1_SEQUENCE_FLAG_ENABLE_DUAL_FILTE=
R));
...
> +			 !!(ctrls->frame->flags & V4L2_AV1_FRAME_FLAG_ALLOW_SCREEN_CONTENT_TO=
OLS));
> +	hantro_reg_write(vpu, &av1_allow_intrabc,
> +			 !!(ctrls->frame->flags & V4L2_AV1_FRAME_FLAG_ALLOW_INTRABC));
> +
> +	if (!(ctrls->frame->flags & V4L2_AV1_FRAME_FLAG_ALLOW_SCREEN_CONTENT_TO=
OLS))
...
> +	hantro_reg_write(vpu, &av1_delta_q_res_log, ctrls->frame->quantization.=
delta_q_res);
...
> +	hantro_reg_write(vpu, &av1_quant_base_qindex, ctrls->frame->quantizatio=
n.base_q_idx);
> +	hantro_reg_write(vpu, &av1_bit_depth_y_minus8, ctx->bit_depth - 8);
> +	hantro_reg_write(vpu, &av1_bit_depth_c_minus8, ctx->bit_depth - 8);
> +
> +	hantro_reg_write(vpu, &av1_mcomp_filt_type, ctrls->frame->interpolation=
_filter);
...
> +
> +	hantro_reg_write(vpu, &av1_qp_delta_y_dc_av1, ctrls->frame->quantizatio=
n.delta_q_y_dc);
> +	hantro_reg_write(vpu, &av1_qp_delta_ch_dc_av1, ctrls->frame->quantizati=
on.delta_q_u_dc);
> +	hantro_reg_write(vpu, &av1_qp_delta_ch_ac_av1, ctrls->frame->quantizati=
on.delta_q_u_ac);
> +	if (ctrls->frame->quantization.flags & V4L2_AV1_QUANTIZATION_FLAG_USING=
_QMATRIX) {
...
> +	}
> +
> +	hantro_reg_write(vpu, &av1_lossless_e, rockchip_vpu981_av1_dec_is_lossl=
ess(ctx));
> +	hantro_reg_write(vpu, &av1_quant_delta_v_dc, ctrls->frame->quantization=
.delta_q_v_dc);
> +	hantro_reg_write(vpu, &av1_quant_delta_v_ac, ctrls->frame->quantization=
.delta_q_v_ac);
> +
> +	hantro_reg_write(vpu, &av1_skip_ref0,
> +			 (ctrls->frame->skip_mode_frame[0]) ? ctrls->frame->skip_mode_frame[0=
] : 1);
> +	hantro_reg_write(vpu, &av1_skip_ref1,
> +			 (ctrls->frame->skip_mode_frame[1]) ? ctrls->frame->skip_mode_frame[1=
] : 1);
> +
...
> +	size_t mv_offset =3D rockchip_vpu981_av1_dec_chroma_size(ctx);
> +
> +	vb2_dst =3D av1_dec->frame_refs[av1_dec->current_frame_index].vb2_ref;
