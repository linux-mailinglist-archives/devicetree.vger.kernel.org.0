...
Received: from out1.vger.email (out1.vger.email [IPv6:2620:137:e000::1:20])
	by mail.lfdr.de (Postfix) with ESMTP id 1250A4EFFF2
	for <lists+devicetree@lfdr.de>; Sat,  2 Apr 2022 11:05:18 +0200 (CEST)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S1353535AbiDBJHB (ORCPT <rfc822;lists+devicetree@lfdr.de>);
        Sat, 2 Apr 2022 05:07:01 -0400
Received: from lindbergh.monkeyblade.net ([23.128.96.19]:40958 "EHLO
        lindbergh.monkeyblade.net" rhost-flags-OK-OK-OK-OK) by vger.kernel.org
        with ESMTP id S1346651AbiDBJHB (ORCPT
        <rfc822;devicetree@vger.kernel.org>); Sat, 2 Apr 2022 05:07:01 -0400
Received: from 189.cn (ptr.189.cn [183.61.185.103])
        by lindbergh.monkeyblade.net (Postfix) with ESMTP id 8ECCC186165;
        Sat,  2 Apr 2022 02:05:04 -0700 (PDT)
HMM_SOURCE_IP: 10.64.8.41:50516.2041797902
HMM_ATTACHE_NUM: 0000
HMM_SOURCE_TYPE: SMTP
Received: from clientip-114.242.206.180 (unknown [10.64.8.41])
        by 189.cn (HERMES) with SMTP id DF3691002AB;
        Sat,  2 Apr 2022 17:04:41 +0800 (CST)
Received: from  ([172.27.8.53])
        by gateway-151646-dep-b7fbf7d79-9vctg with ESMTP id 8ca679e4d48646798473360894cfc73f for mripard@kernel.org;
        Sat, 02 Apr 2022 17:05:03 CST
X-Transaction-ID: 8ca679e4d48646798473360894cfc73f
...
Subject: [PATCH v15 5/6] drm/loongson: add drm driver for loongson display controller
Date:   Sat,  2 Apr 2022 17:02:51 +0800
Message-Id: <20220402090252.1700974-6-15330273260@189.cn>
...
v14:
  1) Drop depends on COMPILE_TEST in Kconfig
...
---
 ...
 20 files changed, 3555 insertions(+)
...
index 000000000000..6e3bbb36637a
--- /dev/null
...
new file mode 100644
index 000000000000..5259cc31c400
--- /dev/null
...
+		ret = pfuncs->compute(pixpll, crtc_state->mode.clock, &priv_state->pparams);
+		if (ret)
+			return 0;
+
+		drm_warn(crtc->dev, "Failed to find PLL parameters for %u\n",
...
+
+	lsdc_enable_display(ldev, dispipe->index);
+
...
+
+	lsdc_disable_display(ldev, dispipe->index);
+
...
+	else if (dispipe->index == 1)
+		writel(0, ldev->reg_base + LSDC_CRTC1_GAMMA_INDEX_REG);
+
+	for (i = 0; i < 256; i++) {
...
new file mode 100644
index 000000000000..5842ac436284
--- /dev/null
...
+	unsigned int i;
+
+	/* Deduce DC variants from the DC device node */
...
+
+	/* LS2K1000 has 0x7A05 */
+	if (pci_get_device(PCI_VENDOR_ID_LOONGSON, 0x7A05, NULL))
...
new file mode 100644
index 000000000000..ebfb355b1ee3
--- /dev/null
...
new file mode 100644
index 000000000000..b40264f9156d
--- /dev/null
...
+
+	snprintf(adapter->name, sizeof(adapter->name), "gpio-i2c");
+
...
+		return NULL;
+
+	drm_info(ddev, "%s: sda pin mask = %u, scl pin mask = %u\n",
...
new file mode 100644
index 000000000000..0557e57ea071
--- /dev/null
...
new file mode 100644
index 000000000000..a2f270162bfd
--- /dev/null
...
+#include "lsdc_i2c.h"
+
+static int loongson_use_vram_helper = -1;
+MODULE_PARM_DESC(use_vram_helper, "Using vram helper based driver(0 = disabled)");
+module_param_named(use_vram_helper, loongson_use_vram_helper, int, 0644);
+
+static int loongson_gamma = -1;
+MODULE_PARM_DESC(gamma, "enable gamma (-1 = disabled (default), >0 = enabled)");
+module_param_named(gamma, loongson_gamma, int, 0644);
+
+static int loongson_relax_alignment = -1;
+MODULE_PARM_DESC(relax_alignment,
+		 "relax crtc stride alignment (-1 = disabled (default), >0 = enabled)");
+module_param_named(relax_alignment, loongson_relax_alignment, int, 0644);
+
...
+
+	if (!np) {
+		ldev->has_of_graph = false;
+		dev_info(ldev->dev, "don't has DT support\n");
+		return;
+	}
+
+	ports = of_get_child_by_name(np, "ports");
+	ldev->has_of_graph = ports ? true : false;
+	of_node_put(ports);
+}
+
...
+	ldev->dev = dev;
+
+	if (loongson_use_vram_helper > 0)
+		ldev->use_vram_helper = true;
+	else if ((loongson_use_vram_helper < 0) && descp->has_vram)
+		ldev->use_vram_helper = true;
+	else
...
+
+	ldev->relax_alignment = loongson_relax_alignment > 0 ? true : false;
+
...
+	if (descp->has_builtin_i2c && ldev->has_of_graph) {
...
new file mode 100644
index 000000000000..c1bd22141d8f
--- /dev/null
...
+					     struct drm_atomic_state *state)
+{
+	struct lsdc_display_pipe *dispipe = lsdc_primary_to_dispipe(plane);
...
+	}
+
+	lsdc_update_fb_start_addr(ldev, dispipe->index, fb_addr);
+
+	lsdc_update_stride(ldev, dispipe->index, fb->pitches[0]);
+
+	if (drm_atomic_crtc_needs_modeset(crtc->state))
+		lsdc_update_fb_format(ldev, dispipe->index, fb->format);
+}
+
...
+				      struct drm_atomic_state *state)
+{
+	struct lsdc_display_pipe *dispipe = lsdc_cursor_to_dispipe(plane);
+	struct drm_device *ddev = plane->dev;
+	struct lsdc_device *ldev = to_lsdc(ddev);
+	struct drm_plane_state *new_plane_state = drm_atomic_get_new_plane_state(state, plane);
...
+	 * locate at CRTC-1, if bit 4 of LSDC_CURSOR_CFG_REG is 0, then
+	 * the cursor will be locate at CRTC-0.
+	 */
+	if (dispipe->index)
...
new file mode 100644
index 000000000000..ac67b06d54e3
--- /dev/null
