...
Received: from out1.vger.email (out1.vger.email [IPv6:2620:137:e000::1:20])
	by mail.lfdr.de (Postfix) with ESMTP id 9576E4B3BCA
	for <lists+devicetree@lfdr.de>; Sun, 13 Feb 2022 15:21:06 +0100 (CET)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S231705AbiBMOVJ (ORCPT <rfc822;lists+devicetree@lfdr.de>);
        Sun, 13 Feb 2022 09:21:09 -0500
Received: from mxb-00190b01.gslb.pphosted.com ([23.128.96.19]:40468 "EHLO
        lindbergh.monkeyblade.net" rhost-flags-OK-OK-OK-OK) by vger.kernel.org
        with ESMTP id S230026AbiBMOVJ (ORCPT
        <rfc822;devicetree@vger.kernel.org>); Sun, 13 Feb 2022 09:21:09 -0500
Received: from 189.cn (ptr.189.cn [183.61.185.101])
        by lindbergh.monkeyblade.net (Postfix) with ESMTP id D8F4E5F27C
        for <devicetree@vger.kernel.org>; Sun, 13 Feb 2022 06:20:59 -0800 (PST)
HMM_SOURCE_IP: 10.64.8.41:34274.536114013
HMM_ATTACHE_NUM: 0000
HMM_SOURCE_TYPE: SMTP
Received: from clientip-114.242.206.180 (unknown [10.64.8.41])
        by 189.cn (HERMES) with SMTP id 56E571001C8;
        Sun, 13 Feb 2022 22:16:54 +0800 (CST)
Received: from  ([114.242.206.180])
        by gateway-151646-dep-b7fbf7d79-9vctg with ESMTP id 5d4a8a72a11d40c7b1881991bcd97961 for mripard@kernel.org;
        Sun, 13 Feb 2022 22:16:56 CST
X-Transaction-ID: 5d4a8a72a11d40c7b1881991bcd97961
...
Subject: [PATCH v7 1/7] drm/lsdc: add drm driver for loongson display controller
Date:   Sun, 13 Feb 2022 22:16:43 +0800
Message-Id: <20220213141649.1115987-2-15330273260@189.cn>
...
---
 ...
 18 files changed, 3577 insertions(+)
...
index 000000000000..30e976f4f0f7
--- /dev/null
...
+	default y
+	help
+	  The LS7A1000 bridge chip has dedicated video RAM, When using this
+	  driver on LS7A1000 + LS3A3000/LS3A4000/LS3A5000 platform, the VRAM
+	  helper based solution is intended to be use by default. Select this
+	  option if you would like to encapsulate this driver mode. If you
+	  don't select this option and/or append "lsdc.use_vram_helper=0" at
+	  the kernel command line, Then, This driver mode will be disabled,
+	  This driver will just fall back to CMA helper based mode.
+
+	  If in doubt, say "Y".
...
new file mode 100644
index 000000000000..01766728564a
--- /dev/null
...
+	.hw_cursor_h = 64,
+	.have_builtin_i2c = true,
+	/* 64, 128, 256 is also supported */
+	.stride_alignment = 32,
+};
+
+struct lsdc_device_id {
+	char name[128];
+	const void *data;
+};
+
+static const struct lsdc_device_id lsdc_match[] = {
...
+	{ .name = "loongson,ls2k1000", .data = &dc_in_ls2k1000 },
+	{ .name = "loongson,loongson2k0500", .data = &dc_in_ls2k0500 },
+	{ /* sentinel */ },
+};
+
+static struct drm_framebuffer *
+lsdc_drm_gem_fb_create(struct drm_device *ddev, struct drm_file *file,
+		       struct drm_mode_fb_cmd2 const *mode_cmd)
+{
+	struct lsdc_device *ldev = to_lsdc(ddev);
+
...
+	const char *model;
+	const char *compatiable;
+	unsigned int i;
+
+	for (i = 0; i < ARRAY_SIZE(lsdc_match); ++i) {
+		compatiable = lsdc_match[i].name;
+
+		np = of_find_compatible_node(NULL, NULL, compatiable);
+		if (np) {
+			/* Just for informative */
+			of_property_read_string(np, "model", &model);
+
+			of_node_put(np);
+
+			ldev->desc = lsdc_match[i].data;
+
+			break;
+		}
+	}
...
+ */
+static bool lsdc_should_vram_helper_based(struct pci_dev *pdev)
+{
...
+			continue;
+
+		ret = of_property_read_bool(np, "has_dedicated_vram");
+		of_node_put(np);
+		break;
+	}
+
...
+	} else if ((lsdc_use_vram_helper != 0) && (lsdc_dirty_update == -1)) {
+		if (lsdc_should_vram_helper_based(pdev)) {
...
+		if (ldev->use_vram_helper || ldev->dirty_update)
+			lsdc_vram_init(ldev);
+
...
new file mode 100644
index 000000000000..8ac86ce5db87
--- /dev/null
...
new file mode 100644
index 000000000000..e704a1bd2eee
--- /dev/null
...
+			       void *src)
+{
+	unsigned int offset = drm_fb_clip_offset(fb->pitches[0], fb->format, clip);
+	void __iomem *dst = ldev->vram + offset;
+
