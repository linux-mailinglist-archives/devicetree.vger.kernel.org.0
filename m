...
Received: from vger.kernel.org (vger.kernel.org [209.132.180.67])
	by mail.lfdr.de (Postfix) with ESMTP id C03C91B1E0
	for <lists+devicetree@lfdr.de>; Mon, 13 May 2019 10:27:57 +0200 (CEST)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S1726218AbfEMI14 (ORCPT <rfc822;lists+devicetree@lfdr.de>);
        Mon, 13 May 2019 04:27:56 -0400
Received: from lb2-smtp-cloud8.xs4all.net ([194.109.24.25]:52285 "EHLO
        lb2-smtp-cloud8.xs4all.net" rhost-flags-OK-OK-OK-OK)
        by vger.kernel.org with ESMTP id S1725866AbfEMI14 (ORCPT
        <rfc822;devicetree@vger.kernel.org>);
        Mon, 13 May 2019 04:27:56 -0400
Received: from [192.168.2.10] ([46.9.252.75])
        by smtp-cloud8.xs4all.net with ESMTPA
        id Q6J4hi1ed0YQeQ6J7hcL6k; Mon, 13 May 2019 10:27:41 +0200
Subject: Re: [RFC PATCH V2 1/4] media: platform: mtk-isp: Add Mediatek sensor
 interface driver
...
References: <20190512060005.5444-1-louis.kuo@mediatek.com>
 <20190512060005.5444-2-louis.kuo@mediatek.com>
From:   Hans Verkuil <hverkuil@xs4all.nl>
Message-ID: <c5242808-8377-a954-df28-a42c67e898cf@xs4all.nl>
Date:   Mon, 13 May 2019 10:27:33 +0200
User-Agent: Mozilla/5.0 (X11; Linux x86_64; rv:60.0) Gecko/20100101
 Thunderbird/60.6.1
MIME-Version: 1.0
In-Reply-To: <20190512060005.5444-2-louis.kuo@mediatek.com>
Content-Type: text/plain; charset=utf-8
Content-Language: en-US
Content-Transfer-Encoding: 7bit
X-CMAE-Envelope: 
...
> index e6deb2597738..9fef94f4931f 100644
...
> index 000000000000..9d245f0388ef
> --- /dev/null
...
Please use an SPDX line instead of a full license notice. E.g.:
...
> new file mode 100644
> index 000000000000..bd8cd19ce434
> --- /dev/null
...
> new file mode 100644
> index 000000000000..affc5d1ef18a
> --- /dev/null
...
Drop the license text, it's no longer needed if you provide the SPDX line.
...
Huh? This function is unused, so why create it? Please drop it. If it is needed in
the future, then add the function when you actually need it.

> +
> +static int set_mux_ctrl(struct _seninf *priv,

Since you are ignoring the return value anyway, perhaps this should be a void
function?
...
Please add a break here as well.
...
Again, perhaps this should be a void function?
...
void function? This always returns 0.

Are you sure that this can never fail? I see a few 'calibration timeout' lines
here, and I assume that if the calibration fails you have a problem.

I suspect you are too optimistic (or lazy :-) ) by always returning 0.
...
If you set fmtidx to 0 in the default case, shouldn't priv->dpcm be set to 0 as well?
...
> +
> +	if (fmt->format.code == 0 || fmt->format.code == ~0U)
> +		fmt->format.code = MEDIA_BUS_FMT_SBGGR10_1X10;

Huh? Weird 'if' statement. Why this strange test?
...
I see no checks for V4L2_SUBDEV_FORMAT_TRY vs V4L2_SUBDEV_FORMAT_ACTIVE
in these two functions. Surely, that can't be right?
...
> +	pad = (node && !strcmp(node->name, SUBDEV_SENSOR_MAIN_NAME)) ?
> +		MTK_CAM_SENINF_PAD_MAIN_SINK : MTK_CAM_SENINF_PAD_SUB_SINK;
> +
> +	dev_info(priv->dev, "%s bounded with pad:%d\n", sd->entity.name, pad);
> +
...
Why #ifdef? If this is disabled, then clk_prepare_enable is never called and
this driver won't work. Just select this option in the Kconfig. Same for
...
Please use proper strings here that actually describe the test pattern.

Also use proper strings, e.g. instead of test_pattern_mode you'd write
"Test Pattern Mode", but then something that really describes the pattern,
since "Test Pattern Mode" is not helpful.
...
> +			return -EINVAL;
> +		}
> +		dev_info(dev, "bus.mipi_csi2.num_data_lanes %d\n",
...
> new file mode 100644
> index 000000000000..fef757db138d
> --- /dev/null
...
> new file mode 100644
> index 000000000000..3dde8f012610
> --- /dev/null
...
Don't add license text if you already have an SPDX line.
