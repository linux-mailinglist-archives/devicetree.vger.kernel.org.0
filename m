...
Received: from vger.kernel.org (vger.kernel.org [209.132.180.67])
	by mail.lfdr.de (Postfix) with ESMTP id 78313299BA
	for <lists+devicetree@lfdr.de>; Fri, 24 May 2019 16:05:48 +0200 (CEST)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S2403976AbfEXOFg (ORCPT <rfc822;lists+devicetree@lfdr.de>);
        Fri, 24 May 2019 10:05:36 -0400
Received: from metis.ext.pengutronix.de ([85.220.165.71]:45821 "EHLO
        metis.ext.pengutronix.de" rhost-flags-OK-OK-OK-OK) by vger.kernel.org
        with ESMTP id S2404089AbfEXOFf (ORCPT
        <rfc822;devicetree@vger.kernel.org>); Fri, 24 May 2019 10:05:35 -0400
Received: from dude02.hi.pengutronix.de ([2001:67c:670:100:1d::28] helo=dude02.pengutronix.de.)
        by metis.ext.pengutronix.de with esmtp (Exim 4.89)
        (envelope-from <p.zabel@pengutronix.de>)
        id 1hUAp3-0002Dk-6C; Fri, 24 May 2019 16:05:25 +0200
...
Subject: [PATCH 02/10] rockchip/vpu: rename from rockchip to hantro
Date:   Fri, 24 May 2019 16:04:51 +0200
Message-Id: <20190524140459.4002-3-p.zabel@pengutronix.de>
...
---
 ...
 .../vpu/rockchip_vpu.h => hantro/hantro.h}    | 132 ++++-----
 .../hantro_common.h}                          |  14 +-
 .../hantro_drv.c}                             | 253 +++++++++---------
 ...
 .../{rockchip/vpu => hantro}/rk3399_vpu_hw.c  |  60 ++---
 ...
 26 files changed, 616 insertions(+), 629 deletions(-)
 rename drivers/staging/media/{rockchip/vpu => hantro}/Kconfig (57%)
 create mode 100644 drivers/staging/media/hantro/Makefile
 rename drivers/staging/media/{rockchip/vpu => hantro}/TODO (100%)
 rename drivers/staging/media/{rockchip/vpu/rockchip_vpu.h => hantro/hantro.h} (70%)
 rename drivers/staging/media/{rockchip/vpu/rockchip_vpu_common.h => hantro/hantro_common.h} (56%)
...
index 878588cfb453..4889577a4ca1 100644
--- a/MAINTAINERS
+++ b/MAINTAINERS
@@ -13292,11 +13292,11 @@ S:	Maintained
...
index 277df2b39384..1db86e1ea7a6 100644
--- a/drivers/staging/media/Kconfig
+++ b/drivers/staging/media/Kconfig
@@ -27,7 +27,7 @@ source "drivers/staging/media/imx/Kconfig"
...
index 0355e3030504..bd2e68be2906 100644
--- a/drivers/staging/media/Makefile
+++ b/drivers/staging/media/Makefile
@@ -6,6 +6,6 @@ obj-$(CONFIG_VIDEO_OMAP4)	+= omap4iss/
...
index f645c19f10d1..ec6585db3abf 100644
...
index a5cf2c1bc75b..8c7dac01b239 100644
...
@@ -326,6 +326,6 @@ static inline u32 vdpu_read(struct rockchip_vpu_dev *vpu, u32 reg)
 	return val;
 }
...
+#endif /* HANTRO_H_ */
diff --git a/drivers/staging/media/rockchip/vpu/rockchip_vpu_common.h b/drivers/staging/media/hantro/hantro_common.h
similarity index 56%
rename from drivers/staging/media/rockchip/vpu/rockchip_vpu_common.h
rename to drivers/staging/media/hantro/hantro_common.h
index 96948ec49c18..73268b378437 100644
--- a/drivers/staging/media/rockchip/vpu/rockchip_vpu_common.h
+++ b/drivers/staging/media/hantro/hantro_common.h
@@ -1,6 +1,6 @@
 /* SPDX-License-Identifier: GPL-2.0 */
 /*
- * Rockchip VPU codec driver
...
+#ifndef HANTRO_COMMON_H_
+#define HANTRO_COMMON_H_
...
index f68bd71c6821..c1ed3a2eda3f 100644
...
+	src_vq->ops = &hantro_queue_ops;
 	...
 	/*
@@ -207,7 +204,7 @@ queue_init(void *priv, struct vb2_queue *src_vq, struct vb2_queue *dst_vq)
...
@@ -221,7 +218,7 @@ queue_init(void *priv, struct vb2_queue *src_vq, struct vb2_queue *dst_vq)
 	 *
...
@@ -233,7 +230,7 @@ queue_init(void *priv, struct vb2_queue *src_vq, struct vb2_queue *dst_vq)
...
@@ -242,12 +239,12 @@ queue_init(void *priv, struct vb2_queue *src_vq, struct vb2_queue *dst_vq)
...
@@ -262,14 +259,14 @@ static int rockchip_vpu_s_ctrl(struct v4l2_ctrl *ctrl)
 	return 0;
 }
...
+		.codec = HANTRO_JPEG_ENCODER,
 		.cfg = {
 			.min = 5,
 			.max = 100,
@@ -278,22 +275,22 @@ static struct rockchip_vpu_ctrl controls[] = {
 		},
 	}, {
...
@@ -304,7 +301,7 @@ static int rockchip_vpu_ctrls_setup(struct rockchip_vpu_dev *vpu,
 			continue;
...
+					  &hantro_ctrl_ops,
 					  ...
 					  controls[i].cfg.max,
 					  controls[i].cfg.step,
@@ -330,12 +327,12 @@ static int rockchip_vpu_ctrls_setup(struct rockchip_vpu_dev *vpu,
...
+	struct hantro_ctx *ctx;
 	...
 	/*
@@ -353,13 +350,13 @@ static int rockchip_vpu_open(struct file *filp)
...
@@ -375,9 +372,9 @@ static int rockchip_vpu_open(struct file *filp)
...
+	ret = hantro_ctrls_setup(vpu, ctx, allowed_codecs);
 	if (ret) {
 		...
 		goto err_fh_free;
@@ -393,10 +390,10 @@ static int rockchip_vpu_open(struct file *filp)
 	return ret;
 }
...
@@ -411,28 +408,27 @@ static int rockchip_vpu_release(struct file *filp)
 	return 0;
 }
...
+				  int function, struct video_device *vdev)
 {
 	char *name;
 	int ret;
@@ -462,8 +458,8 @@ static int rockchip_vpu_register_entity(struct media_device *mdev,
 	return 0;
 }
...
@@ -471,24 +467,24 @@ static int rockchip_attach_func(struct rockchip_vpu_dev *vpu,
...
@@ -554,7 +550,7 @@ static int rockchip_attach_func(struct rockchip_vpu_dev *vpu,
 	return ret;
 }
...
@@ -565,15 +561,14 @@ static void rockchip_detach_func(struct rockchip_vpu_func *func)
...
@@ -583,13 +578,13 @@ static int rockchip_vpu_add_func(struct rockchip_vpu_dev *vpu,
 	func->id = funcid;
...
@@ -606,7 +601,7 @@ static int rockchip_vpu_add_func(struct rockchip_vpu_dev *vpu,
 		return ret;
 	}
...
@@ -622,26 +617,26 @@ static int rockchip_vpu_add_func(struct rockchip_vpu_dev *vpu,
 	return ret;
 }
...
+	struct hantro_func *func;
 	...
 	if (funcid == MEDIA_ENT_F_PROC_VIDEO_ENCODER)
 		func = vpu->encoder;
@@ -651,29 +646,29 @@ static void rockchip_vpu_remove_func(struct rockchip_vpu_dev *vpu,
 	if (!func)
 		return;
...
@@ -686,10 +681,10 @@ static int rockchip_vpu_probe(struct platform_device *pdev)
...
@@ -780,16 +775,16 @@ static int rockchip_vpu_probe(struct platform_device *pdev)
...
@@ -804,9 +799,9 @@ static int rockchip_vpu_probe(struct platform_device *pdev)
 	return 0;
...
@@ -819,15 +814,15 @@ static int rockchip_vpu_probe(struct platform_device *pdev)
 	return ret;
 }
...
@@ -837,24 +832,24 @@ static int rockchip_vpu_remove(struct platform_device *pdev)
 	return 0;
 }
...
index 9fff5a653453..8f8660d459d0 100644
...
@@ -113,12 +112,12 @@ rk3288_vpu_mpeg2_dec_set_buffers(struct rockchip_vpu_dev *vpu,
...
@@ -160,9 +159,9 @@ rk3288_vpu_mpeg2_dec_set_buffers(struct rockchip_vpu_dev *vpu,
...
+	struct hantro_dev *vpu = ctx->dev;
 	...
 	const struct v4l2_ctrl_mpeg2_slice_params *slice_params;
 	const struct v4l2_mpeg2_sequence *sequence;
@@ -176,8 +175,8 @@ void rk3288_vpu_mpeg2_dec_run(struct rockchip_vpu_ctx *ctx)
...
@@ -244,11 +243,11 @@ void rk3288_vpu_mpeg2_dec_run(struct rockchip_vpu_ctx *ctx)
...
new file mode 100644
index 000000000000..40424217d44f
--- /dev/null
...
index 20c4cc77a01b..ebfb4db1f785 100644
...
@@ -266,8 +266,8 @@ static int vidioc_try_fmt_out_mplane(struct file *file, void *priv,
 }
...
@@ -280,16 +280,16 @@ rockchip_vpu_reset_fmt(struct v4l2_pix_format_mplane *fmt,
 }
...
@@ -297,7 +297,7 @@ rockchip_vpu_reset_encoded_fmt(struct rockchip_vpu_ctx *ctx)
 		fmt = &ctx->src_fmt;
 	}
...
@@ -306,16 +306,16 @@ rockchip_vpu_reset_encoded_fmt(struct rockchip_vpu_ctx *ctx)
 }
...
+	const struct hantro_fmt *raw_vpu_fmt, *encoded_vpu_fmt, *formats;
 	struct v4l2_pix_format_mplane *fmt;
 	unsigned int num_fmts;
...
@@ -325,24 +325,24 @@ rockchip_vpu_reset_raw_fmt(struct rockchip_vpu_ctx *ctx)
 		encoded_vpu_fmt = ctx->vpu_src_fmt;
 	}
 
-	rockchip_vpu_reset_fmt(fmt, raw_vpu_fmt);
+	hantro_reset_fmt(fmt, raw_vpu_fmt);
...
+	const struct hantro_fmt *formats;
 	...
 	struct vb2_queue *vq;
 	int ret;
@@ -352,7 +352,7 @@ vidioc_s_fmt_out_mplane(struct file *file, void *priv, struct v4l2_format *f)
...
+	if (!hantro_is_encoder_ctx(ctx)) {
 		...
 		/*
@@ -370,12 +370,12 @@ vidioc_s_fmt_out_mplane(struct file *file, void *priv, struct v4l2_format *f)
 	if (ret)
 		return ret;
...
@@ -384,7 +384,7 @@ vidioc_s_fmt_out_mplane(struct file *file, void *priv, struct v4l2_format *f)
...
@@ -397,8 +397,8 @@ static int vidioc_s_fmt_cap_mplane(struct file *file, void *priv,
 				   struct v4l2_format *f)
 {
 	struct v4l2_pix_format_mplane *pix_mp = &f->fmt.pix_mp;
-	struct rockchip_vpu_ctx *ctx = fh_to_ctx(priv);
...
+	const struct hantro_fmt *formats;
 	...
 	unsigned int num_fmts;
 	int ret;
@@ -408,7 +408,7 @@ static int vidioc_s_fmt_cap_mplane(struct file *file, void *priv,
 	if (vb2_is_streaming(vq))
 		return -EBUSY;
...
+	if (hantro_is_encoder_ctx(ctx)) {
 		...
 		/*
@@ -429,9 +429,9 @@ static int vidioc_s_fmt_cap_mplane(struct file *file, void *priv,
 	if (ret)
 		return ret;
...
@@ -445,8 +445,8 @@ static int vidioc_s_fmt_cap_mplane(struct file *file, void *priv,
...
@@ -460,7 +460,7 @@ static int vidioc_s_fmt_cap_mplane(struct file *file, void *priv,
 	return 0;
 }
...
@@ -489,13 +489,11 @@ const struct v4l2_ioctl_ops rockchip_vpu_ioctl_ops = {
 };
...
@@ -527,9 +525,8 @@ rockchip_vpu_queue_setup(struct vb2_queue *vq,
 }
...
+		       struct v4l2_pix_format_mplane *pixfmt)
 {
 	unsigned int sz;
 	int i;
@@ -546,38 +543,36 @@ rockchip_vpu_buf_plane_check(struct vb2_buffer *vb,
 	return 0;
 }
...
@@ -585,8 +580,8 @@ static int rockchip_vpu_start_streaming(struct vb2_queue *q,
 	else
 		ctx->sequence_cap = 0;
...
@@ -603,10 +598,10 @@ static int rockchip_vpu_start_streaming(struct vb2_queue *q,
 }
...
@@ -620,11 +615,11 @@ rockchip_vpu_return_bufs(struct vb2_queue *q,
 	}
 }
...
@@ -635,19 +630,19 @@ static void rockchip_vpu_stop_streaming(struct vb2_queue *q)
...
@@ -655,14 +650,14 @@ static int rockchip_vpu_buf_out_validate(struct vb2_buffer *vb)
 	return 0;
 }
...
index 2b3689968ef4..0a43e0777189 100644
...
index f9bbcd395b23..6587eb79342c 100644
...
+				 struct hantro_ctx *ctx,
 				 ...
 				 struct vb2_buffer *dst_buf,
 				 const struct v4l2_mpeg2_sequence *sequence,
@@ -115,12 +114,12 @@ rk3399_vpu_mpeg2_dec_set_buffers(struct rockchip_vpu_dev *vpu,
...
+					      slice_params->forward_ref_ts);
 	...
 	/* Source bitstream buffer */
@@ -162,9 +161,9 @@ rk3399_vpu_mpeg2_dec_set_buffers(struct rockchip_vpu_dev *vpu,
 	vdpu_write_relaxed(vpu, backward_addr, VDPU_REG_REFER3_BASE);
 }
...
+	struct hantro_dev *vpu = ctx->dev;
 	...
 	const struct v4l2_ctrl_mpeg2_slice_params *slice_params;
 	const struct v4l2_mpeg2_sequence *sequence;
@@ -178,8 +177,8 @@ void rk3399_vpu_mpeg2_dec_run(struct rockchip_vpu_ctx *ctx)
 	v4l2_ctrl_request_setup(src_buf->vb2_buf.req_obj.req,
 				&ctx->ctrl_handler);
...
index d5b833a15653..000000000000
...
deleted file mode 100644
index 6cecb528f994..000000000000
