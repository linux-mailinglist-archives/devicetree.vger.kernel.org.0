...
Received: from out1.vger.email (out1.vger.email [IPv6:2620:137:e000::1:20])
	by mail.lfdr.de (Postfix) with ESMTP id C584E55DFCD
	for <lists+devicetree@lfdr.de>; Tue, 28 Jun 2022 15:31:14 +0200 (CEST)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S233333AbiF0ID5 (ORCPT <rfc822;lists+devicetree@lfdr.de>);
        Mon, 27 Jun 2022 04:03:57 -0400
Received: from lindbergh.monkeyblade.net ([23.128.96.19]:38570 "EHLO
        lindbergh.monkeyblade.net" rhost-flags-OK-OK-OK-OK) by vger.kernel.org
        with ESMTP id S232176AbiF0IDy (ORCPT
        <rfc822;devicetree@vger.kernel.org>); Mon, 27 Jun 2022 04:03:54 -0400
Received: from mailgw01.mediatek.com (unknown [60.244.123.138])
        by lindbergh.monkeyblade.net (Postfix) with ESMTPS id A584C5F52;
        Mon, 27 Jun 2022 01:03:51 -0700 (PDT)
X-UUID: d2c0aa00aefc45738d189915954d7239-20220627
X-CID-P-RULE: Release_Ham
X-CID-O-INFO: VERSION:1.1.6,REQID:99e433df-ce3f-4d6a-b7ae-ad7851cde887,OB:0,LO
        B:0,IP:0,URL:0,TC:0,Content:-5,EDM:0,RT:0,SF:0,FILE:0,RULE:Release_Ham,ACT
        ION:release,TS:-5
X-CID-META: VersionHash:b14ad71,CLOUDID:8fb9212e-1756-4fa3-be7f-474a6e4be921,C
...
Received: from mtkexhb01.mediatek.inc [(172.21.101.102)] by mailgw01.mediatek.com
        ...
        with ESMTP id 16428909; Mon, 27 Jun 2022 16:03:45 +0800
...
Subject: [PATCH v12 09/10] drm/mediatek: DP audio support for MT8195
Date:   Mon, 27 Jun 2022 16:03:40 +0800
Message-ID: <20220627080341.5087-10-rex-bc.chen@mediatek.com>
...
---
 drivers/gpu/drm/mediatek/mtk_dp.c     | 705 ++++++++++++++++++++++++++
 drivers/gpu/drm/mediatek/mtk_dp_reg.h |   2 +
 2 files changed, 707 insertions(+)

diff --git a/drivers/gpu/drm/mediatek/mtk_dp.c b/drivers/gpu/drm/mediatek/mtk_dp.c
index 92cbcd403a73..62a9ebadd2a8 100644
--- a/drivers/gpu/drm/mediatek/mtk_dp.c
+++ b/drivers/gpu/drm/mediatek/mtk_dp.c
@@ -53,6 +53,7 @@ enum mtk_dp_train_state {
...
@@ -74,9 +75,41 @@ struct mtk_dp_train_info {
...
+	u8 connector_eld[MAX_ELD_BYTES];
 	...
 	bool need_debounce;
@@ -403,6 +449,125 @@ static void mtk_dp_pg_disable(struct mtk_dp *mtk_dp)
...
@@ -423,6 +588,169 @@ static bool mtk_dp_plug_state_avoid_pulse(struct mtk_dp *mtk_dp)
 	return true;
 }
...
@@ -1067,6 +1395,32 @@ static void mtk_dp_video_mute(struct mtk_dp *mtk_dp, bool enable)
...
@@ -1114,6 +1468,78 @@ static void mtk_dp_initialize_priv_data(struct mtk_dp *mtk_dp)
...
@@ -1130,6 +1556,8 @@ static void mtk_dp_setup_tu(struct mtk_dp *mtk_dp)
 	}
...
@@ -1145,10 +1573,14 @@ static void mtk_dp_calculate_pixrate(struct mtk_dp *mtk_dp)
...
@@ -1159,6 +1591,14 @@ static void mtk_dp_set_tx_out(struct mtk_dp *mtk_dp)
...
+
 ...
 {
 	ssize_t ret;
@@ -1192,6 +1632,7 @@ static ssize_t mtk_dp_hpd_sink_event(struct mtk_dp *mtk_dp)
...
@@ -1199,6 +1640,17 @@ static ssize_t mtk_dp_hpd_sink_event(struct mtk_dp *mtk_dp)
 	return 0;
 }
...
+
 ...
 {
@@ -1465,6 +1917,55 @@ static int mtk_dp_parse_capabilities(struct mtk_dp *mtk_dp)
 	return 0;
 }
...
@@ -1557,6 +2058,48 @@ static void mtk_dp_video_enable(struct mtk_dp *mtk_dp, bool enable)
 	}
 }
...
@@ -1583,6 +2126,19 @@ static int mtk_dp_training(struct mtk_dp *mtk_dp)
...
+
 			...
 			break;
 		default:
@@ -1644,12 +2200,15 @@ static irqreturn_t mtk_dp_hpd_event_thread(int hpd, void *dev)
...
+			mtk_dp_edid_free(mtk_dp);
 			mtk_dp_update_bits(mtk_dp, MTK_DP_TOP_PWR_STATE,
 					   ...
 					   DP_PWR_STATE_MASK);
@@ -1798,6 +2357,18 @@ static int mtk_dp_dt_parse(struct mtk_dp *mtk_dp,
 	return 0;
 }
...
@@ -1813,6 +2384,7 @@ static enum drm_connector_status mtk_dp_bdg_detect(struct drm_bridge *bridge)
...
@@ -1835,6 +2407,18 @@ static struct edid *mtk_dp_get_edid(struct drm_bridge *bridge,
 	if (!enabled)
...
@@ -2009,6 +2593,7 @@ static void mtk_dp_bridge_atomic_disable(struct drm_bridge *bridge,
 	}
...
@@ -2039,6 +2624,8 @@ static void mtk_dp_bridge_atomic_enable(struct drm_bridge *bridge,
 	}
...
@@ -2051,7 +2638,13 @@ static void mtk_dp_bridge_atomic_enable(struct drm_bridge *bridge,
 		return;
 	}
...
@@ -2198,6 +2791,104 @@ static void mtk_dp_debounce_timer(struct timer_list *t)
...
@@ -2243,8 +2934,21 @@ static int mtk_dp_probe(struct platform_device *pdev)
...
+
 	mtk_dp->phy_dev = platform_device_register_data(dev, "mediatek-dp-phy",
 							PLATFORM_DEVID_AUTO,
 							&mtk_dp->regs,
@@ -2290,6 +2994,7 @@ static int mtk_dp_remove(struct platform_device *pdev)
...
index 8171e5b03264..5377ccea8178 100644
