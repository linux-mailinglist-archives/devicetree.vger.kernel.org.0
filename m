...
Received: from vger.kernel.org (vger.kernel.org [23.128.96.18])
	by mail.lfdr.de (Postfix) with ESMTP id 5AAFD2FD941
	for <lists+devicetree@lfdr.de>; Wed, 20 Jan 2021 20:15:14 +0100 (CET)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S2387883AbhATTOu (ORCPT <rfc822;lists+devicetree@lfdr.de>);
        Wed, 20 Jan 2021 14:14:50 -0500
Received: from lindbergh.monkeyblade.net ([23.128.96.19]:53832 "EHLO
        lindbergh.monkeyblade.net" rhost-flags-OK-OK-OK-OK) by vger.kernel.org
        with ESMTP id S1733110AbhATSzE (ORCPT
        <rfc822;devicetree@vger.kernel.org>); Wed, 20 Jan 2021 13:55:04 -0500
Received: from m-r2.th.seeweb.it (m-r2.th.seeweb.it [IPv6:2001:4b7a:2000:18::171])
        by lindbergh.monkeyblade.net (Postfix) with ESMTPS id 78F57C0613D3
        for <devicetree@vger.kernel.org>; Wed, 20 Jan 2021 10:54:23 -0800 (PST)
Received: from IcarusMOD.eternityproject.eu (unknown [2.237.20.237])
        (using TLSv1.3 with cipher TLS_AES_128_GCM_SHA256 (128/128 bits)
         key-exchange X25519 server-signature RSA-PSS (2048 bits) server-digest SHA256)
        (No client certificate requested)
        by m-r2.th.seeweb.it (Postfix) with ESMTPSA id EF8813F1FD;
        Wed, 20 Jan 2021 19:54:18 +0100 (CET)
Subject: Re: [PATCH v4 5/7] cpufreq: qcom-hw: Implement CPRh aware OSM
 programming
To:     Taniya Das <tdas@codeaurora.org>, viresh.kumar@linaro.org
...
Message-ID: <a1cb40b6-483c-8ee7-d4d8-31d74f7b9a95@somainline.org>
Date:   Wed, 20 Jan 2021 19:54:18 +0100
User-Agent: Mozilla/5.0 (X11; Linux x86_64; rv:78.0) Gecko/20100101
 Thunderbird/78.5.0
MIME-Version: 1.0
In-Reply-To: <c35bfd76-0d7e-d7bc-79ab-041b1074c1af@codeaurora.org>
...
The CPUFREQ HW designs are effectively Operating State Manager, with
CPR-Hardened, SAWv4.x (etc) and they basically are "the same" between
one another.

For how I see it, since this code is made to drive the OSM states, this
driver is simply incomplete in the current state, as it misses the
ability to program the hardware in case it wasn't programmed by TZ,
which is effectively the case with more than just one SoC.


In any case, I think that the "layer of separation" between a OSM
that was pre-programmed by the TZ/bootloader (anything pre-HLOS) and
one that needs to be programmed by, in this case, Linux, is pretty
much evident: for how I have engineered the code, the already existant
part of the driver is *untouched*.

One bool variable is checked for the OSM programming, if that's true
then it will do the entire programming - otherwise, the flow will be
like nothing was ever added.


Separating this out as another driver, managing the same hardware,
in the same iospace(s), would only be confusing for developers, making
them think that we're dealing with "a different piece", or something
that is assisting "that chip" (oversimplifying, please don't be picky),
...
>> +#define SEQ_MEMACC_REG(base, n)        SEQUENCER_REG(base, 
>> SEQ_MEM_ACC_0 + n)
>> +
>> +/* ACD */
...
>> + * @softstart_reg:     Soft Start Control Register (SSCR) register 
>> offset
...
>> +
>> +/**
>> + * struct qcom_cpufreq_hw_params - Operating State Manager (OSM) 
>> Parameters
>> + *
>> + * @volt_lut_val: Value composed of: virtual corner (vc) and voltage 
>> in mV.
>> + * @freq_lut_val: Value composed of: core count, clock source and output
>> + *                frequency in MHz.
>> + * @override_val: PLL parameters that the OSM uses to override the 
>> previous
>> + *                setting coming from the bootloader, or when 
>> uninitialized.
>> + * @spare_val:    Spare register, used by both this driver and the 
>> OSM HW
>> + *                to identify MEM-ACC levels in relation to virtual 
>> corners.
...
>> +};
>> +/**
>> + * struct qcom_cpufreq_soc_data - SoC specific register offsets of 
>> the OSM
>> + *
...
>>   {
>> @@ -59,6 +290,20 @@ static int qcom_cpufreq_set_bw(struct 
...
>>                      unsigned long freq_khz,
>>                      unsigned long volt)
>> @@ -79,6 +324,17 @@ static int qcom_cpufreq_update_opp(struct device 
>> *cpu_dev,
...
>>                       unsigned int index)
>>   {
>> @@ -94,6 +350,12 @@ static int qcom_cpufreq_hw_target_index(struct 
>> cpufreq_policy *policy,
>>       return 0;
>>   }
>> +/**
...
>> @@ -127,6 +389,644 @@ static unsigned int 
...
>> + * @apm_vc:    APM Virtual Corner crossover number, returned to the 
>> caller
>> + * @acc_vc:    MEMACC Virtual Corner crossover number, returned to 
>> the caller
...
>> +}
>> +
>> +static int qcom_cpufreq_hw_acd_write_autoxfer(struct 
...
>> +}
>> +
>> +static int qcom_cpufreq_hw_acd_write_xfer(struct qcom_cpufreq_data 
>> *data,
...
>> +    writel_relaxed(aregs->softstart_val, acd_base + 
>> aregs->softstart_reg);
>> +    writel_relaxed(aregs->ext_intf0_val, acd_base + 
>> aregs->ext_intf_reg);
...
>> +    struct qcom_cpufreq_data *ddata = policy->driver_data;
>> +    const struct qcom_cpufreq_soc_data *sdata = ddata->soc_data;
>> +    const struct qcom_cpufreq_soc_setup_data *sregs = 
>> &sdata->setup_regs;
>> +    struct qcom_cpufreq_hw_params *hw_tbl;
...
>> +    }
>> +
>> +    /* Sanity check: we *must* have two mem-acc crossovers (four 
>> values) */
...
>> +
>> +    /*
>> +     * APM crossover virtual corner refers to CPRh: there, the APM 
>> corner
>> +     * is always appended to the table (so, at the end of it, right 
>> after
>> +     * the cluster dvfs entries).
>> +     */
>> +    writel(num_entries, ddata->base + sregs->reg_seq1);
>> +    ret = qcom_scm_io_writel(SEQUENCER_REG(seq_addr, 
>> SEQ_APM_CROSSOVER_VC),
>> +                 num_entries);
>> +    if (ret)
>> +        return ret;
>> +
...
>>                       struct cpufreq_policy *policy)
>>   {
>> @@ -166,7 +1066,9 @@ static int qcom_cpufreq_hw_read_lut(struct device 
>> *cpu_dev,
>>       for (i = 0; i < LUT_MAX_ENTRIES; i++) {
...
>> @@ -175,17 +1077,21 @@ static int qcom_cpufreq_hw_read_lut(struct 
>> device *cpu_dev,
...
>> +                    i, freq, core_count);
>>               } else {
>> -                dev_warn(cpu_dev, "failed to update OPP for 
>> freq=%d\n", freq);
>> +                dev_warn(cpu_dev,
...
>>               }
>> @@ -205,18 +1111,19 @@ static int qcom_cpufreq_hw_read_lut(struct 
>> device *cpu_dev,
...
>>           prev_freq = freq;
>>       }
>> @@ -227,10 +1134,18 @@ static int qcom_cpufreq_hw_read_lut(struct 
>> device *cpu_dev,
>>       return 0;
>>   }
...
>> + * @m:     Returned CPU mask
>> + *
>> + * Returns: Count of CPUs inserted in the cpumask or negative number 
>> for error.
>> + */
...
>> @@ -245,34 +1160,237 @@ static void qcom_get_related_cpus(int index, 
>> struct cpumask *m)
>>           if (ret < 0)
>>               continue;
...
>> + * the OS itself, which includes setting LUT and all the various 
>> tunables
...
>> +
>> +    /* Set OSM to XO clock ratio and use XO edge for the cycle 
>> counter */
...
>> +
>> +    /* Set the frequency index 0 and override for cluster power 
>> collapse */
>> +    writel_relaxed(BIT(0), drv_data->base + 
>> setup_regs->reg_cc_zero_behav);
>> +
>> +    /* Treat cores in retention as active */
>> +    writel_relaxed(0, drv_data->base + 
>> setup_regs->reg_spm_core_ret_map);
>> +
>> +    /* Enable CoreCount based DCVS */
>> +    writel_relaxed(0, drv_data->base + setup_regs->reg_spm_cc_dcvs_dis);
>> +
>> +    /* CoreCount DCVS-LLM Policy: Wait time for frequency 
>> inc/decrement */
>> +    val = FIELD_PREP(HYSTERESIS_UP_MASK, HYSTERESIS_LLM_NS);
>> +    val |= FIELD_PREP(HYSTERESIS_DN_MASK, HYSTERESIS_LLM_NS);
>> +    writel_relaxed(val, drv_data->base + 
>> setup_regs->reg_llm_freq_vote_hyst);
>> +
>> +    /* CoreCount DCVS-LLM Policy: Wait time for voltage inc/decrement */
>> +    val = FIELD_PREP(HYSTERESIS_UP_MASK, HYSTERESIS_LLM_NS);
>> +    val |= FIELD_PREP(HYSTERESIS_DN_MASK, HYSTERESIS_LLM_NS);
>> +    writel_relaxed(val, drv_data->base + 
>> setup_regs->reg_llm_volt_vote_hyst);
>> +
>> +    /* Enable LLM frequency+voltage voting */
>> +    writel_relaxed(0, drv_data->base + 
>> setup_regs->reg_llm_intf_dcvs_dis);
>> +
...
>> +    writel_relaxed(val, drv_data->base + 
>> setup_regs->reg_droop_unstall_ctrl);
>> +
...
>> +    writel_relaxed(val,
>> +               drv_data->base + 
>> setup_regs->reg_droop_wait_release_ctrl);
>> +
>> +    /* PLL signal timing control for Droop */
>> +    writel_relaxed(1, drv_data->base + 
>> setup_regs->reg_droop_sync_delay);
>> +
>> +    /* Setup DCVS timers */
...
>>   {
>>       struct platform_device *pdev = cpufreq_get_driver_data();
>> @@ -282,7 +1400,8 @@ static int qcom_cpufreq_hw_cpu_init(struct 
>> cpufreq_policy *policy)
...
>>       if (!cpu_dev) {
>> @@ -303,9 +1422,14 @@ static int qcom_cpufreq_hw_cpu_init(struct 
>> cpufreq_policy *policy)
>>       index = args.args[0];
...
>>       if (!data) {
>> @@ -315,22 +1439,31 @@ static int qcom_cpufreq_hw_cpu_init(struct 
>> cpufreq_policy *policy)
...
>>       if (ret) {
>> @@ -355,6 +1488,7 @@ static int qcom_cpufreq_hw_cpu_init(struct 
>> cpufreq_policy *policy)
>>       return 0;
>>   error:
...
>>       return ret;
>>   }
>> @@ -395,9 +1529,51 @@ static struct cpufreq_driver 
...
>>       if (IS_ERR(clk))
>> @@ -410,16 +1586,16 @@ static int qcom_cpufreq_hw_driver_probe(struct 
