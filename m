...
Received: from vger.kernel.org (vger.kernel.org [209.132.180.67])
	by mail.lfdr.de (Postfix) with ESMTP id BED3CBDA34
	for <lists+devicetree@lfdr.de>; Wed, 25 Sep 2019 10:49:59 +0200 (CEST)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S1726030AbfIYIt5 (ORCPT <rfc822;lists+devicetree@lfdr.de>);
        Wed, 25 Sep 2019 04:49:57 -0400
Received: from relay7-d.mail.gandi.net ([217.70.183.200]:39815 "EHLO
        relay7-d.mail.gandi.net" rhost-flags-OK-OK-OK-OK) by vger.kernel.org
        with ESMTP id S1729361AbfIYIt4 (ORCPT
        <rfc822;devicetree@vger.kernel.org>); Wed, 25 Sep 2019 04:49:56 -0400
X-Originating-IP: 86.250.200.211
Received: from localhost.localdomain (lfbn-1-17395-211.w86-250.abo.wanadoo.fr [86.250.200.211])
        (Authenticated sender: paul.kocialkowski@bootlin.com)
        by relay7-d.mail.gandi.net (Postfix) with ESMTPSA id 4234920015;
        Wed, 25 Sep 2019 08:49:47 +0000 (UTC)
...
Subject: [PATCH v2 2/2] drm: Add support for the LogiCVC display controller
Date:   Wed, 25 Sep 2019 10:49:32 +0200
Message-Id: <20190925084932.147971-3-paul.kocialkowski@bootlin.com>
...
index b14729628993..01bf13d64396 100644
--- a/drivers/gpu/drm/Makefile
+++ b/drivers/gpu/drm/Makefile
@@ -103,6 +103,7 @@ obj-$(CONFIG_DRM_STM) += stm/
...
new file mode 100644
index 000000000000..f13ab62d18ef
--- /dev/null
...
+	if (ret && ret != -ENODEV) {
+		DRM_ERROR("Failed to init memory region\n");
+		goto error_early;
+	}
+
...
+		if (ret) {
+			DRM_ERROR("Failed to get resource from address\n");
+			goto error_reserved_mem;
+		}
+
+		base = devm_ioremap_resource(&pdev->dev, &res);
+		if (IS_ERR(base)) {
+			DRM_ERROR("Failed to map I/O base\n");
+			ret = PTR_ERR(base);
+			goto error_reserved_mem;
+		}
+
...
+		if (IS_ERR(regmap)) {
+			DRM_ERROR("Failed to create regmap for I/O\n");
+			ret = PTR_ERR(regmap);
+			goto error_reserved_mem;
+		}
+	}
...
+	if (ret) {
+		DRM_ERROR("Failed to prepare clocks\n");
+		goto error_logicvc;
+	}
+
...
+	if (ret && ret != -ENODEV) {
+		DRM_ERROR("Failed to parse config\n");
+		goto error_clocks;
+	}
+
...
+	if (ret) {
+		DRM_ERROR("Failed to initialize layers\n");
...
+	if (ret) {
+		if (ret != -EPROBE_DEFER)
+			DRM_ERROR("Failed to initialize interface\n");
+
+		goto error_mode_config;
+	}
+
...
+	if (ret) {
+		DRM_ERROR("Failed to initialize KMS\n");
+		goto error_mode_config;
+	}
+
...
+		DRM_ERROR("Negative on-CRTC positions are not supported.\n");
+		return -EINVAL;
+	}
+
...
+		if (ret) {
+			DRM_ERROR("No viable setup for buffer found.\n");
+			return ret;
+		}
+	}
+
...
+		DRM_ERROR("No reserved memory base was registered!\n");
+		return -ENOMEM;
+	}
+
+	fb_addr = drm_fb_cma_get_gem_addr(fb, state, 0);
+	if (fb_addr < logicvc->reserved_mem_base) {
+		DRM_ERROR("Framebuffer memory below reserved memory base!\n");
+		return -EINVAL;
+	}
+
...
+		DRM_ERROR("Framebuffer offset below layer base offset!\n");
+		return -EINVAL;
+	}
+
...
+
+	if (gap) {
+		DRM_ERROR("Unable to find layer %d buffer setup for 0x%x byte gap\n",
...
+	if (ret) {
+		DRM_ERROR("Failed to parse config for layer #%d\n", index);
+		goto error;
+	}
+
+	formats = logicvc_layer_formats_lookup(layer);
+	if (!formats) {
+		DRM_ERROR("Failed to lookup formats for layer #%d\n", index);
+		goto error;
+	}
+
...
+	if (ret) {
+		DRM_ERROR("Failed to initialize layer plane\n");
+		return ret;
+	}
+
...
new file mode 100644
index 000000000000..6095da975d7c
--- /dev/null
