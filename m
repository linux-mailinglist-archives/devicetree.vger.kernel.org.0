...
Received: from vger.kernel.org (vger.kernel.org [209.132.180.67])
	by mail.lfdr.de (Postfix) with ESMTP id 2F2B762EC9
	for <lists+devicetree@lfdr.de>; Tue,  9 Jul 2019 05:27:05 +0200 (CEST)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S1727152AbfGIDWd (ORCPT <rfc822;lists+devicetree@lfdr.de>);
        Mon, 8 Jul 2019 23:22:33 -0400
Received: from mailgw02.mediatek.com ([210.61.82.184]:48479 "EHLO
        mailgw02.mediatek.com" rhost-flags-OK-FAIL-OK-FAIL) by vger.kernel.org
        with ESMTP id S1726294AbfGIDWd (ORCPT
        <rfc822;devicetree@vger.kernel.org>); Mon, 8 Jul 2019 23:22:33 -0400
X-UUID: fcb0e15ce283485a856e2617dc6a9bf7-20190709
X-UUID: fcb0e15ce283485a856e2617dc6a9bf7-20190709
Received: from mtkcas07.mediatek.inc [(172.21.101.84)] by mailgw02.mediatek.com
        (envelope-from <xia.jiang@mediatek.com>)
        (mhqrelay.mediatek.com ESMTP with TLS)
        with ESMTP id 406967743; Tue, 09 Jul 2019 11:22:19 +0800
Received: from mtkcas09.mediatek.inc (172.21.101.178) by
 mtkmbs07n2.mediatek.inc (172.21.101.141) with Microsoft SMTP Server (TLS) id
 15.0.1395.4; Tue, 9 Jul 2019 11:22:18 +0800
Received: from localhost.localdomain (10.17.3.153) by mtkcas09.mediatek.inc
 (172.21.101.73) with Microsoft SMTP Server id 15.0.1395.4 via Frontend
 Transport; Tue, 9 Jul 2019 11:22:17 +0800
...
Date:   Tue, 9 Jul 2019 11:21:01 +0800
Message-ID: <20190709032103.10291-4-xia.jiang@mediatek.com>
...
@@ -659,10 +863,92 @@ static void mtk_jpeg_set_queue_data(struct mtk_jpeg_ctx *ctx,
...
+	struct mtk_jpeg_enc_param *enc_param;
 	...
 	bool header_valid;
@@ -674,29 +960,45 @@ static void mtk_jpeg_buf_queue(struct vb2_buffer *vb)
 		goto end;
...
@@ -739,16 +1041,16 @@ static void mtk_jpeg_stop_streaming(struct vb2_queue *q)
...
@@ -803,6 +1105,28 @@ static int mtk_jpeg_set_dec_dst(struct mtk_jpeg_ctx *ctx,
 	return 0;
 }
...
@@ -813,6 +1137,8 @@ static void mtk_jpeg_device_run(void *priv)
 	struct mtk_jpeg_src_buf *jpeg_src_buf;
...
@@ -823,30 +1149,45 @@ static void mtk_jpeg_device_run(void *priv)
...
@@ -906,30 +1247,30 @@ static void mtk_jpeg_clk_on(struct mtk_jpeg_dev *jpeg)
...
@@ -940,21 +1281,42 @@ static irqreturn_t mtk_jpeg_dec_irq(int irq, void *priv)
...
+irq_end:
 	...
 	v4l2_m2m_job_finish(jpeg->m2m_dev, ctx->fh.m2m_ctx);
@@ -964,32 +1326,72 @@ static irqreturn_t mtk_jpeg_dec_irq(int irq, void *priv)
...
@@ -1021,6 +1423,13 @@ static int mtk_jpeg_open(struct file *file)
 		goto error;
 	}
...
+
 	...
 	return 0;
@@ -1041,6 +1450,7 @@ static int mtk_jpeg_release(struct file *file)
...
+	v4l2_ctrl_handler_free(&ctx->ctrl_hdl);
 	...
 	v4l2_fh_exit(&ctx->fh);
 	kfree(ctx);
@@ -1065,6 +1475,7 @@ static int mtk_jpeg_clk_init(struct mtk_jpeg_dev *jpeg)
...
+
 	...
 	if (WARN_ON(!pdev)) {
 		of_node_put(node);
@@ -1074,19 +1485,24 @@ static int mtk_jpeg_clk_init(struct mtk_jpeg_dev *jpeg)
...
@@ -1096,28 +1512,26 @@ static int mtk_jpeg_probe(struct platform_device *pdev)
...
@@ -1141,33 +1555,35 @@ static int mtk_jpeg_probe(struct platform_device *pdev)
 		goto err_m2m_init;
 	}
...
@@ -1175,10 +1591,10 @@ static int mtk_jpeg_probe(struct platform_device *pdev)
...
+err_vfd_jpeg_alloc:
 ...
 err_m2m_init:
@@ -1198,8 +1614,8 @@ static int mtk_jpeg_remove(struct platform_device *pdev)
...
@@ -1210,7 +1626,11 @@ static __maybe_unused int mtk_jpeg_pm_suspend(struct device *dev)
 {
...
+
 	...
 	return 0;
@@ -1221,7 +1641,10 @@ static __maybe_unused int mtk_jpeg_pm_resume(struct device *dev)
 	struct mtk_jpeg_dev *jpeg = dev_get_drvdata(dev);
...
@@ -1257,11 +1680,15 @@ static const struct dev_pm_ops mtk_jpeg_pm_ops = {
