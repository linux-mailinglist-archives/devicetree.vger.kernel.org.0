...
Received: from out1.vger.email (out1.vger.email [IPv6:2620:137:e000::1:20])
	by mail.lfdr.de (Postfix) with ESMTP id ECD2764B64D
	for <lists+devicetree@lfdr.de>; Tue, 13 Dec 2022 14:33:44 +0100 (CET)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S235621AbiLMNdl (ORCPT <rfc822;lists+devicetree@lfdr.de>);
        Tue, 13 Dec 2022 08:33:41 -0500
Received: from lindbergh.monkeyblade.net ([23.128.96.19]:47106 "EHLO
        lindbergh.monkeyblade.net" rhost-flags-OK-OK-OK-OK) by vger.kernel.org
        with ESMTP id S235626AbiLMNdd (ORCPT
        <rfc822;devicetree@vger.kernel.org>); Tue, 13 Dec 2022 08:33:33 -0500
Received: from relay2-d.mail.gandi.net (relay2-d.mail.gandi.net [IPv6:2001:4b98:dc4:8::222])
        by lindbergh.monkeyblade.net (Postfix) with ESMTPS id 9E79419C27;
        Tue, 13 Dec 2022 05:33:22 -0800 (PST)
Received: (Authenticated sender: herve.codina@bootlin.com)
        by mail.gandi.net (Postfix) with ESMTPA id 9BA2B40014;
        Tue, 13 Dec 2022 13:33:19 +0000 (UTC)
DKIM-Signature: v=1; a=rsa-sha256; c=relaxed/relaxed; d=bootlin.com; s=gm1;
        ...
        bh=hiY2K2hVYjfl7cDcrnj44pD6wKEnCehRAmsKViuMKWs=;
        b=
...
Subject: [PATCH v4 3/5] usb: gadget: udc: add Renesas RZ/N1 USBF controller support
Date:   Tue, 13 Dec 2022 14:33:00 +0100
Message-Id: <20221213133302.218955-4-herve.codina@bootlin.com>
...
index 000000000000..8d3d36d0510d
--- /dev/null
...
+		TRACEEP(epn, "ep%u buf unaligned -> fallback pio\n", epn->id);
...
+		if (left == 0) {
+			TRACEEP(epn, "ep%u send null\n", epn->id);
+			usbf_epn_send_null(epn);
+			req->xfer_step = USBF_XFER_WAIT_END;
+			break;
+		}
+		if (left < 4) {
+			TRACEEP(epn, "ep%u send residue %u\n", epn->id, left);
...
+			TRACEEP(epn, "ep%u dma not done\n", epn->id);
+			break;
+		}
+		TRACEEP(epn, "ep%u dma done\n", epn->id);
+
...
+			TRACEEP(epn, "ep%u end not done\n", epn->id);
+			break;
+		}
+		TRACEEP(epn, "ep%u send done %u/%u\n", epn->id,
+			req->req.actual, req->req.length);
...
+		left = req->req.length - req->req.actual;
+
+		TRACEEP(epn, "ep%u recv %u, left %u, mpkt %u\n", epn->id,
...
+			}
+		}
+		TRACEEP(epn, "ep%u recv %u/%u\n", epn->id,
+			req->req.actual, req->req.length);
+
+		if (req_status) {
+			TRACEEP(epn, "ep%u req.status=%d\n", epn->id,
+				req_status);
+			req->req.status = req_status;
+			return 0;
+		}
...
+
+		if (!(epn->status & USBF_EPN_OUT_INT)) {
+			TRACEEP(epn, "ep%u OUT_INT not set -> spurious\n",
+				epn->id);
+			break;
+		}
+
...
+			usbf_ep_reg_readl(epn, USBF_REG_EPN_LEN_DCNT));
+		if (!recv) {
+			TRACEEP(epn, "ep%u recv = 0 -> spurious\n",
+				epn->id);
+			break;
+		}
+
...
+				req->req.actual += recv;
+
+				TRACEEP(epn, "ep%u recv done %u/%u\n", epn->id,
+					req->req.actual, req->req.length);
+
+				req->xfer_step = USBF_XFER_START;
+				return 0;
...
+
+			TRACEEP(epn, "ep%u dma short xfer %zu\n", epn->id,
+				req->dma_size);
+
...
+
+		TRACEEP(epn, "ep%u dma xfer %zu (%u)\n", epn->id,
+			req->dma_size, npkt);
+
+		req->xfer_step = USBF_XFER_WAIT_DMA;
+		break;
...
+			TRACEEP(epn, "ep%u dma short not done\n", epn->id);
+			break;
+		}
+		TRACEEP(epn, "ep%u dma short done\n", epn->id);
+
...
+		count = recv & 0x3;
+		if (count) {
+			TRACEEP(epn, "ep%u recv residue %u\n", epn->id,
+				count);
+			usbf_epn_recv_residue(epn,
...
+
+			TRACEEP(epn, "ep%u dma xfer done %u\n", epn->id, count);
+
+			req->req.actual += count;
+
+			if (epn->status & USBF_EPN_OUT_NULL_INT) {
+				/* DMA was stopped by a null packet reception */
+				TRACEEP(epn, "ep%u dma stopped by null pckt\n",
+					epn->id);
...
+		TRACEEP(epn, "ep%u bridge transfers done\n", epn->id);
+
+		/* Restore interrupt mask */
+		usbf_ep_reg_clrset(epn, USBF_REG_EPN_INT_ENA,
+			USBF_EPN_OUT_END_EN,
...
+			return 0;
+		}
+		TRACEEP(epn, "ep%u recv done %u/%u, wait more data\n", epn->id,
+			req->req.actual, req->req.length);
+		break;
+	}
+
...
+	}
+
+	TRACEEP(ep, "ep%u %s req done length %u/%u, status=%d\n", ep->id,
+		ep->is_in ? "in" : "out",
...
+	struct usbf_req *req;
+
+	TRACEEP(ep, "ep%u %s nuke status %d\n", ep->id,
+		ep->is_in ? "in" : "out",
+		status);
+
...
+		TRACEEP(ep, "ep%u bad descriptor\n", ep->id);
+		return -EINVAL;
+	}
+
+	TRACEEP(ep, "ep%u %s mpkts %d\n", ep->id,
...
+
+	if (!_req || !_req->buf) {
+		TRACEEP(ep, "ep%u invalid request\n", ep->id);
+		return -EINVAL;
+	}
+
+	if (!udc || !udc->driver) {
+		TRACEEP(ep, "ep%u invalid device\n", ep->id);
+		return -EINVAL;
+	}
+
+	TRACEEP(ep, "ep%u %s req queue length %u, zero %u, short_not_ok %u\n",
...
+	spin_lock_irqsave(&ep->udc->lock, flags);
+
+	TRACEEP(ep, "ep%u %s req dequeue length %u/%u\n",
+		ep->id, ep->is_in ? "in" : "out",
+		req->req.actual, req->req.length);
+
...
+
+	TRACEEP(epn, "ep%u %s status=0x%08x, enable=%08x\n, ctrl=0x%08x\n",
...
+		TRACEEP(epn, "ep%u %s process queue (in interrupts)\n",
+			epn->id, epn->is_in ? "in" : "out");
...
+		TRACEEP(epn, "ep%u %s process queue (out interrupts)\n",
+			epn->id, epn->is_in ? "in" : "out");
+		usbf_epn_process_queue(epn);
+	}
+
+	TRACEEP(epn, "ep%u %s done status=0x%08x. next=0x%08x\n",
+		epn->id, epn->is_in ? "in" : "out",
