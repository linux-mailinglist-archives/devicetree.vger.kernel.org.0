...
Received: from vger.kernel.org (vger.kernel.org [23.128.96.18])
	by mail.lfdr.de (Postfix) with ESMTP id 5D7D63959AF
	for <lists+devicetree@lfdr.de>; Mon, 31 May 2021 13:24:55 +0200 (CEST)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S231478AbhEaL0b (ORCPT <rfc822;lists+devicetree@lfdr.de>);
        Mon, 31 May 2021 07:26:31 -0400
Received: from comms.puri.sm ([159.203.221.185]:59620 "EHLO comms.puri.sm"
        rhost-flags-OK-OK-OK-OK) by vger.kernel.org with ESMTP
        id S231450AbhEaL0U (ORCPT <rfc822;devicetree@vger.kernel.org>);
        Mon, 31 May 2021 07:26:20 -0400
Received: from localhost (localhost [127.0.0.1])
        by comms.puri.sm (Postfix) with ESMTP id 004F6E212E;
        Mon, 31 May 2021 04:24:10 -0700 (PDT)
Received: from comms.puri.sm ([127.0.0.1])
        by localhost (comms.puri.sm [127.0.0.1]) (amavisd-new, port 10024)
        with ESMTP id RyIjTTOJlWYi; Mon, 31 May 2021 04:24:07 -0700 (PDT)
...
Subject: [PATCH v2 2/3] media: imx: add a driver for i.MX8MQ mipi csi rx phy and controller
Date:   Mon, 31 May 2021 13:23:25 +0200
Message-Id: <20210531112326.90094-3-martin.kepplinger@puri.sm>
...
index 000000000000..d97405d7b2c1
--- /dev/null
...
+
+	/* phy-reset */
+	ret = of_property_read_u32(np, "phy-reset", &ph);
+	if (ret) {
+		dev_err(dev, "no phy-reset property found: %d\n", ret);
+		return ret;
+	}
+
...
+	if (!node) {
+		ret = -ENODEV;
+		dev_err(dev, "Error finding phy-reset node by phandle\n");
+		goto out;
+	}
...
+
+	/* phy-gpr */
+	ret = of_property_read_u32(np, "phy-gpr", &ph);
+	if (ret) {
+		dev_err(dev, "no phy-gpr property found: %d\n", ret);
+		return ret;
+	}
+
+	node = of_find_node_by_phandle(ph);
+	if (!node) {
+		ret = -ENODEV;
+		dev_err(dev, "Error finding phy-gpr node by phandle\n");
+		goto out;
+	}
