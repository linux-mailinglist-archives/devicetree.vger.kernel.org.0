...
Received: from vger.kernel.org (vger.kernel.org [23.128.96.18])
	by mail.lfdr.de (Postfix) with ESMTP id D05EA2414FF
	for <lists+devicetree@lfdr.de>; Tue, 11 Aug 2020 04:36:41 +0200 (CEST)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S1726958AbgHKCgk (ORCPT <rfc822;lists+devicetree@lfdr.de>);
        Mon, 10 Aug 2020 22:36:40 -0400
Received: from lindbergh.monkeyblade.net ([23.128.96.19]:54072 "EHLO
        lindbergh.monkeyblade.net" rhost-flags-OK-OK-OK-OK) by vger.kernel.org
        with ESMTP id S1726446AbgHKCgk (ORCPT
        <rfc822;devicetree@vger.kernel.org>); Mon, 10 Aug 2020 22:36:40 -0400
Received: from perceval.ideasonboard.com (perceval.ideasonboard.com [IPv6:2001:4b98:dc2:55:216:3eff:fef7:d647])
        by lindbergh.monkeyblade.net (Postfix) with ESMTPS id 67CE7C06174A;
        Mon, 10 Aug 2020 19:36:39 -0700 (PDT)
Received: from pendragon.ideasonboard.com (62-78-145-57.bb.dnainternet.fi [62.78.145.57])
        by perceval.ideasonboard.com (Postfix) with ESMTPSA id 71F3458;
        Tue, 11 Aug 2020 04:36:35 +0200 (CEST)
DKIM-Signature: v=1; a=rsa-sha256; c=relaxed/simple; d=ideasonboard.com;
        s=mail; t=1597113395;
        bh=l8QIw4xoFeMNER7tcp4006jGwv7METaDV4KaqtUjweQ=;
        h=Date:From:To:Cc:Subject:References:In-Reply-To:From;
        b=U/XykBTlcwXac9kxExl72n1/o94qVQS/YPqFAQ8XosQ97mZmy6ZsuPRDZHWaG8kaz
         8UxOdH5kOr/qWXbNxp8GNY+e/XfhVE5ieJkRrCEt6RAT0xb9Shxm3Zm9Bv5EvFoL3b
         0I+Ya+MDlOdN+ian6X57bL0x/GBsUhk8Gt5se8IM=
Date:   Tue, 11 Aug 2020 05:36:22 +0300
...
Subject: Re: [PATCH v8 2/3] drm: bridge: Add support for Cadence MHDP DPI/DP
 bridge
Message-ID: <20200811023622.GC13513@pendragon.ideasonboard.com>
...
How about adding a TODO comment block at the top of the file to record
the TODO items you have mentioned in the cover letter, as well as this
one ?
...
I wonder if this should be called cdns_mhdp_mailbox_receive_header(),
and the function below cdns_mhdp_mailbox_receive_data(), to make their
purpose clearer (possibly with s/receiver/recv/ as commonly done in
communication-related code if you prefer).
...
I still wonder why this would happen, as explained in a reply to v6.
There can of course be bugs in the firmware, but if that happens and we
don't get the message we expect, I think all bets are off. We can
discuss the issue here or in the v6 thread, up to you.
...
I'd name this label "done" or "out", as it's a bit weird to have a label
named as an error in the normal code path. Same in other functions
below. Not a big issue, it's up to you.
...
There's a mix of DRM_DEV_* and dev_*. Should we use one or the other ? I
personally tend to use dev_*, but it's up to you.
...
Given that the only possible error code returned from read functions is
-ETIMEDOUT, I wonder if it's worth printing the value (here and in other
error messages). No big deal though.
...
Can there be other events than HPD events ? If not this could be named
cdns_mhdp_read_hpd_event().
...
You can use unsigned int for nlanes and udelay. Shorter types for
variables whose size doesn't matter much usually hinder performance
instead of improving it. There are a few other locations where this
...
or you could drop this and use DP_LINK_STATUS_SIZE instead of nregs
below.
...
This is only called from cdns_mhdp_fw_cb() which also prints a debugging
message, I would drop this one.
...
If I understand this correctly, the purpose it to protect against a race
condition with remove(). The != MHDP_HW_INACTIVE and != MHDP_HW_STOPPED
case should really not happen, so I'd drop that check. For the !=
MHDP_HW_INACTIVE check, it can only happen when the wait for
MHDP_HW_READY in remove() times out, which would likely crash in any
case as there's a very little chance that the firmware callback would be
called after remove() sets MHDP_HW_STOPPED and before it completes. I
think this gives a false sense of safety, I would rather drop it, and
add an item to the TODO list to implement request_firmware_cancel().

> +	mhdp->hw_state = MHDP_HW_LOADING;

Nothing checks for MHDP_HW_LOADING, so I'd drop that state. You wouldn't
have to set hw_state back to MHDP_HW_INACTIVE below, so you could get
rid of the goto error and just return ret.
...
cdns_mhdp_set_firmware_active() prints an error already, I'd drop this
one and the one in remove().
...
Such a lovely issue... I still think that loading the firmware
asynchronously is a hack around a deficiency in the graphics stack.
Fixing it will require a very substantial effort, so I'm fine with the
current implementation. Could you add this to the TODO list comment
block ?
...
I've seen similar code in a different driver, I think it would make
sense to move this to a helper function in the DRM core. No need to
address it yet, it can be added to the TODO list.
...
You can initialize variables when declaring them.
	...
	struct cdns_mhdp_bridge_state *state =
		to_cdns_mhdp_bridge_state(bridge_state);
...
As Tomi pointed out, this has to be freed in
