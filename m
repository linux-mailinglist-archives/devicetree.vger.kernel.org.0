...
Received: from vger.kernel.org (vger.kernel.org [209.132.180.67])
	by mail.lfdr.de (Postfix) with ESMTP id CAD3B6B0D1
	for <lists+devicetree@lfdr.de>; Tue, 16 Jul 2019 23:14:07 +0200 (CEST)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S2388838AbfGPVMm (ORCPT <rfc822;lists+devicetree@lfdr.de>);
        Tue, 16 Jul 2019 17:12:42 -0400
Received: from hqemgate14.nvidia.com ([216.228.121.143]:8212 "EHLO
        hqemgate14.nvidia.com" rhost-flags-OK-OK-OK-OK) by vger.kernel.org
        with ESMTP id S1728294AbfGPVMm (ORCPT
        <rfc822;devicetree@vger.kernel.org>); Tue, 16 Jul 2019 17:12:42 -0400
Received: from hqpgpgate101.nvidia.com (Not Verified[216.228.121.13]) by hqemgate14.nvidia.com (using TLS: TLSv1.2, DES-CBC3-SHA)
        id <B5d2e3dc70002>; Tue, 16 Jul 2019 14:12:39 -0700
Received: from hqmail.nvidia.com ([172.20.161.6])
  by hqpgpgate101.nvidia.com (PGP Universal service);
  Tue, 16 Jul 2019 14:12:39 -0700
X-PGP-Universal: processed;
        by hqpgpgate101.nvidia.com on Tue, 16 Jul 2019 14:12:39 -0700
...
Message-ID: <10c4b9a2-a857-d124-c22d-7fd71a473079@nvidia.com>
...
DKIM-Signature: v=1; a=rsa-sha256; c=relaxed/relaxed; d=nvidia.com; s=n1;
        t=1563311559; bh=jSr6oXibI3bnE25r/sq4SDRAKfRgY+SLeqGAJmMGsEw=;
        h=X-PGP-Universal:Subject:To:CC:References:From:Message-ID:Date:
         User-Agent:MIME-Version:In-Reply-To:X-Originating-IP:
         X-ClientProxiedBy:Content-Type:Content-Transfer-Encoding:
         Content-Language;
        b=
...
>>>>>>>>>>>> icky.
>>>>>>>>>>>>
>>>>>>>>>>> Should I add check for successful dfll clk register explicitly =
in
...
>>>>>>>>
>>>>>>>> [1]
>>>>>>>> https://elixir.bootlin.com/linux/v5.2.1/source/drivers/gpu/drm/teg=
ra/dc.c#L2383
>>>>>>>>
>>>>>>>>
>>>>>>>>
...
t/tree/drivers/devfreq/tegra20-devfreq.c#n100
>>>>>>>>
>>>>>>>>
>>>>>>>>
...
>>>>> probed, hence everything should be fine as-is and there is no real ne=
ed
...
>>>>>>>>>> part.
>>>>>>>>>>
>>>>>>>>>> As you know it, the DFLL clock is one of the CPU clock sources a=
nd
>>>>>>>>>> integrated with DVFS control logic with the regulator. We will n=
ot
>>>>>>>>>> switch
...
>>>>>>>> PLLP's
>>>>>>>> rate before switching to PLLP in order to have a proper CPU voltag=
e.
>>>>>>> PLLP freq is safe to work for any CPU voltage. So no need to enforc=
e
>>>>>>> DFLL freq to PLLP rate before changing CCLK_G source to PLLP during
>>>>>>> suspend
>>>>>>>
>>>>>> Sorry, please ignore my above comment. During suspend, need to chang=
e
>>>>>> CCLK_G source to PLLP when dfll is in closed loop mode first and the=
n
...
>>>>>>>>>> mode. That's
>>>>>>>>>> all. The sc7-entryfirmware will handle the rest of the sequence =
to
>>>>>>>>>> turn off
>>>>>>>>>> the CPU power.
>>>>>>>>>>
...
>>>>>>>> The DFLL is re-inited after switching CCLK to DFLL parent during o=
f
>>>>>>>> the
>>>>>>>> early clocks-state restoring by CaR driver. Hence instead of havin=
g
>>>>>>>> odd
...
>>>>>>>> responsible for
>>>>>>>> management of the DFLL's state during of suspend/resume process. I=
f
>>>>>>>> CPUFreq driver disables DFLL during suspend and re-enables it duri=
ng
...
>>>>>>>>> 0) Change cclk_g parent to PLLP so the CPU frequency is ok for an=
y
>>>>>>>>> vdd_cpu voltage
>>>>>>>>> 1) Program DFLL to open loop mode
>>>>>>>>>
...
>>>>>
>>>> CPU parents are PLL_X, PLL_P, and dfll. PLL_X always runs at higher ra=
te
>>>> so switching to PLL_P during CPUFreq probe prior to dfll clock enable
>>>> should be safe.
>>> AFAIK, PLLX could run at ~200MHz. There is also a divided output of PLL=
P
...
PLLP_OUT4 rate update is not needed as it is safe to run at 408Mhz=20
