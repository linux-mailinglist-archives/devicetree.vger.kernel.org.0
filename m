...
Received: from vger.kernel.org (vger.kernel.org [23.128.96.18])
	by mail.lfdr.de (Postfix) with ESMTP id A5162358128
	for <lists+devicetree@lfdr.de>; Thu,  8 Apr 2021 12:53:17 +0200 (CEST)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S229721AbhDHKx1 (ORCPT <rfc822;lists+devicetree@lfdr.de>);
        Thu, 8 Apr 2021 06:53:27 -0400
Received: from mx2.suse.de ([195.135.220.15]:33880 "EHLO mx2.suse.de"
        rhost-flags-OK-OK-OK-OK) by vger.kernel.org with ESMTP
        id S229714AbhDHKx0 (ORCPT <rfc822;devicetree@vger.kernel.org>);
        Thu, 8 Apr 2021 06:53:26 -0400
X-Virus-Scanned: by amavisd-new at test-mx.suse.de
Received: from relay2.suse.de (unknown [195.135.221.27])
        by mx2.suse.de (Postfix) with ESMTP id 963F1B007;
        Thu,  8 Apr 2021 10:53:14 +0000 (UTC)
...
Subject: Re: [PATCH v4 4/6] drm/sprd: add Unisoc's drm display controller
 driver
Message-ID: <c5a3dfa7-3739-8a0e-9ffd-b748f8f7f7b3@suse.de>
Date:   Thu, 8 Apr 2021 12:53:13 +0200
...
Content-Type: multipart/signed; micalg=pgp-sha256;
 ...
 boundary="LhMuCzmc7eugLTC0CUqdUAJBzIg6SKVs2"
Precedence: bulk
List-ID: <devicetree.vger.kernel.org>
X-Mailing-List: devicetree@vger.kernel.org

This is an OpenPGP/MIME signed message (RFC 4880 and 3156)
--LhMuCzmc7eugLTC0CUqdUAJBzIg6SKVs2
Content-Type: multipart/mixed; boundary="XthsEpWbHSQ9WxE0veh5oGkpE7Uw7JcRq";
...
--XthsEpWbHSQ9WxE0veh5oGkpE7Uw7JcRq
...
I cannot figure out the purpose of this structure. 'index' is the index=20
of the plane. But there are only primary planes. So you should be able=20
to use drm_plane.index instead.

If index is more like a position in the plane stack, can't you use the=20
zpos property directly?

If there are other non-primary planes, this structure should be called=20
...
> +		return 0;
> +
> +	rc =3D wait_event_interruptible_timeout(ctx->wait_queue, ctx->evt_sto=
p,
...
> +
> +	rc =3D wait_event_interruptible_timeout(ctx->wait_queue, ctx->evt_upd=
ate,
> +					       msecs_to_jiffies(500));
> +
> +	if (!rc) {
...
I think this needs a rebase onto the latest DRM code. Many of these=20
callbacks now receive struct drm_atomic_state instead of=20
...
I find this confusing: there are only primary planes, right? But what's=20
special about the first one? The returned pointer is not used. Maybe=20
...
> +	int ret;
> +
> +	plane =3D sprd_plane_init(drm);
> +	if (IS_ERR(plane))
> +		return PTR_ERR(plane);
> +
> +	dpu =3D sprd_crtc_init(drm, &plane->base);
> +	if (IS_ERR(dpu))
> +		return PTR_ERR(dpu);
> +
> +	dpu->drm =3D drm;
...
> +
> +	ret =3D sprd_dpu_context_init(dpu, dev);
> +	if (ret)
> +		return ret;
> +
> +	return 0;
...
> +
> +static inline void
> +layer_reg_wr(struct dpu_context *ctx, u32 offset, u32 cfg_bits, int in=
dex)
> +{
> +	u32 layer_offset =3D  offset + index * DPU_LAY_REG_OFFSET;
> +
...
> @@ -191,6 +191,7 @@ static struct platform_driver sprd_drm_driver =3D {=
...

=tFti
-----END PGP SIGNATURE-----
