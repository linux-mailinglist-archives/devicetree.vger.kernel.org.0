...
Received: from vger.kernel.org (vger.kernel.org [23.128.96.18])
	by mail.lfdr.de (Postfix) with ESMTP id 60B0A20F5B1
	for <lists+devicetree@lfdr.de>; Tue, 30 Jun 2020 15:33:54 +0200 (CEST)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S2387745AbgF3Nd1 (ORCPT <rfc822;lists+devicetree@lfdr.de>);
        Tue, 30 Jun 2020 09:33:27 -0400
Received: from outils.crapouillou.net ([89.234.176.41]:53922 "EHLO
        crapouillou.net" rhost-flags-OK-OK-OK-OK) by vger.kernel.org
        with ESMTP id S2387581AbgF3NdS (ORCPT
        <rfc822;devicetree@vger.kernel.org>); Tue, 30 Jun 2020 09:33:18 -0400
...
Le mar. 30 juin 2020 =E0 14:16, Sam Ravnborg <sam@ravnborg.org> a =E9crit=20
:
> Hi Paul.
>=20
...
>>  on newer SoCs, bilinear filtering on older SoCs. Nearest-neighbour=20
>> can
>>  also be obtained with proper coefficients.
>>=20
>>  Starting from the JZ4725B, the IPU supports a mode where its output=20
>> is
>>  sent directly to the LCDC, without having to be written to RAM=20
>> first.
>>  This makes it possible to use the IPU as a DRM plane on the=20
>> compatible
>>  SoCs, and have it convert and scale anything the userspace asks for=20
>> to
>>  what's available for the display.
>>=20
>>  Regarding pixel formats, older SoCs support packed YUV 4:2:2 and=20
>> various
>>  planar YUV formats. Newer SoCs introduced support for RGB.
>>=20
>=20
>>  Since the IPU is a separate hardware block, to make it work=20
>> properly the
>>  Ingenic DRM driver will now register itself as a component master in
>>  case the IPU driver has been enabled in the config.
> I cannot see why this driver needs the component framework to deal=20
> with
...
The IPU IP is separate from the LCD controller. Its registers are in a=20
completely different area, it has its own clock, and interrupt line. It=20
has its own node in devicetree. As a result, you have one struct device=20
for the LCD controller, and one for the IPU.

Where it gets more complicated, is that the IPU driver needs a pointer=20
to the drm_device structure in order to register its drm_panel. When=20
the ingenic-drm driver calls component_bind_all(), it passes the=20
pointer, which allows the IPU plane to be created, which will be=20
retrieved by the ingenic-drm driver later on. There would be no way of=20
...
>>=20
>>  When enabled in the config, the CRTC will see the IPU as a second=20
>> primary
>>  plane. It cannot be enabled at the same time as the regular primary
>>  plane. It has the same priority, which means that it will also=20
>> display
...
>>   drivers/gpu/drm/ingenic/ingenic-ipu.c     | 848=20
>> ++++++++++++++++++++++
...
>>  +	help
>>  +	  Choose this option to enable support for the IPU found in=20
>> Ingenic SoCs.
>>  +
...
>>=20
>>  +		if (priv->ipu_plane)
>>  +			ipu_state =3D drm_atomic_get_plane_state(state->state,=20
>> priv->ipu_plane);
>>  +
...
>>   					  struct drm_crtc_state *oldstate)
>>   {
>>  @@ -303,10 +333,9 @@ static void ingenic_drm_plane_enable(struct=20
>> ingenic_drm *priv,
>>   	}
>>   }
>>=20
...
>>   	}
>>   }
>>  @@ -574,6 +612,7 @@ static const struct drm_plane_helper_funcs=20
...
>>   };
>>  @@ -600,10 +639,17 @@ static void ingenic_drm_free_dma_hwdesc(void=20
>> *d)
...
>>  @@ -640,6 +686,19 @@ static int ingenic_drm_probe(struct=20
...
>>  +		}
>>  +
>>  +		ret =3D devm_add_action_or_reset(dev, ingenic_drm_unbind_all,=20
>> priv);
>>  +		if (ret)
>>  +			return ret;
>>  +	}
...
>>  @@ -704,6 +763,9 @@ static int ingenic_drm_probe(struct=20
>> platform_device *pdev)
>>   	if (ret)
>>   		return ret;
>>=20
...
>>  @@ -712,7 +774,7 @@ static int ingenic_drm_probe(struct=20
>> platform_device *pdev)
...
>>   	}
>>=20
>>  @@ -828,9 +890,14 @@ static int ingenic_drm_probe(struct=20
>> platform_device *pdev)
>>   	return ret;
>>   }
>>=20
...
>>  @@ -838,6 +905,42 @@ static int ingenic_drm_remove(struct=20
>> platform_device *pdev)
>>=20
...
>>   	return 0;
>>   }
>>  @@ -879,7 +982,29 @@ static struct platform_driver=20
...
>>  +};
>>  +
>>  +/* Signed 15.16 fixed-point math (for bicubic scaling=20
>> coefficients) */
...
>>  +
>>  +static inline struct ingenic_ipu *plane_to_ingenic_ipu(struct=20
>> drm_plane *plane)
>>  +{
...
>>  + *        A larger magnitude increases perceived sharpness, but=20
>> going past
>>  + *        -2.0 might cause ringing artifacts to outweigh any=20
>> improvement.
>>  + *        Nice values on a 320x240 LCD are between -0.75 and -2.0.
>>  + *
>>  + *  @f_x: Absolute distance in pixels from 'pixel 0' sample=20
>> position
>>  + *        along horizontal (or vertical) source axis. Range is [0,=20
>> +2.0].
>>  + *
>>  + *  returns: Weight of this pixel within 4-pixel sample group.=20
>> Range is
>>  + *           [-2.0, +2.0]. For moderate (i.e. > -3.0) sharpness=20
>> factors,
...
>>  + */
>>  +static void jz4760_set_coefs(struct ingenic_ipu *ipu, unsigned int=20
>> reg,
>>  +			     unsigned int sharpness, bool downscale,
>>  +			     unsigned int weight, unsigned int offset)
>>  +{
>>  +	u32 val;
>>  +	s32 w0, w1, w2, w3; /* Pixel weights at X (or Y) offsets -1,0,1,2=20
>> */
>>  +
...
>>  +		weight =3D weight >=3D 256 ? 512 : 0;
>>  +
>>  +	val |=3D (weight & JZ4725B_IPU_RSZ_LUT_COEF_MASK) <<=20
...
>>  +}
>>  +
>>  +static void ingenic_ipu_set_downscale_coefs(struct ingenic_ipu=20
>> *ipu,
>>  +					    unsigned int reg,
>>  +					    unsigned int num,
>>  +					    unsigned int denom)
...
>>  +}
>>  +
>>  +static void ingenic_ipu_set_integer_upscale_coefs(struct=20
>> ingenic_ipu *ipu,
>>  +						  unsigned int reg,
>>  +						  unsigned int num)
>>  +{
>>  +	/*
>>  +	 * Force nearest-neighbor scaling and use simple math when=20
>> upscaling
>>  +	 * by an integer ratio. It looks better, and fixes a few problem=20
>> cases.
>>  +	 */
>>  +	unsigned int i;
>>  +
...
>>  +}
>>  +
>>  +static void ingenic_ipu_set_coefs(struct ingenic_ipu *ipu,=20
>> unsigned int reg,
...
>>  +}
>>  +
>>  +static void ingenic_ipu_plane_atomic_update(struct drm_plane=20
>> *plane,
>>  +					    struct drm_plane_state *oldstate)
>>  +{
...
>>  +	 * If the IPU supports bicubic, we enable it unconditionally,=20
>> since it
...
>>  +	}
>>  +
>>  +	regmap_update_bits(ipu->map, JZ_REG_IPU_CTRL,=20
...
>>  +
>>  +	dev_dbg(ipu->dev, "Scaling %ux%u to %ux%u (%u:%u horiz, %u:%u=20
>> vert)\n",
...
>>  +		return -EINVAL;
>>  +
>>  +	/* Request a full modeset if we are enabling or disabling the=20
>> IPU. */
...
>>  +}
>>  +
>>  +static void ingenic_ipu_plane_atomic_disable(struct drm_plane=20
>> *plane,
>>  +					     struct drm_plane_state *old_state)
>>  +{
>>  +	struct ingenic_ipu *ipu =3D plane_to_ingenic_ipu(plane);
...
>>  +};
>>  +
>>  +static int ingenic_ipu_bind(struct device *dev, struct device=20
>> *master, void *d)
>>  +{
>>  +	struct platform_device *pdev =3D to_platform_device(dev);
>>  +	const struct soc_info *soc_info;
...
>>  +	}
>>  +
>>  +	ipu->map =3D devm_regmap_init_mmio(dev, base,=20
...
>>  +
>>  +	err =3D drm_universal_plane_init(drm, plane, 1,=20
...
>>  +	 * 0       : nearest-neighbor
>>  +	 * 1       : bilinear
>>  +	 * 2 .. 32 : bicubic (translated to sharpness factor -0.25 ..=20
>> -4.0)
>>  +	 */
>>  +	sharpness_max =3D soc_info->has_bicubic ? 32 : 1;
>>  +	ipu->sharpness_prop =3D drm_property_create_range(drm, 0,=20
>> "sharpness",
>>  +							0, sharpness_max);
...
>>  +	{ .compatible =3D "ingenic,jz4725b-ipu", .data =3D &jz4725b_soc_info=20
>> },
...
>>  +};
>>  +
>>  +struct platform_driver *ingenic_ipu_driver_ptr =3D=20
>> &ingenic_ipu_driver;
>>  diff --git a/drivers/gpu/drm/ingenic/ingenic-ipu.h=20
...
>>  +#define JZ_IPU_D_FMT_OUT_FMT_RGB555	(0x0 <<=20
>> JZ_IPU_D_FMT_OUT_FMT_LSB)
>>  +#define JZ_IPU_D_FMT_OUT_FMT_RGB565	(0x1 <<=20
>> JZ_IPU_D_FMT_OUT_FMT_LSB)
>>  +#define JZ_IPU_D_FMT_OUT_FMT_RGB888	(0x2 <<=20
>> JZ_IPU_D_FMT_OUT_FMT_LSB)
>>  +#define JZ_IPU_D_FMT_OUT_FMT_YUV422	(0x3 <<=20
>> JZ_IPU_D_FMT_OUT_FMT_LSB)
>>  +#define JZ_IPU_D_FMT_OUT_FMT_RGBAAA	(0x4 <<=20
>> JZ_IPU_D_FMT_OUT_FMT_LSB)
>>  +
>>  +#define JZ_IPU_D_FMT_RGB_OUT_OFT_LSB	22
>>  +#define JZ_IPU_D_FMT_RGB_OUT_OFT_RGB	(0x0 <<=20
>> JZ_IPU_D_FMT_RGB_OUT_OFT_LSB)
>>  +#define JZ_IPU_D_FMT_RGB_OUT_OFT_RBG	(0x1 <<=20
>> JZ_IPU_D_FMT_RGB_OUT_OFT_LSB)
>>  +#define JZ_IPU_D_FMT_RGB_OUT_OFT_GBR	(0x2 <<=20
>> JZ_IPU_D_FMT_RGB_OUT_OFT_LSB)
>>  +#define JZ_IPU_D_FMT_RGB_OUT_OFT_GRB	(0x3 <<=20
>> JZ_IPU_D_FMT_RGB_OUT_OFT_LSB)
>>  +#define JZ_IPU_D_FMT_RGB_OUT_OFT_BRG	(0x4 <<=20
>> JZ_IPU_D_FMT_RGB_OUT_OFT_LSB)
>>  +#define JZ_IPU_D_FMT_RGB_OUT_OFT_BGR	(0x5 <<=20
>> JZ_IPU_D_FMT_RGB_OUT_OFT_LSB)
>>  +
