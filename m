...
Received: from vger.kernel.org (vger.kernel.org [209.132.180.67])
	by mail.lfdr.de (Postfix) with ESMTP id E4E7BB2FBA
	for <lists+devicetree@lfdr.de>; Sun, 15 Sep 2019 13:28:03 +0200 (CEST)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S1726247AbfIOL2D (ORCPT <rfc822;lists+devicetree@lfdr.de>);
        Sun, 15 Sep 2019 07:28:03 -0400
Received: from mail.kernel.org ([198.145.29.99]:33716 "EHLO mail.kernel.org"
        rhost-flags-OK-OK-OK-OK) by vger.kernel.org with ESMTP
        id S1726246AbfIOL2C (ORCPT <rfc822;devicetree@vger.kernel.org>);
        Sun, 15 Sep 2019 07:28:02 -0400
Received: from archlinux (cpc149474-cmbg20-2-0-cust94.5-4.cable.virginm.net [82.4.196.95])
        (using TLSv1.2 with cipher ECDHE-RSA-AES256-GCM-SHA384 (256/256 bits))
        (No client certificate requested)
        by mail.kernel.org (Postfix) with ESMTPSA id B4C322077C;
        Sun, 15 Sep 2019 11:27:57 +0000 (UTC)
DKIM-Signature: v=1; a=rsa-sha256; c=relaxed/simple; d=kernel.org;
        s=default; t=1568546879;
        bh=hZF5n79NPKBykNpUxlQF+srCoa1If3lfiZhLNIZ1L90=;
        h=Date:From:To:Cc:Subject:In-Reply-To:References:From;
        b=utOVIw55TWhTa6FmQbMSHwaf8JkoPp9pcdj37gjGj+R3H0+mlNFTxLEG0q17qNL5i
         zDhRohEjoiD0k/9LXNIJYbRNWEuvETc53Ed1mdQMPp4vDQOuSBKEDFfvr22lZeG1Xv
         zFndqMUauqlL+VEQcmxEM067EL6/3VOd6i9/owiE=
Date:   Sun, 15 Sep 2019 12:27:54 +0100
...
Message-ID: <20190915122754.4a529e9e@archlinux>
...
>  create mode 100644 Documentation/ABI/testing/sysfs-bus-iio-temperature-l=
tc2983
>  create mode 100644 drivers/iio/temperature/ltc2983.c
>=20
> diff --git a/Documentation/ABI/testing/sysfs-bus-iio-temperature-ltc2983 =
...
For each of these, I presume we know which type of device is attached at an=
y time?
Using the channel naming to convey this (and I assume the fact that differe=
nt
conversions need to be done in userspace?) is a bit messy.  If we need
to convey the channel type, then a separate in_tempY_mode attribute may mak=
e more
sense.  That would keep this ABI 'closer' to standard. Software that just l=
ogs
...
> +
> +	n_entries =3D of_property_count_elems_of_size(np, "adi,custom-sensor",
> +						sizeof(u64));
...
> +		thermo->custom =3D __ltc2983_custom_sensor_new(st, child, false,
> +							     16384, true);
...
> +	thermo->sensor.name =3D "thermocouple";
...
> +		rtd->custom =3D __ltc2983_custom_sensor_new(st, child, false,
> +							  2048, false);
...
> +			sensor->type =3D=3D LTC2983_SENSOR_THERMISTOR_STEINHART ?
> +							  true : false, 64,
> +							  false);
...
> +	}
> +	/* set common parameters */
> +	thermistor->sensor.name =3D "thermistor";
...
> +	}
> +	/* get raw value */
> +	ret =3D of_property_read_u64(child, "adi,rsense-val-micro-ohms", &temp);
> +	if (ret) {
...
> +
> +	/* 2^10 resolution */
> +	rsense->r_sense_val =3D __convert_to_raw(temp, 1024);
> +
> +	/* set common parameters */
> +	rsense->sensor.name =3D "r_sense";
...
> +	global_cfg =3D LTC2983_NOTCH_FREQ(st->filter_notch_freq);
> +	global_cfg |=3D LTC2983_TEMP_UNITS(st->temp_farenheit);
...
> +
> +	mutex_lock(&st->lock);
> +	ret =3D regmap_write(st->regmap, LTC2983_STATUS_REG, LTC2983_SLEEP);
> +	st->reset =3D true;
