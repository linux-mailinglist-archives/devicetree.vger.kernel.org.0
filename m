...
Received: from vger.kernel.org (vger.kernel.org [23.128.96.18])
	by mail.lfdr.de (Postfix) with ESMTP id 2C5E049DD5D
	for <lists+devicetree@lfdr.de>; Thu, 27 Jan 2022 10:09:55 +0100 (CET)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S238261AbiA0JJD (ORCPT <rfc822;lists+devicetree@lfdr.de>);
        Thu, 27 Jan 2022 04:09:03 -0500
Received: from mxout70.expurgate.net ([91.198.224.70]:55450 "EHLO
        mxout70.expurgate.net" rhost-flags-OK-OK-OK-OK) by vger.kernel.org
        with ESMTP id S238246AbiA0JJB (ORCPT
        <rfc822;devicetree@vger.kernel.org>); Thu, 27 Jan 2022 04:09:01 -0500
Received: from [127.0.0.1] (helo=localhost)
        by relay.expurgate.net with smtp (Exim 4.92)
        (envelope-from <fe@dev.tdt.de>)
        id 1nD0lw-000F7j-VG; Thu, 27 Jan 2022 10:08:53 +0100
Received: from [195.243.126.94] (helo=securemail.tdt.de)
        by relay.expurgate.net with esmtps (TLS1.3:ECDHE_RSA_AES_256_GCM_SHA384:256)
        (Exim 4.92)
        (envelope-from <fe@dev.tdt.de>)
        id 1nD0lv-000LSN-Sz; Thu, 27 Jan 2022 10:08:52 +0100
Received: from securemail.tdt.de (localhost [127.0.0.1])
        by securemail.tdt.de (Postfix) with ESMTP id DE5D4240052;
        Thu, 27 Jan 2022 10:08:50 +0100 (CET)
Received: from mail.dev.tdt.de (unknown [10.2.4.42])
        by securemail.tdt.de (Postfix) with ESMTP id 501FB240040;
        Thu, 27 Jan 2022 10:08:50 +0100 (CET)
Received: from localhost.localdomain (unknown [10.2.3.40])
        by mail.dev.tdt.de (Postfix) with ESMTPSA id 4709924BDA;
...
Subject: [PATCH v5 1/2] leds: ktd20xx: Extension of the KTD20xx family of LED drivers from Kinetic
Date:   Thu, 27 Jan 2022 10:08:40 +0100
Message-ID: <20220127090841.396-2-fe@dev.tdt.de>
...
X-purgate: clean
X-purgate-ID: 151534::1643274532-00000DA7-704DB655/0/0
Precedence: bulk
...
+	struct device *dev =3D &led->chip->client->dev;
+	unsigned long rgb =3D 0;
+	int ret;
+	int i;
+
...
+			dev_warn(dev, "property 'color' is not equal to the value 'LED_COLOR_=
ID_MULTI'\n");
+			ret =3D -EINVAL;
+			goto child_out;
+		}
...
+		switch (led->index) {
+		case RGB_A1:
+			led->select =3D devm_regmap_field_alloc(dev,
+					chip->regmap, kt20xx_a1_select);
+			led->enable =3D devm_regmap_field_alloc(dev,
...
+	}
+
+	chip->control_mode =3D devm_regmap_field_alloc(dev, chip->regmap,
+			kt20xx_control_mode);
+	chip->faderate =3D devm_regmap_field_alloc(dev, chip->regmap,
+			kt20xx_faderate);
+	chip->vendor =3D devm_regmap_field_alloc(dev, chip->regmap,
+			kt20xx_vendor);
+	chip->chip_id =3D devm_regmap_field_alloc(dev, chip->regmap,
+			kt20xx_chip_id);
+	chip->chip_rev =3D devm_regmap_field_alloc(dev, chip->regmap,
+			kt20xx_chip_rev);
+
