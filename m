...
Received: from out1.vger.email (out1.vger.email [IPv6:2620:137:e000::1:20])
	by mail.lfdr.de (Postfix) with ESMTP id 036886FC39F
	for <lists+devicetree@lfdr.de>; Tue,  9 May 2023 12:12:18 +0200 (CEST)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S229674AbjEIKMQ (ORCPT <rfc822;lists+devicetree@lfdr.de>);
        Tue, 9 May 2023 06:12:16 -0400
Received: from lindbergh.monkeyblade.net ([23.128.96.19]:49484 "EHLO
        lindbergh.monkeyblade.net" rhost-flags-OK-OK-OK-OK) by vger.kernel.org
        with ESMTP id S234878AbjEIKMP (ORCPT
        <rfc822;devicetree@vger.kernel.org>); Tue, 9 May 2023 06:12:15 -0400
Received: from perceval.ideasonboard.com (perceval.ideasonboard.com [213.167.242.64])
        by lindbergh.monkeyblade.net (Postfix) with ESMTPS id D53074499;
        Tue,  9 May 2023 03:11:58 -0700 (PDT)
Received: from [192.168.0.43] (cpc141996-chfd3-2-0-cust928.12-3.cable.virginm.net [86.13.91.161])
        by perceval.ideasonboard.com (Postfix) with ESMTPSA id 74460FB;
        Tue,  9 May 2023 12:11:49 +0200 (CEST)
DKIM-Signature: v=1; a=rsa-sha256; c=relaxed/simple; d=ideasonboard.com;
        s=mail; t=1683627110;
        bh=veMhPPoZfpm/uLuXlVv6v4aQQ0pk9Tg5iKZLS5ZIEGw=;
        h=Date:From:Subject:To:Cc:References:In-Reply-To:From;
        b=sAA0dN5zdUm1c+y+hIJIS/2JDlQ47AwwiK5T+Sp6qLcdaMRtxMKBW58H0RDAjAQzL
         sygiYWqTcsX0e18BKtP81e5DQyGYIWG0xGEDc/VmOXJfrralK/TCfakzQ9/R3kpmcz
         1jkHShcowzZJHX478jipFArIUUiAoByJbLV7gc50=
Message-ID: <8d96a7f5-fc2c-2405-68aa-7c44f09ba545@ideasonboard.com>
Date:   Tue, 9 May 2023 11:11:53 +0100
MIME-Version: 1.0
...
In-Reply-To: <20220910144010.34272-4-hugues.fruchet@foss.st.com>
...
vcap->ved.vdev_get_format, but I can't see that that's used; is it a hangover of something?
...
VIDIOC_ENUM_FMT is supposed to behave differently depending on whether the device is a video-node 
centric device or a media controller centric one. I think this is the latter and so this function 
needs to be able to return the pixelformat that's supported by a particular media bus code too.
...
If I'm understanding right, I think this can be dropped - afaik it just calls the .s_power() op for 
each element in the graph, but none of them implement that op that I can see (and I think it's 
deprecated anyway) - gc2145 uses runtime PM. st-mipid02 actually seems to be always-on following 
...
This section is based on the return from readl_relaxed_poll_timeout()? Can it not go earlier as part 
of a block with the dev_warn()?
...
I think along with the change to dcmipp_bytecap_enum_fmt_vid_cap() suggested above, you'd need 
V4L2_CAP_IO_MC here to advertise the ability.
...
Given this is only used once in fmt_default I think the extra macro is unecessary - better to just 
use the MEDIA_BUS_FMT_RGB565_2X8_LE one directly imo.
...
I think that this might need some refinement - the enumeration for the source pad will depend on the 
format set on the sink pad as unless I'm mistaken the DCMIPP can't do any kind of conversion? It 
should also take into account the code->which field to make sure it's looking at the right set format.
...
And same here for the source pad - if the sink has had the decimation applied the source won't be 
able to output anything outside the bounds of that rectangle anymore.
> +
...
"Sub-devices that support the frame interval enumeration ioctl should implemented it on a single pad 
only. Its behaviour when supported on multiple pads of the same sub-device is not defined."
...
"Sub-devices that support the frame interval ioctls should implement them on a single pad only. 
Their behaviour when supported on multiple pads of the same sub-device is not defined."
...
I was going to say I thought maybe MEDIA_ENT_F_PROC_VIDEO_SCALER was better here, on the grounds 
that the docs [3] say:

"Video scaler. An entity capable of video scaling must have at least one sink pad and one source 
pad, and scale the video frame(s) received on its sink pad(s) to a different resolution output on 
its source pad(s). The range of supported scaling ratios is entity-specific and can differ between 
the horizontal and vertical directions (in particular scaling can be supported in one direction 
only). Binning and sub-sampling (occasionally also referred to as skipping) are considered as scaling."
...
This sets the source pad's ACTIVE format with the adjusted format regardless of whether fmt->which 
is TRY or ACTIVE.
> +
> +	return 0;
> +}
