...
Received: from vger.kernel.org (vger.kernel.org [23.128.96.18])
	by mail.lfdr.de (Postfix) with ESMTP id 678432DA008
	for <lists+devicetree@lfdr.de>; Mon, 14 Dec 2020 20:13:42 +0100 (CET)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S2502546AbgLNTNb (ORCPT <rfc822;lists+devicetree@lfdr.de>);
        Mon, 14 Dec 2020 14:13:31 -0500
Received: from mga07.intel.com ([134.134.136.100]:56837 "EHLO mga07.intel.com"
        rhost-flags-OK-OK-OK-OK) by vger.kernel.org with ESMTP
        id S2440614AbgLNTNV (ORCPT <rfc822;devicetree@vger.kernel.org>);
        Mon, 14 Dec 2020 14:13:21 -0500
IronPort-SDR: XcEDSdQJs/SqCUavYCXvaF9iia+hO+gopiHi9vg6KlbAf2LgEBWlHk4Ow78oM9bhNUim7/S5Se
 oH1Ba9TItiEg==
X-IronPort-AV: E=McAfee;i="6000,8403,9834"; a="238864127"
X-IronPort-AV: E=Sophos;i="5.78,420,1599548400"; 
   d="scan'208";a="238864127"
Received: from orsmga008.jf.intel.com ([10.7.209.65])
  by orsmga105.jf.intel.com with ESMTP/TLS/ECDHE-RSA-AES256-GCM-SHA384; 14 Dec 2020 11:11:31 -0800
IronPort-SDR: AV1mUBEz+cNEhEgY/KIx2gr9hl4ZUvxTUOWjIjmRRJqopx4l8hKhceaXq/N+UC2OqGaJcgtCuh
 5kMOlfIip0ag==
X-IronPort-AV: E=Sophos;i="5.78,420,1599548400"; 
   d="scan'208";a="367575433"
Received: from akemp-mobl3.ger.corp.intel.com ([10.252.25.47])
  by orsmga008-auth.jf.intel.com with ESMTP/TLS/ECDHE-RSA-AES256-GCM-SHA384; 14 Dec 2020 11:11:28 -0800
Message-ID: <73ccd8a6d3d44b1b61b751135d3e2dfe99722b95.camel@linux.intel.com>
Subject: Re: [PATCH v3 2/3] crypto: keembay: Add Keem Bay OCS HCU driver
...
Date:   Mon, 14 Dec 2020 19:11:22 +0000
In-Reply-To: <20201119231541.GA32940@mtg-dev.jf.intel.com>
References: <20201116185846.773464-1-daniele.alessandrelli@linux.intel.com>
         <20201116185846.773464-3-daniele.alessandrelli@linux.intel.com>
         <20201119231541.GA32940@mtg-dev.jf.intel.com>
...
Intel internal crypto team, who, due to internal policy, had to review
this driver before it could be made public.

I understand this may look strange to the Kernel community, but I'm not
allowed to upstream SHA224 support without following the
recommendations from our internal crypto team.

Anyway, to isolate this issue, in v4, I've isolated SHA224 support in a
different patch, so that discussion about SHA224 can continue without
blocking the upstreaming of the rest of the driver.

> * do you need spin lock bh  or would a plane spin lock do?
> * would a mutex do instead of the spinlock?

Other drivers (atmel-sha.c, omap-sham.c, stm32-hash.c, etc.) are also
using spin_lock_bh()). I think that's because some ahash functions can
be called from softirqs.
...
Explained above. Basically, it's just due to some company internal
policy. I understand that the Linux crypto maintainers may reject this.
...
My understanding is that some crypto ahash functions can be called from
softirqs, so spin_lock_bh() is required.

Regarding the list processing, we just get the first element, so the
operation is time bound.
...
Number of entries (I think), it's a variable name commonly used by the
scatterlist API.
...
Good point. The '<' is just to handle possible bugs gracefully.

In v4, I've added a WARN() to notify of the unexpected event.
...
I have tried to improve the error message in v4.
...
Good point, I've fixed it in v4.
...
An admin should just report the bug to the driver maintainers.

Anyway, v4 tries to improve this message.
...
Other crypto device drivers are using the simple spin_lock() /
spin_unlock() here. Not sure why, I'll change it to spin_lock_bh()
because I think it's more correct.
...
Same as above. I think we can have a concurrency issue if another
device is already probed and running (this is only theoretical in Keem
Bay, since there is only one OCS HCU device).
...
Good question. I've tried to separate the code handling the HW from the
one implementing the crypto API. The ocs-hcu.c driver only contains the
fist kind of code, so I didn't put any define related to the second.
I'd prefer to keep it this way if possible.
...
idata->digest is an array of SHA512_DIGEST_SIZE bytes and the maximum
value othat ocs_hcu_num_chains() can return
is OCS_HCU_NUM_CHAINS_SHA384_512 which is SHA512_DIGEST_SIZE / 4.

But you are right, I need a comment to explain this.
...
I reworked the code and removed this function all together in v4.
...
Function removed in v4 (all PIO ashing code has been removed in favor
of DMA operations).
