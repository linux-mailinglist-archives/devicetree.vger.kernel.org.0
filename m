...
Received: from out1.vger.email (out1.vger.email [IPv6:2620:137:e000::1:20])
	by mail.lfdr.de (Postfix) with ESMTP id 4CE16583CCF
	for <lists+devicetree@lfdr.de>; Thu, 28 Jul 2022 13:03:57 +0200 (CEST)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S236142AbiG1LDy (ORCPT <rfc822;lists+devicetree@lfdr.de>);
        Thu, 28 Jul 2022 07:03:54 -0400
Received: from lindbergh.monkeyblade.net ([23.128.96.19]:33448 "EHLO
        lindbergh.monkeyblade.net" rhost-flags-OK-OK-OK-OK) by vger.kernel.org
        with ESMTP id S234566AbiG1LDx (ORCPT
        <rfc822;devicetree@vger.kernel.org>); Thu, 28 Jul 2022 07:03:53 -0400
Received: from mail-wm1-x334.google.com (mail-wm1-x334.google.com [IPv6:2a00:1450:4864:20::334])
        by lindbergh.monkeyblade.net (Postfix) with ESMTPS id 830E4664D7;
        Thu, 28 Jul 2022 04:03:50 -0700 (PDT)
Received: by mail-wm1-x334.google.com with SMTP id b6so756988wmq.5;
        Thu, 28 Jul 2022 04:03:50 -0700 (PDT)
DKIM-Signature: v=1; a=rsa-sha256; c=relaxed/relaxed;
        d=gmail.com; s=20210112;
        h=mime-version:subject:from:in-reply-to:date:cc
         :content-transfer-encoding:message-id:references:to;
        bh=n3PmPErZ3qz515kkpqX3FC9Qmz7SVOo0xd78m90Zoi4=;
        b=
X-Google-DKIM-Signature: v=1; a=rsa-sha256; c=relaxed/relaxed;
        ...
        h=x-gm-message-state:mime-version:subject:from:in-reply-to:date:cc
         :content-transfer-encoding:message-id:references:to;
        bh=n3PmPErZ3qz515kkpqX3FC9Qmz7SVOo0xd78m90Zoi4=;
        b=
X-Gm-Message-State: AJIora8TtDzfLuqBISRqgSOvTsafLEu5jM68kYHegwl1o1XDL/ZRHhOJ
        UsGJDHvtJ01jvVTbmoO9orekGoaWRkaKhgru
X-Google-Smtp-Source: AGRyM1vLK66d1xtw9ViCvUkXCldhot1vnsD7C5GnZu7lkhhvIATkwNjTkuhg+VWtAkpjvgtsfhG4wA==
X-Received: by 2002:a05:600c:3d93:b0:3a3:3a93:fb16 with SMTP id bi19-20020a05600c3d9300b003a33a93fb16mr6393409wmb.190.1659006228676;
        Thu, 28 Jul 2022 04:03:48 -0700 (PDT)
Received: from smtpclient.apple ([87.200.95.144])
        by smtp.gmail.com with ESMTPSA id v62-20020a1cac41000000b003a31f1edfa7sm5276765wme.41.2022.07.28.04.03.45
...
Date:   Thu, 28 Jul 2022 15:03:43 +0400
...
Message-Id: <A555E3E2-289D-4D27-A63A-B6BAFE33C912@gmail.com>
...
> On 27 Jul 2022, at 3:03 am, Jiucheng Xu <jiucheng.xu@amlogic.com> =
wrote:
>=20
...
>   aml_ddr_bw/chan_1_rw_bytes,arm=3D1/ -I 1000 \
>   sleep 10
>=20
...
>=20
> $ perf stat -a -e \
>   aml_ddr_bw/chan_2_rw_bytes,arm=3D1,gpu=3D1,usb3_0=3D1,nna=3D1/ -I =
1000 \
>   sleep 10
>=20
...
> +obj-$(CONFIG_AML_DDR_PMU) +=3D amlogic/

Upstream kernel Amlogic drivers normally use MESON naming, so the
config item would be e.g. CONFIG_MESON_DDR_PMU.

> obj-$(CONFIG_ARM_CCI_PMU) +=3D arm-cci.o
> obj-$(CONFIG_ARM_CCN) +=3D arm-ccn.o
> obj-$(CONFIG_ARM_CMN) +=3D arm-cmn.o
> diff --git a/drivers/perf/amlogic/Kconfig =
...
> +# SPDX-License-Identifier: GPL-2.0-only
> +
> +obj-$(CONFIG_AML_DDR_PMU) +=3D aml_ddr_pmu.o
> +
> +aml_ddr_pmu-y	:=3D aml_ddr_pmu_core.o aml_ddr_pmu_g12.o

I would similarly expect to see meson_ filenames to reflect the
MESON driver name, e.g. meson_ddr_pmu

> diff --git a/drivers/perf/amlogic/aml_ddr_pmu_core.c =
...
> +
> +#define to_ddr_pmu(p)		container_of(p, struct ddr_pmu, =
pmu)
> +
...
> +		chann =3D event->attr.config - CHAN1_COUNTER_ID;
> +
...
> +	u64 cnt1, cnt2;
> +
> +	sum->all_cnt =3D add1->all_cnt + add2->all_cnt;
> +	sum->all_req =3D add1->all_req + add2->all_req;
> +	for (i =3D 0; i < chann_nr; i++) {
> +		cnt1 =3D add1->channel_cnt[i];
> +		cnt2 =3D add2->channel_cnt[i];
> +
> +		sum->channel_cnt[i] =3D cnt1 + cnt2;
> +	}
> +}
> +
...
> +{
> +	struct ddr_pmu *pmu =3D to_ddr_pmu(event->pmu);
> +	u64 new_raw_count =3D 0;
> +	struct dmc_counter dc =3D {0}, sum_dc =3D {0};
> +	int idx;
> +
...
> +		new_raw_count =3D sum_dc.all_cnt;
> +		break;
...
> +		idx =3D event->attr.config - CHAN1_COUNTER_ID;
> +		new_raw_count =3D sum_dc.channel_cnt[idx];
> +		break;
> +	}
> +
...
> +{
> +	struct ddr_pmu *pmu =3D to_ddr_pmu(event->pmu);
> +	u64 config1 =3D event->attr.config1;
> +
...
> +	if (event->attr.config !=3D ALL_CHAN_COUNTER_ID &&
...
> +}
> +
> +static int aml_ddr_perf_event_add(struct perf_event *event, int =
flags)
> +{
> +	u64 config1 =3D event->attr.config1;
> +	int i;
> +
> +	for_each_set_bit(i, (const unsigned long *)&config1, =
sizeof(config1))
...
> +}
> +
> +static void aml_ddr_perf_event_del(struct perf_event *event, int =
flags)
> +{
...
> +}
> +
> +static struct device_attribute aml_ddr_perf_cpumask_attr =3D
> +__ATTR(cpumask, 0444, aml_ddr_perf_cpumask_show, NULL);
> +
> +static struct attribute *aml_ddr_perf_cpumask_attrs[] =3D {
> +	&aml_ddr_perf_cpumask_attr.attr,
> +	NULL,
> +};
> +
> +static const struct attribute_group ddr_perf_cpumask_attr_group =3D {
> +	.attrs =3D aml_ddr_perf_cpumask_attrs,
> +};
> +
> +static ssize_t
...
attr);
> +
> +	/* one count =3D 16byte =3D 1.52587890625e-05 MB */
...
> +{									=
\
> +	.attr =3D __ATTR(_name, 0444, pmu_event_show, NULL),		=
\
> +	.id =3D _id,							=
\
> +}
...
> +
> +static struct device_attribute event_unit_attrs[] =3D {
...
> +};
> +
> +static struct device_attribute event_scale_attrs[] =3D {
...
> +};
> +
> +static struct perf_pmu_events_attr event_attrs[] =3D {
...
> +
> +static struct attribute_group ddr_perf_events_attr_group =3D {
> +	.name =3D "events",
...
> +}
> +
> +static struct attribute_group ddr_perf_format_attr_group =3D {
> +	.name =3D "format",
> +	.attrs =3D ddr_perf_format_attrs,
> +	.is_visible =3D aml_ddr_perf_format_attr_visible,
> +};
> +
...
> +}
> +
> +static struct device_attribute aml_ddr_perf_identifier_attr =3D
> +__ATTR(identifier, 0444, aml_ddr_perf_identifier_show, NULL);
> +
> +static struct attribute *aml_ddr_perf_identifier_attrs[] =3D {
> +	&aml_ddr_perf_identifier_attr.attr,
> +	NULL,
> +};
> +
> +static const struct attribute_group ddr_perf_identifier_attr_group =3D =
{
> +	.attrs =3D aml_ddr_perf_identifier_attrs,
> +};
> +
...
> +{
> +	struct dmc_hw_info *info =3D dev_id;
> +	struct ddr_pmu *pmu;
> +	struct dmc_counter counters, *sum_cnter;
> +	int i;
> +
> +	pmu =3D hw_info_to_pmu(info);
> +
> +	if (info->ops->irq_handler(info, &counters) !=3D 0)
> +		goto out;
> +
> +	sum_cnter =3D &pmu->counters;
> +	sum_cnter->all_cnt +=3D counters.all_cnt;
> +	sum_cnter->all_req +=3D counters.all_req;
> +
> +	for (i =3D 0; i < pmu->info.chann_nr; i++)
> +		sum_cnter->channel_cnt[i] +=3D counters.channel_cnt[i];
> +
> +	if (pmu->pmu_enabled)
> +		/*
...
> +	pmu->cpu =3D target;
> +
> +	WARN_ON(irq_set_affinity(pmu->info.irq_num, =
cpumask_of(pmu->cpu)));
> +
> +	return 0;
> +}
...
> +{
> +	int i, j, k;
> +	struct attribute **dst =3D ddr_perf_events_attrs;
> +
> +	j =3D 0;
> +	k =3D 0;
> +
> +	/* fill ALL_CHAN_COUNTER_ID event */
> +	dst[j++] =3D &event_attrs[k].attr.attr;
> +	dst[j++] =3D &event_unit_attrs[k].attr;
> +	dst[j++] =3D &event_scale_attrs[k].attr;
> +
> +	k++;
> +
> +	/* fill each channel event */
> +	for (i =3D 0; i < pmu->info.chann_nr; i++, k++) {
> +		dst[j++] =3D &event_attrs[k].attr.attr;
> +		dst[j++] =3D &event_unit_attrs[k].attr;
> +		dst[j++] =3D &event_scale_attrs[k].attr;
> +	}
> +
> +	dst[j] =3D NULL; /* mark end */
> +}
> +
...
> +	int i;
> +
> +	for (i =3D 0; fmt_attr[i]; i++)
> +		ddr_perf_format_attrs[i] =3D fmt_attr[i];
> +
> +	ddr_perf_format_attrs[i] =3D NULL;
> +}
> +
> +static int ddr_pmu_parse_dt(struct platform_device *pdev, struct =
dmc_hw_info *info)
> +{
> +	/*struct pinctrl *p;*/
> +	void __iomem *base;
> +	int i, ret =3D -EINVAL;
> +
> +	for (i =3D 0; i < info->dmc_nr; i++) {
> +		/* resource 0 for ddr register base */
> +		base =3D devm_platform_ioremap_resource(pdev, i);
> +		if (IS_ERR(base)) {
> +			dev_err(&pdev->dev, "couldn't ioremap ddr reg =
%d\n", i);
> +			return PTR_ERR(base);
> +		}
> +		info->ddr_reg[i] =3D base;
> +	}
> +
...
> +	info->irq_num =3D ret;
> +
> +	ret =3D devm_request_irq(&pdev->dev, info->irq_num, =
dmc_irq_handler,
...
> +}
> +
> +int aml_ddr_pmu_create(struct platform_device *pdev, struct ddr_pmu =
*pmu)
> +{
> +	int ret;
> +	char *name;
> +	struct pmu tmp_pmu =3D {
> +		.module		=3D THIS_MODULE,
> +		.capabilities	=3D PERF_PMU_CAP_NO_EXCLUDE,
> +		.task_ctx_nr	=3D perf_invalid_context,
> +		.attr_groups	=3D attr_groups,
> +		.event_init	=3D aml_ddr_perf_event_init,
> +		.add		=3D aml_ddr_perf_event_add,
> +		.del		=3D aml_ddr_perf_event_del,
> +		.start		=3D aml_ddr_perf_event_start,
> +		.stop		=3D aml_ddr_perf_event_stop,
> +		.read		=3D aml_ddr_perf_event_update,
> +	};
> +
> +	pmu->pmu =3D tmp_pmu;
> +
> +	ret =3D ddr_pmu_parse_dt(pdev, &pmu->info);
> +	if (ret < 0)
> +		return ret;
> +
...
> +	}
> +
> +	ret =3D cpuhp_setup_state_multi(CPUHP_AP_ONLINE_DYN, name, NULL,
...
> +	ret =3D cpuhp_state_add_instance_nocalls(pmu->cpuhp_state, =
&pmu->node);
> +	if (ret) {
...
> +
> +pmu_register_err:
> +	cpuhp_state_remove_instance_nocalls(pmu->cpuhp_state, =
&pmu->node);
> +cpuhp_instance_err:
...
> +
> +static struct attribute *g12_pmu_format_attrs[] =3D {
...
> +	unsigned int od_div =3D 0xfff;
> +	unsigned long freq =3D 0;
> +
> +	val =3D readl(info->pll_reg);
> +	val =3D val & 0xfffff;
> +	switch ((val >> 16) & 7) {
> +	case 0:
> +		od_div =3D 2;
...
> +	m =3D val & 0x1ff;
> +	n =3D ((val >> 10) & 0x1f);
> +	od1 =3D (((val >> 19) & 0x1)) =3D=3D 1 ? 2 : 1;
> +	freq =3D DEFAULT_XTAL_FREQ / 1000;        /* avoid overflow */
> +	if (n)
> +		freq =3D ((((freq * m) / n) >> od1) / od_div) * 1000;
> +
> +	return freq;
> +}
> +
...
> +
> +	for (i =3D 0; i < 9; i++) {
> +		r  =3D readl(db->ddr_reg[0] + (DMC_MON_G12_CTRL0 + (i << =
2)));
> +		pr_notice("DMC_MON_CTRL%d:        %08x\n", i, r);
> +	}
> +	r  =3D readl(db->ddr_reg[0] + DMC_MON_G12_ALL_REQ_CNT);
> +	pr_notice("DMC_MON_ALL_REQ_CNT:  %08x\n", r);
> +	r  =3D readl(db->ddr_reg[0] + DMC_MON_G12_ALL_GRANT_CNT);
> +	pr_notice("DMC_MON_ALL_GRANT_CNT:%08x\n", r);
> +	r  =3D readl(db->ddr_reg[0] + DMC_MON_G12_ONE_GRANT_CNT);
> +	pr_notice("DMC_MON_ONE_GRANT_CNT:%08x\n", r);
> +	r  =3D readl(db->ddr_reg[0] + DMC_MON_G12_SEC_GRANT_CNT);
> +	pr_notice("DMC_MON_SEC_GRANT_CNT:%08x\n", r);
> +	r  =3D readl(db->ddr_reg[0] + DMC_MON_G12_THD_GRANT_CNT);
> +	pr_notice("DMC_MON_THD_GRANT_CNT:%08x\n", r);
> +	r  =3D readl(db->ddr_reg[0] + DMC_MON_G12_FOR_GRANT_CNT);
> +	pr_notice("DMC_MON_FOR_GRANT_CNT:%08x\n", r);
> +	r  =3D readl(db->ddr_reg[0] + DMC_MON_G12_TIMER);
...
> +{
> +	unsigned int val;
> +	unsigned long clock_count =3D dmc_g12_get_freq_quick(info) / 10; =
/* 100ms */
> +
> +	writel(clock_count, info->ddr_reg[0] + DMC_MON_G12_TIMER);
> +
> +	val =3D readl(info->ddr_reg[0] + DMC_MON_G12_CTRL0);
> +
> +	/* enable all channel */
> +	val =3D  BIT(31) |	/* enable bit */
> +	       BIT(20) |	/* use timer  */
> +	       0x0f;		/* 4 channels */
> +
...
> +{
> +	u32 val;
> +	u32 rp[MAX_CHANNEL_NUM] =3D {DMC_MON_G12_CTRL1, =
DMC_MON_G12_CTRL3,
> +					DMC_MON_G12_CTRL5, =
DMC_MON_G12_CTRL7};
> +	u32 rs[MAX_CHANNEL_NUM] =3D {DMC_MON_G12_CTRL2, =
DMC_MON_G12_CTRL4,
> +					DMC_MON_G12_CTRL6, =
DMC_MON_G12_CTRL8};
> +	int subport =3D -1;
...
> +	}
> +
> +	if (port >=3D PORT_MAJOR)
> +		subport =3D port - PORT_MAJOR;
> +
> +	if (subport < 0) {
> +		val =3D readl(info->ddr_reg[0] + rp[channel]);
> +		val |=3D  (1 << port);
> +		writel(val, info->ddr_reg[0] + rp[channel]);
> +		val =3D 0xffff;
> +		writel(val, info->ddr_reg[0] + rs[channel]);
> +	} else {
> +		val =3D BIT(23);		/* select device */
> +		writel(val, info->ddr_reg[0] + rp[channel]);
> +		val =3D readl(info->ddr_reg[0] + rs[channel]);
> +		val |=3D (1 << subport);
> +		writel(val, info->ddr_reg[0] + rs[channel]);
> +	}
> +}
...
> +	unsigned int reg;
> +
> +	counter->all_cnt =3D readl(info->ddr_reg[0] + =
DMC_MON_G12_ALL_GRANT_CNT);
> +	counter->all_req   =3D readl(info->ddr_reg[0] + =
DMC_MON_G12_ALL_REQ_CNT);
> +
> +	for (i =3D 0; i < info->chann_nr; i++) {
> +		reg =3D DMC_MON_G12_ONE_GRANT_CNT + (i << 2);
> +		counter->channel_cnt[i] =3D readl(info->ddr_reg[0] + =
reg);
> +	}
> +}
> +
...
> +}
> +
> +static struct dmc_pmu_hw_ops g12_ops =3D {
> +	.enable		=3D dmc_g12_counter_enable,
> +	.disable	=3D dmc_g12_counter_disable,
> +	.irq_handler	=3D dmc_g12_irq_handler,
> +	.get_counters	=3D dmc_g12_get_counters,
> +	.config_axi_id	=3D dmc_g12_config_axi_id,
> +};
> +
...
> +		format_attr_nna.attr.mode =3D 0;
> +		format_attr_gdc.attr.mode =3D 0;
> +		format_attr_arm1.attr.mode =3D 0;
> +		format_attr_mipi_isp.attr.mode =3D 0;
...
> +	}
> +
> +	pmu =3D devm_kzalloc(&pdev->dev, sizeof(struct ddr_pmu), =
GFP_KERNEL);
> +	if (!pmu)
> +		return -ENOMEM;
> +
...
> +	 */
> +	pmu->info.dmc_nr =3D 1;
> +	pmu->info.chann_nr =3D 4;
> +	pmu->info.ops =3D &g12_ops;
> +	pmu->info.fmt_attr =3D g12_pmu_format_attrs;
> +
...
> +}
> +
> +static const struct of_device_id aml_ddr_pmu_dt_match[] =3D {
> +	{
> +		.compatible =3D "amlogic,g12-ddr-pmu",
> +	},
> +	{
> +		.compatible =3D "amlogic,g12a-ddr-pmu",
> +	},
> +	{
> +		.compatible =3D "amlogic,g12b-ddr-pmu",
> +	},
> +	{
> +		.compatible =3D "amlogic,sm1-ddr-pmu",
> +	},
> +	{}
> +};
> +
> +static struct platform_driver g12_ddr_pmu_driver =3D {
> +	.driver =3D {
> +		.name =3D "amlogic,ddr-pmu",
> +		.of_match_table =3D aml_ddr_pmu_dt_match,
> +	},
> +	.remove =3D __exit_p(g12_ddr_pmu_remove),
> +};
> +
...
*/
> +	int chann_nr;			/* The number of dmc bandwidth =
monitor channels */
> +	int id;				/* The number of supported =
channels */
