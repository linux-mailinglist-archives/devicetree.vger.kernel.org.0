...
Received: from vger.kernel.org (vger.kernel.org [209.132.180.67])
	by mail.lfdr.de (Postfix) with ESMTP id ED9F3E2E47
	for <lists+devicetree@lfdr.de>; Thu, 24 Oct 2019 12:10:11 +0200 (CEST)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S2406004AbfJXKKA (ORCPT <rfc822;lists+devicetree@lfdr.de>);
        Thu, 24 Oct 2019 06:10:00 -0400
Received: from lelv0143.ext.ti.com ([198.47.23.248]:55714 "EHLO
        lelv0143.ext.ti.com" rhost-flags-OK-OK-OK-OK) by vger.kernel.org
        with ESMTP id S2390306AbfJXKJ7 (ORCPT
        <rfc822;devicetree@vger.kernel.org>); Thu, 24 Oct 2019 06:09:59 -0400
Received: from fllv0035.itg.ti.com ([10.64.41.0])
        by lelv0143.ext.ti.com (8.15.2/8.15.2) with ESMTP id x9OA9oQ7059948;
        Thu, 24 Oct 2019 05:09:50 -0500
DKIM-Signature: v=1; a=rsa-sha256; c=relaxed/relaxed; d=ti.com;
        s=ti-com-17Q1; t=1571911790;
        bh=9qIRPFqgCYhlyyKGZHrPfmrjlRMHaux9FYJ4bd9B6go=;
        h=From:To:CC:Subject:Date:In-Reply-To:References;
        b=fissE3oLNLwuCUsWO90V6iJz0EUZQVKdFzaZkfgvJZVxjoYwrGREC8s5kz7EHjSPB
         oQ8XDNNeSW/KFA+D3gX1e4DnJC+3/pg2r7uqBvh4sVb0TDCpV67q5Ao3sphaJB/Cmr
         qc9OQOghKU1eNbeFkWZwRVBXPba46qphBe5ljiiw=
Received: from DFLE115.ent.ti.com (dfle115.ent.ti.com [10.64.6.36])
        by fllv0035.itg.ti.com (8.15.2/8.15.2) with ESMTP id x9OA9oCH121873;
        Thu, 24 Oct 2019 05:09:50 -0500
Received: from DFLE104.ent.ti.com (10.64.6.25) by DFLE115.ent.ti.com
 (10.64.6.36) with Microsoft SMTP Server (version=TLS1_2,
 cipher=TLS_ECDHE_RSA_WITH_AES_128_CBC_SHA256_P256) id 15.1.1713.5; Thu, 24
 Oct 2019 05:09:39 -0500
Received: from lelv0326.itg.ti.com (10.180.67.84) by DFLE104.ent.ti.com
 (10.64.6.25) with Microsoft SMTP Server (version=TLS1_2,
 cipher=TLS_ECDHE_RSA_WITH_AES_128_CBC_SHA256_P256) id 15.1.1713.5 via
 Frontend Transport; Thu, 24 Oct 2019 05:09:39 -0500
Received: from localhost (ileax41-snat.itg.ti.com [10.172.224.153])
        by lelv0326.itg.ti.com (8.15.2/8.15.2) with ESMTP id x9OA9mYK043793;
        Thu, 24 Oct 2019 05:09:49 -0500
...
Subject: [PATCH v5 net-next 07/12] net: ethernet: ti: introduce cpsw switchdev based driver part 2 - switch
Date:   Thu, 24 Oct 2019 13:09:09 +0300
Message-ID: <20191024100914.16840-8-grygorii.strashko@ti.com>
...
index 748c65e1e66c..f3606ccc747f 100644
...
@@ -353,6 +363,7 @@ static void cpsw_rx_handler(void *token, int len, int status)
 		goto requeue;
 	}
...
+	skb->offload_fwd_mark = priv->offload_fwd_mark;
 	...
 	skb_put(skb, len);
 	skb->dev = ndev;
@@ -383,8 +394,8 @@ static void cpsw_rx_handler(void *token, int len, int status)
 	}
 }
...
@@ -429,6 +440,11 @@ static int cpsw_ndo_vlan_rx_add_vid(struct net_device *ndev,
 	struct cpsw_common *cpsw = priv->cpsw;
 	int ret, i;
...
@@ -483,9 +499,36 @@ static void cpsw_restore(struct cpsw_priv *priv)
...
@@ -530,7 +573,10 @@ static void cpsw_init_host_port(struct cpsw_priv *priv)
...
@@ -562,6 +608,41 @@ static void cpsw_port_add_dual_emac_def_ale_entries(struct cpsw_priv *priv,
 			     ALE_PORT_NOLEARN, 1);
 }
...
@@ -674,7 +755,10 @@ static void cpsw_slave_open(struct cpsw_slave *slave, struct cpsw_priv *priv)
...
@@ -742,7 +826,8 @@ static int cpsw_ndo_open(struct net_device *ndev)
 	struct cpsw_common *cpsw = priv->cpsw;
 	int ret;
...
@@ -926,6 +1011,11 @@ static int cpsw_ndo_vlan_rx_kill_vid(struct net_device *ndev,
 	int ret;
 	int i;
...
@@ -1004,6 +1094,17 @@ static int cpsw_ndo_xdp_xmit(struct net_device *ndev, int n,
 	return n - drops;
 }
...
@@ -1023,6 +1124,7 @@ static const struct net_device_ops cpsw_netdev_ops = {
...
@@ -1328,8 +1430,249 @@ static void cpsw_unregister_ports(struct cpsw_common *cpsw)
 	}
 }
...
+
 static int cpsw_dl_ale_ctrl_get(struct devlink *dl, u32 id,
 				struct devlink_param_gset_ctx *ctx)
 {
@@ -1362,6 +1705,10 @@ static int cpsw_dl_ale_ctrl_set(struct devlink *dl, u32 id,
...
+		}
 	...
 	default:
 		return -EOPNOTSUPP;
@@ -1371,6 +1718,11 @@ static int cpsw_dl_ale_ctrl_set(struct devlink *dl, u32 id,
 }
...
+			     NULL),
 	DEVLINK_PARAM_DRIVER(CPSW_DL_PARAM_ALE_BYPASS,
 			     ...
 			     BIT(DEVLINK_PARAM_CMODE_RUNTIME),
@@ -1520,6 +1872,7 @@ static int cpsw_probe(struct platform_device *pdev)
...
@@ -1574,6 +1927,10 @@ static int cpsw_probe(struct platform_device *pdev)
...
+
 	...
 	if (ret)
 		goto clean_unregister_netdev;
@@ -1611,6 +1968,7 @@ static int cpsw_remove(struct platform_device *pdev)
 		return ret;
 	}
...
index 083ebb1af420..20f2a3b377cf 100644
