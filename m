...
Received: from vger.kernel.org (vger.kernel.org [23.128.96.18])
	by mail.lfdr.de (Postfix) with ESMTP id 70CD144AB42
	for <lists+devicetree@lfdr.de>; Tue,  9 Nov 2021 11:15:45 +0100 (CET)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S237584AbhKIKS3 (ORCPT <rfc822;lists+devicetree@lfdr.de>);
        Tue, 9 Nov 2021 05:18:29 -0500
Received: from mxout70.expurgate.net ([194.37.255.70]:50389 "EHLO
        mxout70.expurgate.net" rhost-flags-OK-OK-OK-OK) by vger.kernel.org
        with ESMTP id S230123AbhKIKS1 (ORCPT
        <rfc822;devicetree@vger.kernel.org>); Tue, 9 Nov 2021 05:18:27 -0500
X-Greylist: delayed 422 seconds by postgrey-1.27 at vger.kernel.org; Tue, 09 Nov 2021 05:18:26 EST
Received: from [127.0.0.1] (helo=localhost)
        by relay.expurgate.net with smtp (Exim 4.92)
        (envelope-from <fe@dev.tdt.de>)
        id 1mkO3U-0004MW-QT; Tue, 09 Nov 2021 11:08:40 +0100
Received: from [195.243.126.94] (helo=securemail.tdt.de)
        by relay.expurgate.net with esmtps (TLS1.2:ECDHE_RSA_AES_256_GCM_SHA384:256)
        (Exim 4.92)
        (envelope-from <fe@dev.tdt.de>)
        id 1mkO3U-0007Xo-1B; Tue, 09 Nov 2021 11:08:40 +0100
Received: from securemail.tdt.de (localhost [127.0.0.1])
        by securemail.tdt.de (Postfix) with ESMTP id 2063B240041;
        Tue,  9 Nov 2021 11:08:39 +0100 (CET)
Received: from mail.dev.tdt.de (unknown [10.2.4.42])
        by securemail.tdt.de (Postfix) with ESMTP id 9F327240040;
        Tue,  9 Nov 2021 11:08:38 +0100 (CET)
Received: from localhost.localdomain (unknown [10.2.3.40])
        by mail.dev.tdt.de (Postfix) with ESMTPSA id 26C7220176;
        Tue,  9 Nov 2021 11:08:37 +0100 (CET)
...
Subject: [PATCH 1/2] leds: ktd20xx: Add the KTD20xx family of the RGB LEDs driver from Kinetic
Date:   Tue,  9 Nov 2021 11:08:21 +0100
Message-ID: <20211109100822.5412-2-fe@dev.tdt.de>
...
X-purgate-ID: 151534::1636452520-00002D2E-17E510C2/0/0
Precedence: bulk
...
color registers 0 must be preset with 0x00 (Red=3D0x00 Green=3D0x00 Blue=3D=
0x00).
...
+
+	if (fwnode_property_read_u8_array(fwnode, "kinetic,color-current0", val=
ue, 3)) {
+		dev_warn(dev, "no kinetic,color-current0 property, use chip default va=
lue 0x28.\n");
+	} else {
...
+	}
+
+	if (fwnode_property_read_u8_array(fwnode, "kinetic,color-current1", val=
ue, 3)) {
+		dev_warn(dev, "no kinetic,color-current1 property, use chip default va=
lue 0x60.\n");
+	} else {
...
+		if (fwnode_property_read_u8_array(child, "kinetic,color-selection", va=
lue, 3))
+			dev_warn(dev, "no kinetic,color-selection property found, use default=
 rgbt color selection from register 0.\n");
+
+		led->subled_info[0].color_index =3D LED_COLOR_ID_RED;
+		led->subled_info[0].channel =3D 0;
+		led->subled_info[0].intensity =3D value[0];
+		led->subled_info[1].color_index =3D LED_COLOR_ID_GREEN;
+		led->subled_info[1].channel =3D 1;
+		led->subled_info[1].intensity =3D value[1];
+		led->subled_info[2].color_index =3D LED_COLOR_ID_BLUE;
+		led->subled_info[2].channel =3D 2;
+		led->subled_info[2].intensity =3D value[2];
+
...
+ * To set a new RGB value, 3 values must be passed. This value may not b=
e
+ * less than 0 and also not greater than 194. The chip can only process =
the
...
+	len +=3D sprintf(buf + len, "%d", value);
...
+	ssize_t ret;
+
+	ret =3D sscanf(buf, "%u %u %u", &value[0], &value[1], &value[2]);
+	if (ret < 3) {
+		ret =3D -EINVAL;
+		goto err_out;
+	}
...
+}
+
+static ssize_t faderate_store(struct device *dev, struct device_attribut=
e *a,
+		const char *buf, size_t count)
+{
+	struct i2c_client *client =3D to_i2c_client(dev);
