...
Received: from out1.vger.email (out1.vger.email [IPv6:2620:137:e000::1:20])
	by mail.lfdr.de (Postfix) with ESMTP id B62B75B586E
	for <lists+devicetree@lfdr.de>; Mon, 12 Sep 2022 12:29:44 +0200 (CEST)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S229527AbiILK2l (ORCPT <rfc822;lists+devicetree@lfdr.de>);
        Mon, 12 Sep 2022 06:28:41 -0400
Received: from lindbergh.monkeyblade.net ([23.128.96.19]:49860 "EHLO
        lindbergh.monkeyblade.net" rhost-flags-OK-OK-OK-OK) by vger.kernel.org
        with ESMTP id S230162AbiILK2k (ORCPT
        <rfc822;devicetree@vger.kernel.org>); Mon, 12 Sep 2022 06:28:40 -0400
Received: from madras.collabora.co.uk (madras.collabora.co.uk [46.235.227.172])
        by lindbergh.monkeyblade.net (Postfix) with ESMTPS id 166AF1B7B0;
...
Received: from mercury (unknown [185.122.133.20])
        ...
        by madras.collabora.co.uk (Postfix) with ESMTPSA id BC86C6601FDB;
        Mon, 12 Sep 2022 11:28:34 +0100 (BST)
DKIM-Signature: v=1; a=rsa-sha256; c=relaxed/simple; d=collabora.com;
        s=mail; t=1662978514;
        bh=mCQSK0s6vzQCvM2HKZv1+cvttCDGFozFzPPMDYSd7AA=;
        h=Date:From:To:Cc:Subject:References:In-Reply-To:From;
        b=
Received: by mercury (Postfix, from userid 1000)
        id 0B0EA1063348; Sun, 11 Sep 2022 14:16:07 +0200 (CEST)
Date:   Sun, 11 Sep 2022 14:16:07 +0200
From:   Sebastian Reichel <sebastian.reichel@collabora.com>
To:     lee@kernel.org, Chris Morgan <macroalpha82@gmail.com>
...
Subject: Re: [PATCH V10 3/4] power: supply: Add charger driver for Rockchip
 RK817
Message-ID: <20220911121607.q2yah7msv7argxts@mercury.elektranox.org>
References: <20220827021623.23829-1-macroalpha82@gmail.com>
 <20220827021623.23829-4-macroalpha82@gmail.com>
MIME-Version: 1.0
Content-Type: multipart/signed; micalg=pgp-sha512;
        protocol="application/pgp-signature"; boundary="7lhanzew6atwvykv"
...
> @@ -91,6 +91,7 @@ obj-$(CONFIG_CHARGER_BQ2515X)	+=3D bq2515x_charger.o
...
> +		return CHG_3_5A;
> +	else if (chg_cur_ma >=3D 3000)
> +		return CHG_3A;
> +	else if (chg_cur_ma >=3D 2750)
> +		return CHG_2_75A;
> +	else if (chg_cur_ma >=3D 2500)
> +		return CHG_2_5A;
> +	else if (chg_cur_ma >=3D 2000)
> +		return CHG_2A;
> +	else if (chg_cur_ma >=3D 1500)
> +		return CHG_1_5A;
> +	else if (chg_cur_ma >=3D 1000)
> +		return CHG_1A;
> +	else if (chg_cur_ma >=3D 500)
> +		return CHG_0_5A;
> +	else
> +		return -EINVAL;
> +}
...
> +			 bulk_reg, 2);
> +	vcalib0 =3D get_unaligned_be16(bulk_reg);
> +
> +	regmap_bulk_read(charger->rk808->regmap, RK817_GAS_GAUGE_VCALIB1_H,
> +			 bulk_reg, 2);
> +	vcalib1 =3D get_unaligned_be16(bulk_reg);
> +
...
> +
> +/*
> + * note that only the fcc_mah is really used by this driver, the other v=
alues
> + * are to ensure we can remain backwards compatible with the BSP kernel.
> + */
> +static int rk817_record_battery_nvram_values(struct rk817_charger *charg=
er)
> +{
> +	u8 bulk_reg[3];
> +	int ret, rsoc;
...
> +	ret =3D regmap_bulk_write(charger->rk808->regmap, RK817_GAS_GAUGE_BAT_R=
1,
> +				bulk_reg, 3);
> +	if (ret < 0)
> +		return ret;
...
> +	 */
> +	rsoc =3D (charger->soc * charger->fcc_mah) / 100000;
> +	put_unaligned_le24(rsoc, bulk_reg);
> +	ret =3D regmap_bulk_write(charger->rk808->regmap, RK817_GAS_GAUGE_DATA0,
> +				bulk_reg, 3);
> +	if (ret < 0)
> +		return ret;
> +	/* write the fcc_mah in mAh, just as the BSP kernel does. */
> +	put_unaligned_le24(charger->fcc_mah, bulk_reg);
> +	ret =3D regmap_bulk_write(charger->rk808->regmap, RK817_GAS_GAUGE_DATA3,
> +				bulk_reg, 3);
> +	if (ret < 0)
> +		return ret;
> +
...
> +
> +	if (charger->charge_status =3D=3D CHARGE_FINISH && (!charger->soc_cal))=
 {
> +		/*
...
> +		 */
> +
> +		charger->soc =3D 100000;
> +		charge_now_adc =3D CHARGE_TO_ADC(charger->fcc_mah,
> +					       charger->res_div);
> +		put_unaligned_be32(charge_now_adc, bulk_reg);
> +		regmap_bulk_write(rk808->regmap, RK817_GAS_GAUGE_Q_INIT_H3,
> +				  bulk_reg, 4);
> +
> +		charger->soc_cal =3D 1;
> +		dev_dbg(charger->dev,
...
> +		charge_now =3D ADC_TO_CHARGE_UAH(charge_now_adc,
> +					       charger->res_div);
> +
> +		/*
...
> +	 * battery (soc_cal =3D 1) and are now empty (at or below minimum design
...
> +			 bulk_reg, 2);
> +	tmp =3D get_unaligned_be16(bulk_reg);
> +	volt_avg =3D (charger->voltage_k * tmp) + 1000 * charger->voltage_b;
> +	if (volt_avg <=3D charger->bat_voltage_min_design_uv &&
> +	    charger->soc_cal) {
> +		regmap_bulk_read(rk808->regmap, RK817_GAS_GAUGE_Q_PRES_H3,
> +				 bulk_reg, 4);
> +		charge_now_adc =3D get_unaligned_be32(bulk_reg);
...
> +		 * we want).
> +		 */
> +		charger->fcc_mah =3D charger->fcc_mah - (charge_now / 1000);
> +
> +		dev_dbg(charger->dev,
...
> +	charger->charge_now_uah =3D ADC_TO_CHARGE_UAH(tmp, charger->res_div);
> +	if (charger->charge_now_uah < 0)
> +		charger->charge_now_uah =3D 0;
> +	if (charger->charge_now_uah > charger->fcc_mah * 1000)
> +		charger->charge_now_uah =3D charger->fcc_mah * 1000;
> +
> +	/* Update soc based on reported charge. */
> +	charger->soc =3D charger->charge_now_uah * 100 / charger->fcc_mah;
> +
> +	/* Update reported voltage. */
> +	regmap_bulk_read(charger->rk808->regmap, RK817_GAS_GAUGE_BAT_VOL_H,
> +			 bulk_reg, 2);
> +	tmp =3D get_unaligned_be16(bulk_reg);
> +	charger->volt_avg_uv =3D (charger->voltage_k * tmp) + 1000 *
> +				charger->voltage_b;
> +
> +	/*
...
> +			 bulk_reg, 2);
> +	tmp =3D (short int)get_unaligned_be16(bulk_reg);
> +	charger->cur_avg_ua =3D ADC_TO_CURRENT(tmp, charger->res_div);
> +
> +	/*
...
> +	 */
> +	regmap_read(charger->rk808->regmap, RK817_PMIC_CHRG_OUT, &reg);
> +	charger->max_chg_volt_uv =3D ((((reg & RK817_CHRG_VOL_SEL) >> 4) *
> +				    50000) + 4100000);
> +
> +	/* Check if battery still present. */
> +	regmap_read(charger->rk808->regmap, RK817_PMIC_CHRG_STS, &reg);
> +	charger->battery_present =3D (reg & RK817_BAT_EXS);
> +
> +	/* Get which type of charge we are using (if any). */
> +	regmap_read(charger->rk808->regmap, RK817_PMIC_CHRG_STS, &reg);
> +	charger->charge_status =3D (reg >> 4) & 0x07;
> +
> +	/*
...
> +			 bulk_reg, 2);
> +	reg =3D get_unaligned_be16(bulk_reg);
> +	if (reg > 1) {
> +		tmp =3D ((charger->voltage_k * reg / 1000 + charger->voltage_b) *
> +		       60 / 46);
> +		charger->charger_input_volt_avg_uv =3D tmp * 1000;
> +	} else {
> +		charger->charger_input_volt_avg_uv =3D 0;
> +	}
> +
...
> +		val->intval =3D charger->battery_present;
> +		break;
...
> +		val->intval =3D charger->fcc_mah * 1000;
> +		break;
> +	case POWER_SUPPLY_PROP_CHARGE_FULL_DESIGN:
> +		val->intval =3D charger->bat_charge_full_design_uah;
> +		break;
> +	case POWER_SUPPLY_PROP_CHARGE_EMPTY_DESIGN:
> +		val->intval =3D 0;
> +		break;
> +	case POWER_SUPPLY_PROP_CHARGE_NOW:
> +		val->intval =3D charger->charge_now_uah;
> +		break;
> +	case POWER_SUPPLY_PROP_VOLTAGE_MIN_DESIGN:
> +		val->intval =3D charger->bat_voltage_min_design_uv;
> +		break;
> +	case POWER_SUPPLY_PROP_CAPACITY:
> +		/* Add 500 so that values like 99999 are 100% not 99%. */
> +		val->intval =3D (charger->soc + 500) / 1000;
> +		if (val->intval > 100)
> +			val->intval =3D 100;
> +		if (val->intval < 0)
> +			val->intval =3D 0;
...
> +		val->intval =3D charger->volt_avg_uv;
> +		break;
> +	case POWER_SUPPLY_PROP_CURRENT_AVG:
> +		val->intval =3D charger->cur_avg_ua;
> +		break;
> +	case POWER_SUPPLY_PROP_CONSTANT_CHARGE_CURRENT_MAX:
> +		val->intval =3D charger->max_chg_cur_ua;
> +		break;
> +	case POWER_SUPPLY_PROP_CONSTANT_CHARGE_VOLTAGE_MAX:
> +		val->intval =3D charger->max_chg_volt_uv;
> +		break;
> +	case POWER_SUPPLY_PROP_VOLTAGE_MAX_DESIGN:
> +		val->intval =3D charger->bat_voltage_max_design_uv;
> +		break;
> +	default:
> +		return -EINVAL;
> +	}
...
> +		break;
> +	case POWER_SUPPLY_PROP_VOLTAGE_AVG:
> +		val->intval =3D charger->charger_input_volt_avg_uv;
> +		break;
> +	/*
...
> +};
> +
> +static enum power_supply_usb_type rk817_usb_type[] =3D {
...
> +	.usb_types =3D rk817_usb_type,
> +	.num_usb_types =3D ARRAY_SIZE(rk817_usb_type),
...
> +	ret =3D regmap_bulk_read(charger->rk808->regmap,
> +			       RK817_GAS_GAUGE_DATA3, bulk_reg, 3);
> +	if (ret < 0)
> +		return ret;
> +	charger->fcc_mah =3D get_unaligned_le24(bulk_reg);
> +
> +	/*
...
> +		charger->fcc_mah =3D charger->bat_charge_full_design_uah / 1000;
> +	}
> +
> +	/*
...
> +	if (ret < 0)
> +		return ret;
> +	charger->soc =3D get_unaligned_le24(bulk_reg);
> +	if (charger->soc > 10000)
> +		charger->soc =3D 10000;
> +
> +	return 0;
> +}
> +
...
> +	 * needs to be set up.
> +	 */
> +	ret =3D regmap_read(rk808->regmap, RK817_GAS_GAUGE_GG_STS, &reg);
> +	if (ret < 0)
> +		return ret;
> +	first_boot =3D reg & RK817_BAT_CON;
> +	/*
...
> +				 bulk_reg, 2);
> +		tmp =3D get_unaligned_be16(bulk_reg);
> +		boot_voltage =3D (charger->voltage_k * tmp) +
...
> +		 * table.
> +		 */
> +		charger->soc =3D power_supply_batinfo_ocv2cap(bat_info,
> +							    boot_voltage,
> +							    20) * 1000;
> +		if (charger->soc < 0)
> +			charger->soc =3D 0;
...
> +				  RK817_BAT_CON, 0);
> +		/* Save nvram values */
> +		ret =3D rk817_record_battery_nvram_values(charger);
> +		if (ret < 0)
> +			return ret;
> +	} else {
> +		ret =3D rk817_read_battery_nvram_values(charger);
> +		if (ret < 0)
> +			return ret;
> +
> +		regmap_bulk_read(rk808->regmap, RK817_GAS_GAUGE_Q_PRES_H3,
...
> +							     20) * 1000;
> +		} else {
> +			charger->soc =3D (boot_charge_mah * 1000 * 100 /
> +					charger->fcc_mah);
> +		}
> +	}
> +
...
> +			 bulk_reg, 2);
> +	tmp =3D get_unaligned_be16(bulk_reg);
> +	boot_voltage =3D (charger->voltage_k * tmp) + 1000 * charger->voltage_b;
> +	regmap_bulk_read(rk808->regmap, RK817_GAS_GAUGE_Q_PRES_H3,
> +			 bulk_reg, 4);
> +	tmp =3D get_unaligned_be32(bulk_reg);
> +	if (tmp < 0)
...
> +	 * counter.
> +	 */
> +	boot_charge_mah =3D charger->soc * charger->fcc_mah / 100 / 1000;
> +	if (boot_charge_mah > charger->fcc_mah)
> +		boot_charge_mah =3D charger->fcc_mah;
> +	tmp =3D CHARGE_TO_ADC(boot_charge_mah, charger->res_div);
> +	put_unaligned_be32(tmp, bulk_reg);
> +	ret =3D regmap_bulk_write(rk808->regmap, RK817_GAS_GAUGE_Q_INIT_H3,
> +			  bulk_reg, 4);
> +	if (ret < 0)
> +		return ret;
> +
> +	/* Set QMAX value to max design capacity. */
> +	tmp =3D CHARGE_TO_ADC((charger->bat_charge_full_design_uah / 1000),
> +			    charger->res_div);
> +	put_unaligned_be32(tmp, bulk_reg);
...
> +	tmp =3D CURRENT_TO_ADC(charger->sleep_enter_current_ua,
> +			     charger->res_div);
...
> +	tmp =3D CURRENT_TO_ADC(charger->sleep_filter_current_ua,
> +			     charger->res_div);
> +	put_unaligned_be16(tmp, bulk_reg);
...
> +	 */
> +	max_chg_vol_mv =3D bat_info->constant_charge_voltage_max_uv / 1000;
> +	max_chg_cur_ma =3D bat_info->constant_charge_current_max_ua / 1000;
> +
...
> +	 * default.
> +	 */
> +	chg_term_ma =3D bat_info->charge_term_current_ua / 1000;
...
> +
> +	ret =3D rk817_read_or_set_full_charge_on_boot(charger, bat_info);
> +	if (ret < 0)
> +		return ret;
> +
> +	/*
...
> +	int ret;
> +
> +	node =3D of_get_child_by_name(dev->parent->of_node, "charger");
> +	if (!node)
> +		return -ENODEV;
> +
...
> +	 */
> +	ret =3D of_property_read_u32(node,
...
> +	    (bat_info->voltage_min_design_uv <=3D 0) ||
> +	    (bat_info->voltage_max_design_uv <=3D 0) ||
...
h;
> +	charger->bat_voltage_min_design_uv =3D bat_info->voltage_min_design_uv;
> +	charger->bat_voltage_max_design_uv =3D bat_info->voltage_max_design_uv;
> +
> +	/*
...

=WEWY
-----END PGP SIGNATURE-----
