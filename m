...
Received: from vger.kernel.org (vger.kernel.org [23.128.96.18])
	by mail.lfdr.de (Postfix) with ESMTP id 180DC2B08FC
	for <lists+devicetree@lfdr.de>; Thu, 12 Nov 2020 16:53:25 +0100 (CET)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S1728433AbgKLPxY (ORCPT <rfc822;lists+devicetree@lfdr.de>);
        Thu, 12 Nov 2020 10:53:24 -0500
Received: from relay10.mail.gandi.net ([217.70.178.230]:44939 "EHLO
        relay10.mail.gandi.net" rhost-flags-OK-OK-OK-OK) by vger.kernel.org
        with ESMTP id S1728741AbgKLPxX (ORCPT
        <rfc822;devicetree@vger.kernel.org>); Thu, 12 Nov 2020 10:53:23 -0500
Received: from aptenodytes (196.109.29.93.rev.sfr.net [93.29.109.196])
        (Authenticated sender: paul.kocialkowski@bootlin.com)
        by relay10.mail.gandi.net (Postfix) with ESMTPSA id 79F9F240016;
        Thu, 12 Nov 2020 15:53:06 +0000 (UTC)
Date:   Thu, 12 Nov 2020 16:53:05 +0100
...
Subject: Re: [PATCH 2/2] media: i2c: Add support for the OV5648 image sensor
Message-ID: <20201112155305.GA1908425@aptenodytes>
References: <20201023174944.504358-1-paul.kocialkowski@bootlin.com>
 <20201023174944.504358-3-paul.kocialkowski@bootlin.com>
 <20201030165223.GT26150@paasikivi.fi.intel.com>
MIME-Version: 1.0
Content-Type: multipart/signed; micalg=pgp-sha256;
        protocol="application/pgp-signature"; boundary="+QahgC5+KEYLbs62"
Content-Disposition: inline
In-Reply-To: <20201030165223.GT26150@paasikivi.fi.intel.com>
Precedence: bulk
...
On Fri 30 Oct 20, 18:52, Sakari Ailus wrote:
> Thank you for the patchset.

And thanks for the review!

> It's nice to see a driver that is not based on register lists --- most ar=
e.
>=20
...
Yes that's correct, I'll fix the commit message.
...
Understood, I'll remove the ones that are not needed.
...
This is necessary because v4l2_ctrl_auto_cluster takes a pointer to an array
of struct v4l2_ctrl pointers. Here we don't actually have an array but foll=
owing
elements in a structure, so we need to make sure that there is no gap betwe=
en
elements for the two to be equivalent. Note that I've also put the handler
structure at the end to still have a good chance that things remain word-al=
igned
so there's no actual overhead due to unaligned access.

Thoguh I'll remove it from the OV8865 driver since we're not using
v4l2_ctrl_auto_cluster there.
...
> > +};
> > +
> > +static const struct ov5648_mode ov5648_mode_qsxga =3D {
> > +	/* Horizontal */
> > +	.crop_start_x	=3D 16,
> > +	.offset_x	=3D 0,
...
> > +};
> > +
> > +static const struct ov5648_mode ov5648_mode_uxga =3D {
> > +	/* Horizontal */
> > +	.crop_start_x	=3D 512,
> > +	.offset_x	=3D 0,
...
> > +};
> > +
> > +static const struct ov5648_mode ov5648_mode_1080p =3D {
> > +	/* Horizontal */
> > +	.crop_start_x	=3D 352,
> > +	.offset_x	=3D 0,
...
> > +};
> > +
> > +static const struct ov5648_mode ov5648_mode_1280x960 =3D {
> > +	/* From gf ref */
> > +	/* Horizontal */
> > +	.crop_start_x	=3D 16,
> > +	.offset_x	=3D 8,
...
> > +};
> > +
> > +static const struct ov5648_mode ov5648_mode_720p =3D {
> > +	/* From gf ref */
> > +	/* Horizontal */
> > +	.crop_start_x	=3D 16,
> > +	.offset_x	=3D 8,
...
> > +};
> > +
> > +static const struct ov5648_mode ov5648_mode_vga =3D {
> > +	/* From gf ref */
> > +	/* Horizontal */
> > +	.crop_start_x	=3D 0,
> > +	.offset_x	=3D 8,
...
> > +	int ret;
> > +
> > +	ret =3D ov5648_write(sensor, OV5648_A_PWC_PK_O0_REG,
...
> > +	/* Disable FREX pin. */
> > +
> > +	ret =3D ov5648_write(sensor, OV5648_PAD_PK_REG,
...
> > +		return ret;
> > +
> > +	ret =3D ov5648_write(sensor, OV5648_MIPI_SC_CTRL0_REG,
...
> > +		return ret;
> > +
> > +	ret =3D ov5648_update_bits(sensor, OV5648_BLC_CTRL5_REG,
...
> > +		return ret;
> > +
> > +	ret =3D ov5648_write(sensor, OV5648_MANUAL_CTRL_REG,
...
> > +		return ret;
> > +
> > +	ret =3D ov5648_write(sensor, OV5648_SRB_CTRL_REG,
...
> > +		return ret;
> > +
> > +	ret =3D ov5648_write(sensor, OV5648_PLLS_CTRL_REG,
...
> > +	mipi_clk_rate =3D pll1_rate / config->sys_div / config->mipi_div / 2;
>=20
...
> > +	int ret;
> > +
> > +	ret =3D ov5648_update_bits(sensor, OV5648_MANUAL_CTRL_REG,
>=20
...
Mhh to be honest I don't really like the idea. The point of these functions
is to provide some abstraction between generic values (enable boolean) and =
the
registrer/bit values. Splitting the latter in two places (register name here
and bit value in the caller) makes things more difficult to understand and
defeats the purpose of these abstraction functions altogether (why not dire=
ctly
call ov5648_update_bits instead of this function then)?
...
> > +		return ret;
> > +
> > +	ret =3D ov5648_write(sensor, OV5648_EXPOSURE_CTRL_L_REG,
...
> > +		return ret;
> > +
> > +	ret =3D ov5648_write(sensor, OV5648_GAIN_CTRL_L_REG,
...
> > +	int ret;
> > +
> > +	ret =3D ov5648_write(sensor, OV5648_AWB_CTRL_REG,
...
> > +	int ret;
> > +
> > +	ret =3D ov5648_update_bits(sensor, OV5648_TC20_REG, bits,
> > +				 enable ? bits : 0);
> > +	if (ret)
> > +		return ret;
> > +
...
> > +	int ret;
> > +
> > +	ret =3D ov5648_update_bits(sensor, OV5648_TC21_REG, bits,
> > +				 enable ? bits : 0);
> > +	if (ret)
> > +		return ret;
> > +
...
> > +	int ret;
> > +
> > +	ret =3D ov5648_write(sensor, OV5648_ISP_CTRL3D_REG, bits);
> > +	if (ret)
> > +		return ret;
> > +
> > +	return 0;
...
> > +		bits =3D ov5648_test_pattern_bits[ctrl->val];
...
> > +	/* Gain */
> > +
> > +	ctrls->gain_auto =3D v4l2_ctrl_new_std(handler, ops, V4L2_CID_AUTOGAI=
N,
> > +					     0, 1, 1, 1);
> > +
...
> > +	/* White Balance */
> > +
> > +	ctrls->white_balance_auto =3D v4l2_ctrl_new_std(handler, ops,
...
> > +	/* Flip */
> > +
> > +	ctrls->flip_horz =3D v4l2_ctrl_new_std(handler, ops, V4L2_CID_HFLIP, =
0, 1,
> > +					     1, 0);
> > +	ctrls->flip_vert =3D v4l2_ctrl_new_std(handler, ops, V4L2_CID_VFLIP, =
0, 1,
> > +					     1, 0);
> > +
> > +	/* Test Pattern */
...
> > +
> > +	ctrls->pixel_rate =3D v4l2_ctrl_new_std(handler, ops, V4L2_CID_PIXEL_=
RATE,
> > +					      1, INT_MAX, 1, 1);
> > +
...
> > +	struct ov5648_sensor *sensor =3D ov5648_subdev_sensor(subdev);
> > +	struct ov5648_state *state =3D &sensor->state;
> > +	bool ctrl_handler_setup =3D false;
> > +	int ret =3D 0;
> > +
> > +	mutex_lock(&sensor->mutex);
> > +
> > +	state->power_count +=3D on ? 1 : -1;
> > +
> > +	/* Neither first power on nor last power off. */
> > +	if (state->power_count !=3D !!on)
> > +		goto complete;
> > +
> > +	ret =3D ov5648_sensor_power(sensor, on);
> > +	if (ret)
> > +		goto complete;
> > +
...
> > +}
> > +
> > +static const struct v4l2_subdev_core_ops ov5648_subdev_core_ops =3D {
> > +	.s_power		=3D ov5648_s_power,
> > +};
> > +
...
> > +		goto complete;
> > +
> > +	sensor->state.streaming =3D !!enable;
> > +
> > +complete:
> > +	mutex_unlock(&sensor->mutex);
> > +
...
> > +	struct ov5648_sensor *sensor =3D ov5648_subdev_sensor(subdev);
> > +	struct v4l2_mbus_framefmt *mbus_format =3D &format->format;
> > +	const struct ov5648_mode *mode =3D ov5648_modes[0];
> > +	u32 mbus_code =3D ov5648_mbus_codes[0];
> > +	unsigned int index;
> > +	int ret =3D 0;
> > +
> > +	mutex_lock(&sensor->mutex);
...
> > +	for (index =3D 0; index < ARRAY_SIZE(ov5648_modes); index++) {
> > +		if (ov5648_modes[index]->output_size_x =3D=3D mbus_format->width &&
> > +		    ov5648_modes[index]->output_size_y =3D=3D mbus_format->height) {
>=20
...
> > +
> > +	if (format->which =3D=3D V4L2_SUBDEV_FORMAT_TRY) {
> > +		*v4l2_subdev_get_try_format(subdev, cfg, format->pad) =3D
> > +			*mbus_format;
...
> > +	.core		=3D &ov5648_subdev_core_ops,
...
> > +
> > +	ret =3D v4l2_fwnode_endpoint_parse(handle, &sensor->endpoint);
...
> > +	/* External Clock */
> > +
> > +	sensor->xvclk =3D devm_clk_get(dev, "xvclk");
> > +	if (IS_ERR(sensor->xvclk)) {
> > +		dev_err(dev, "failed to get xvclk external clock\n");
> > +		return PTR_ERR(sensor->xvclk);
> > +	}
> > +
> > +	ret =3D clk_set_rate_exclusive(sensor->xvclk, OV5648_XVCLK_RATE);
>=20
...
> compare that against what the driver can do, and fail if the driver doesn=
't
...
> > +}
> > +
> > +static const struct i2c_device_id ov5648_id[] =3D {
> > +	{ "ov5648", 0 },
>=20
> Please don't add I=C2=B2C ID table unless you really need it. The OF matc=
h table
...

=Wznb
-----END PGP SIGNATURE-----
