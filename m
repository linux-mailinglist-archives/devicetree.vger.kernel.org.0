...
Received: from vger.kernel.org (vger.kernel.org [23.128.96.18])
	by mail.lfdr.de (Postfix) with ESMTP id 1355222F301
	for <lists+devicetree@lfdr.de>; Mon, 27 Jul 2020 16:50:09 +0200 (CEST)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S1728613AbgG0OuI (ORCPT <rfc822;lists+devicetree@lfdr.de>);
        Mon, 27 Jul 2020 10:50:08 -0400
Received: from mailout02.rmx.de ([62.245.148.41]:47791 "EHLO mailout02.rmx.de"
        rhost-flags-OK-OK-OK-OK) by vger.kernel.org with ESMTP
        id S1727032AbgG0OuH (ORCPT <rfc822;devicetree@vger.kernel.org>);
        Mon, 27 Jul 2020 10:50:07 -0400
Received: from kdin02.retarus.com (kdin02.dmz1.retloc [172.19.17.49])
        (using TLSv1.2 with cipher AECDH-AES256-SHA (256/256 bits))
        (No client certificate requested)
        by mailout02.rmx.de (Postfix) with ESMTPS id 4BFjQH2lLyzNr8l;
        Mon, 27 Jul 2020 16:49:59 +0200 (CEST)
Received: from mta.arri.de (unknown [217.111.95.66])
        (using TLSv1.2 with cipher ECDHE-RSA-AES256-SHA384 (256/256 bits))
        (No client certificate requested)
        by kdin02.retarus.com (Postfix) with ESMTPS id 4BFjPX4LHxz2TTLx;
        Mon, 27 Jul 2020 16:49:20 +0200 (CEST)
...
Subject: [PATCH 2/2] iio: light: as73211: New driver
Date:   Mon, 27 Jul 2020 16:47:41 +0200
Message-ID: <20200727144741.3927-3-ceggers@arri.de>
...
X-RMX-ID: 20200727-164924-4BFjPX4LHxz2TTLx-0@kdin02
...
index 000000000000..93f392f1bea1
--- /dev/null
...
+				.addr = data->client->addr,
+				.flags = I2C_M_RD,
+				.len = 4 * sizeof(*data->buffer),
+				.buf = (u8 *)&data->buffer[0]
+			},
+		};
...
+		data->buffer[0] = le16_to_cpu(data->buffer[0]);
+
+		if (data_result == 0) {
+			/* convert byte order (AS73211 sends LSB first) */
+			data->buffer[1] = le16_to_cpu(data->buffer[1]);
+			data->buffer[2] = le16_to_cpu(data->buffer[2]);
+			data->buffer[3] = le16_to_cpu(data->buffer[3]);
+		} else {
+			/* saturate all channels (useful for overflows) */
+			data->buffer[1] = 0xffff;
+			data->buffer[2] = 0xffff;
+			data->buffer[3] = 0xffff;
+		}
+
+	}
...
+				(char *)&data->buffer[1], 3 * sizeof(*data->buffer));
+		if (ret < 0)
+			goto done;
+
+		if (data_result == 0) {
