...
Received: from vger.kernel.org (vger.kernel.org [23.128.96.18])
	by mail.lfdr.de (Postfix) with ESMTP id 9A8FF465E6D
	for <lists+devicetree@lfdr.de>; Thu,  2 Dec 2021 07:54:51 +0100 (CET)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S1345391AbhLBG6J (ORCPT <rfc822;lists+devicetree@lfdr.de>);
        Thu, 2 Dec 2021 01:58:09 -0500
Received: from mailgw02.mediatek.com ([210.61.82.184]:56500 "EHLO
        mailgw02.mediatek.com" rhost-flags-OK-FAIL-OK-FAIL) by vger.kernel.org
        with ESMTP id S230521AbhLBG6I (ORCPT
        <rfc822;devicetree@vger.kernel.org>); Thu, 2 Dec 2021 01:58:08 -0500
X-UUID: 25b7806c119145109409fc4adcc055e7-20211202
X-UUID: 25b7806c119145109409fc4adcc055e7-20211202
Received: from mtkexhb01.mediatek.inc [(172.21.101.102)] by mailgw02.mediatek.com
        (envelope-from <moudy.ho@mediatek.com>)
        (Generic MTA with TLSv1.2 ECDHE-RSA-AES256-SHA384 256/256)
        with ESMTP id 703272339; Thu, 02 Dec 2021 14:54:41 +0800
Received: from mtkcas11.mediatek.inc (172.21.101.40) by
 mtkmbs10n2.mediatek.inc (172.21.101.183) with Microsoft SMTP Server
 (version=TLS1_2, cipher=TLS_ECDHE_RSA_WITH_AES_256_CBC_SHA384) id 15.2.792.3;
 Thu, 2 Dec 2021 14:54:39 +0800
Received: from mtksdccf07 (172.21.84.99) by mtkcas11.mediatek.inc
 (172.21.101.73) with Microsoft SMTP Server id 15.0.1497.2 via Frontend
 Transport; Thu, 2 Dec 2021 14:54:39 +0800
Message-ID: <70969aeb43b3bc8d32cb433e88ebb00527d6c374.camel@mediatek.com>
...
Date:   Thu, 2 Dec 2021 14:54:38 +0800
...
On Thu, 2021-12-02 at 14:27 +0800, Moudy Ho wrote:
...
Depend on:
[1]
https://patchwork.kernel.org/project/linux-mediatek/patch/20211202061322.19917-2-moudy.ho@mediatek.com/
[2]
https://patchwork.kernel.org/project/linux-mediatek/patch/20211202061322.19917-3-moudy.ho@mediatek.com/
[3]
https://patchwork.kernel.org/project/linux-mediatek/patch/20211202061322.19917-4-moudy.ho@mediatek.com/
[4]
https://patchwork.kernel.org/project/linux-mediatek/patch/20211202061322.19917-5-moudy.ho@mediatek.com/

Sorry for missing dependency patches
...
> index a148553babfc..29bdfa92a09a 100644
> --- a/drivers/media/platform/Makefile
> +++ b/drivers/media/platform/Makefile
> @@ -78,6 +78,8 @@ obj-$(CONFIG_VIDEO_MEDIATEK_VCODEC)	+= mtk-vcodec/
>  
...
> diff --git a/drivers/media/platform/mtk-mdp3/Makefile
...
> diff --git a/drivers/media/platform/mtk-mdp3/mdp_reg_ccorr.h
...
> diff --git a/drivers/media/platform/mtk-mdp3/mdp_reg_isp.h
...
> +
> +#endif  // __ISP_REG_H__
> diff --git a/drivers/media/platform/mtk-mdp3/mdp_reg_rdma.h
...
> diff --git a/drivers/media/platform/mtk-mdp3/mdp_reg_rsz.h
...
> diff --git a/drivers/media/platform/mtk-mdp3/mdp_reg_wdma.h
...
> diff --git a/drivers/media/platform/mtk-mdp3/mdp_reg_wrot.h
...
> diff --git a/drivers/media/platform/mtk-mdp3/mtk-img-ipi.h
...
> new file mode 100644
> index 000000000000..41e33995f5be
> --- /dev/null
> +++ b/drivers/media/platform/mtk-mdp3/mtk-mdp3-cmdq.c
> @@ -0,0 +1,505 @@
> +// SPDX-License-Identifier: GPL-2.0-only
...
> +	(has_op(ctx, op) ? (ctx)->comp->ops->op(ctx, ##__VA_ARGS__) :
> 0)
> +
...
> +};
> +
> +static bool is_output_disable(const struct img_compparam *param, u32
> count)
> +{
...
> +		case MDP_COMP_RDMA0:
> +			mutex_bit =
> mtk_mutex_get_mdp_mod(mutex[mutex_id], id);
...
> +}
> +
> +static int mdp_path_ctx_init(struct mdp_dev *mdp, struct mdp_path
> *path)
> +{
> +	const struct img_config *config = path->config;
> +	int index, ret;
...
> +					 path->comps[index].comp->id,
> +					 path->comps[index + 1].comp-
> >id);
> +	return 0;
> +}
> +
> +static int mdp_path_config(struct mdp_dev *mdp, struct
> mmsys_cmdq_cmd *cmd,
...
> diff --git a/drivers/media/platform/mtk-mdp3/mtk-mdp3-cmdq.h
...
> +
> +struct platform_device *mdp_get_plat_device(struct platform_device
> *pdev);
> +
> +int mdp_cmdq_sendtask(struct platform_device *pdev, struct
> img_config *config,
> +		      struct img_ipi_frameparam *param, struct
> v4l2_rect *compose,
> +		      void (*cmdq_cb)(struct cmdq_cb_data data), void
> *cb_data);
> +
...
> diff --git a/drivers/media/platform/mtk-mdp3/mtk-mdp3-comp.c
> b/drivers/media/platform/mtk-mdp3/mtk-mdp3-comp.c
> new file mode 100644
> index 000000000000..41544dee9284
> --- /dev/null
> +++ b/drivers/media/platform/mtk-mdp3/mtk-mdp3-comp.c
> @@ -0,0 +1,1264 @@
> +// SPDX-License-Identifier: GPL-2.0-only
...
> +
> +static const struct mdp_platform_config *__get_plat_cfg(const struct
> mdp_comp_ctx *ctx)
> +{
> +	if (!ctx)
> +		return NULL;
...
> +}
> +
> +static int init_rdma(struct mdp_comp_ctx *ctx, struct mmsys_cmdq_cmd
> *cmd)
> +{
> +	const struct mdp_platform_config *mdp_cfg =
> __get_plat_cfg(ctx);
...
> +{
> +	const struct mdp_rdma_subfrm *subfrm = &ctx->param-
> >rdma.subfrms[index];
> +	const struct img_comp_subfrm *csf = &ctx->param-
> >subfrms[index];
> +	const struct mdp_platform_config *mdp_cfg =
> __get_plat_cfg(ctx);
> +	u32 colorformat = ctx->input->buffer.format.colorformat;
...
> +
> +	/* Enable RDMA */
> +	MM_REG_WRITE(cmd, subsys_id, base, MDP_RDMA_EN, BIT(0),
> BIT(0));
> +
> +	/* Set Y pixel offset */
...
> +	/* Set 10bit UFO mode */
> +	if (mdp_cfg)
> +		if (mdp_cfg->rdma_support_10bit && block10bit &&
> en_ufo)
> +			MM_REG_WRITE(cmd, subsys_id, base,
...
> +			MM_REG_WRITE(cmd, subsys_id, base,
> +				     MDP_RDMA_RESV_DUMMY_0, BIT(2),
> BIT(2));
> +
> +	return 0;
> +}
> +
> +static int wait_rdma_event(struct mdp_comp_ctx *ctx, struct
> mmsys_cmdq_cmd *cmd)
> +{
...
> +};
> +
> +static int init_rsz(struct mdp_comp_ctx *ctx, struct mmsys_cmdq_cmd
> *cmd)
> +{
> +	phys_addr_t base = ctx->comp->reg_base;
> +	u8 subsys_id = ctx->comp->subsys_id;
> +
> +	/* Reset RSZ */
> +	MM_REG_WRITE(cmd, subsys_id, base, PRZ_ENABLE, 0x10000,
> BIT(16));
...
> +		/* Disable RSZ */
> +		MM_REG_WRITE(cmd, subsys_id, base, PRZ_ENABLE, 0x0,
> BIT(0));
> +		return 0;
> +	}
> +
> +	MM_REG_WRITE(cmd, subsys_id, base, PRZ_CONTROL_1, rsz-
> >control1,
> +		     0x03FFFDF3);
> +	MM_REG_WRITE(cmd, subsys_id, base, PRZ_CONTROL_2, rsz-
> >control2,
> +		     0x0FFFC290);
...
> +			     struct mmsys_cmdq_cmd *cmd, u32 index)
> +{
> +	const struct mdp_rsz_subfrm *subfrm = &ctx->param-
> >rsz.subfrms[index];
> +	const struct img_comp_subfrm *csf = &ctx->param-
> >subfrms[index];
> +	const struct mdp_platform_config *mdp_cfg =
...
> +	u8 subsys_id = ctx->comp->subsys_id;
> +
> +	MM_REG_WRITE(cmd, subsys_id, base, PRZ_CONTROL_2, subfrm-
> >control2,
> +		     0x00003800);
> +	MM_REG_WRITE(cmd, subsys_id, base, PRZ_INPUT_IMAGE, subfrm-
> >src,
> +		     0xFFFFFFFF);
> +
...
> +
> +	MM_REG_WRITE(cmd, subsys_id, base,
> PRZ_LUMA_HORIZONTAL_INTEGER_OFFSET,
...
> +		     csf->luma.top, 0xFFFF);
> +	MM_REG_WRITE(cmd, subsys_id, base,
> PRZ_LUMA_VERTICAL_SUBPIXEL_OFFSET,
...
> +
> +	MM_REG_WRITE(cmd, subsys_id, base, PRZ_OUTPUT_IMAGE, subfrm-
> >clip,
> +		     0xFFFFFFFF);
> +
> +	return 0;
...
> +};
> +
> +static int init_wrot(struct mdp_comp_ctx *ctx, struct mmsys_cmdq_cmd
> *cmd)
> +{
> +	phys_addr_t base = ctx->comp->reg_base;
> +	u8 subsys_id = ctx->comp->subsys_id;
> +
> +	/* Reset WROT */
> +	MM_REG_WRITE(cmd, subsys_id, base, VIDO_SOFT_RST, BIT(0),
> BIT(0));
> +	MM_REG_POLL(cmd, subsys_id, base, VIDO_SOFT_RST_STAT, BIT(0),
> BIT(0));
> +	MM_REG_WRITE(cmd, subsys_id, base, VIDO_SOFT_RST, 0x0, BIT(0));
> +	MM_REG_POLL(cmd, subsys_id, base, VIDO_SOFT_RST_STAT, 0x0,
> BIT(0));
> +	return 0;
> +}
> +
...
> +	MM_REG_WRITE(cmd, subsys_id, base, VIDO_BASE_ADDR, wrot-
> >iova[0],
> +		     0xFFFFFFFF);
> +	MM_REG_WRITE(cmd, subsys_id, base, VIDO_BASE_ADDR_C, wrot-
> >iova[1],
> +		     0xFFFFFFFF);
> +	MM_REG_WRITE(cmd, subsys_id, base, VIDO_BASE_ADDR_V, wrot-
> >iova[2],
> +		     0xFFFFFFFF);
...
> +		     0xF131510F);
> +	/* Write frame Y pitch */
> +	MM_REG_WRITE(cmd, subsys_id, base, VIDO_STRIDE, wrot-
> >stride[0],
> +		     0x0000FFFF);
> +	/* Write frame UV pitch */
> +	MM_REG_WRITE(cmd, subsys_id, base, VIDO_STRIDE_C, wrot-
> >stride[1],
> +		     0xFFFF);
> +	MM_REG_WRITE(cmd, subsys_id, base, VIDO_STRIDE_V, wrot-
> >stride[2],
> +		     0xFFFF);
> +	/* Write matrix control */
> +	MM_REG_WRITE(cmd, subsys_id, base, VIDO_MAT_CTRL, wrot-
> >mat_ctrl, 0xF3);
> +
...
> +		     0xFF000000);
> +	/* Set VIDO_EOL_SEL */
> +	MM_REG_WRITE(cmd, subsys_id, base, VIDO_RSV_1, BIT(31),
> BIT(31));
> +	/* Set VIDO_FIFO_TEST */
> +	if (wrot->fifo_test != 0)
...
> +			      struct mmsys_cmdq_cmd *cmd, u32 index)
> +{
> +	const struct mdp_wrot_subfrm *subfrm = &ctx->param-
> >wrot.subfrms[index];
> +	phys_addr_t base = ctx->comp->reg_base;
> +	u8 subsys_id = ctx->comp->subsys_id;
> +
...
> +		     0x1FFF1FFF);
> +	MM_REG_WRITE(cmd, subsys_id, base, VIDO_CROP_OFST, subfrm-
> >clip_ofst,
> +		     0x1FFF1FFF);
> +
> +	MM_REG_WRITE(cmd, subsys_id, base, VIDO_MAIN_BUF_SIZE,
...
> +
> +	/* Enable WROT */
> +	MM_REG_WRITE(cmd, subsys_id, base, VIDO_ROT_EN, BIT(0),
> BIT(0));
> +
> +	return 0;
> +}
> +
> +static int wait_wrot_event(struct mdp_comp_ctx *ctx, struct
> mmsys_cmdq_cmd *cmd)
> +{
> +	const struct mdp_platform_config *mdp_cfg =
> __get_plat_cfg(ctx);
...
> +};
> +
> +static int init_wdma(struct mdp_comp_ctx *ctx, struct mmsys_cmdq_cmd
> *cmd)
> +{
> +	phys_addr_t base = ctx->comp->reg_base;
> +	u8 subsys_id = ctx->comp->subsys_id;
...
> +	MM_REG_POLL(cmd, subsys_id, base, WDMA_FLOW_CTRL_DBG, BIT(0),
> BIT(0));
...
> +	MM_REG_WRITE(cmd, subsys_id, base, WDMA_DST_ADDR,   wdma-
> >iova[0],
> +		     0xFFFFFFFF);
> +	MM_REG_WRITE(cmd, subsys_id, base, WDMA_DST_U_ADDR, wdma-
> >iova[1],
> +		     0xFFFFFFFF);
> +	MM_REG_WRITE(cmd, subsys_id, base, WDMA_DST_V_ADDR, wdma-
> >iova[2],
> +		     0xFFFFFFFF);
> +	/* Setup Y pitch */
...
> +			      struct mmsys_cmdq_cmd *cmd, u32 index)
> +{
> +	const struct mdp_wdma_subfrm *subfrm = &ctx->param-
> >wdma.subfrms[index];
> +	phys_addr_t base = ctx->comp->reg_base;
> +	u8 subsys_id = ctx->comp->subsys_id;
> +
...
> +		     0x3FFF3FFF);
> +	/* Write target size */
> +	MM_REG_WRITE(cmd, subsys_id, base, WDMA_CLIP_SIZE, subfrm-
> >clip,
> +		     0x3FFF3FFF);
> +	/* Write clip offset */
> +	MM_REG_WRITE(cmd, subsys_id, base, WDMA_CLIP_COORD, subfrm-
> >clip_ofst,
> +		     0x3FFF3FFF);
> +
> +	/* Enable WDMA */
...
> +}
> +
> +static int wait_wdma_event(struct mdp_comp_ctx *ctx, struct
> mmsys_cmdq_cmd *cmd)
> +{
> +	phys_addr_t base = ctx->comp->reg_base;
> +	u8 subsys_id = ctx->comp->subsys_id;
...
> +};
> +
> +static int init_ccorr(struct mdp_comp_ctx *ctx, struct
> mmsys_cmdq_cmd *cmd)
> +{
> +	phys_addr_t base = ctx->comp->reg_base;
> +	u8 subsys_id = ctx->comp->subsys_id;
> +
> +	/* CCORR enable */
> +	MM_REG_WRITE(cmd, subsys_id, base, MDP_CCORR_EN, BIT(0),
> BIT(0));
> +	/* Relay mode */
> +	MM_REG_WRITE(cmd, subsys_id, base, MDP_CCORR_CFG, BIT(0),
> BIT(0));
> +	return 0;
> +}
> +
...
> +};
> +
> +static int init_isp(struct mdp_comp_ctx *ctx, struct mmsys_cmdq_cmd
> *cmd)
> +{
...
> +	MM_REG_WRITE_MASK(cmd, subsys_id, base, 0x2890, isp-
> >smxi_iova[0],
> +			  0xFFFFFFFF);
> +	MM_REG_WRITE_MASK(cmd, subsys_id, base, 0x27D0, isp-
> >smxi_iova[0],
> +			  0xFFFFFFFF);
> +	/* DIP_X_SMX2I_BASE_ADDR, DIP_X_SMX2O_BASE_ADDR */
> +	MM_REG_WRITE_MASK(cmd, subsys_id, base, 0x28C0, isp-
> >smxi_iova[1],
> +			  0xFFFFFFFF);
> +	MM_REG_WRITE_MASK(cmd, subsys_id, base, 0x2800, isp-
> >smxi_iova[1],
> +			  0xFFFFFFFF);
> +	/* DIP_X_SMX3I_BASE_ADDR, DIP_X_SMX3O_BASE_ADDR */
> +	MM_REG_WRITE_MASK(cmd, subsys_id, base, 0x28F0, isp-
> >smxi_iova[2],
> +			  0xFFFFFFFF);
> +	MM_REG_WRITE_MASK(cmd, subsys_id, base, 0x2830, isp-
> >smxi_iova[2],
> +			  0xFFFFFFFF);
> +	/* DIP_X_SMX4I_BASE_ADDR, DIP_X_SMX4O_BASE_ADDR */
> +	MM_REG_WRITE_MASK(cmd, subsys_id, base, 0x2920, isp-
> >smxi_iova[3],
> +			  0xFFFFFFFF);
> +	MM_REG_WRITE_MASK(cmd, subsys_id, base, 0x2860, isp-
> >smxi_iova[3],
> +			  0xFFFFFFFF);
> +
> +	switch (isp->cq_idx) {
...
> +		break;
> +	default:
> +		dev_err(dev, "Do not support this cq (%d)", isp-
> >cq_idx);
> +		return -EINVAL;
> +	}
> +
...
> +			  ctx->param->isp.tpipe_iova[index],
> 0xFFFFFFFF);
> +	return 0;
> +}
> +
> +static int wait_isp_event(struct mdp_comp_ctx *ctx, struct
> mmsys_cmdq_cmd *cmd)
> +{
> +	const struct isp_data *isp = &ctx->param->isp;
> +	struct device *dev = &ctx->comp->mdp_dev->pdev->dev;
...
> +	switch (isp->cq_idx) {
> +	case ISP_DRV_DIP_CQ_THRE0:
> +		MM_REG_WRITE_MASK(cmd, subsys_id, base, 0x2000, BIT(0),
> BIT(0));
> +		MM_REG_WAIT(cmd, ISP_P2_0_DONE);
> +		break;
> +	case ISP_DRV_DIP_CQ_THRE1:
> +		MM_REG_WRITE_MASK(cmd, subsys_id, base, 0x2000, BIT(1),
> BIT(1));
> +		MM_REG_WAIT(cmd, ISP_P2_1_DONE);
> +		break;
> +	case ISP_DRV_DIP_CQ_THRE2:
> +		MM_REG_WRITE_MASK(cmd, subsys_id, base, 0x2000, BIT(2),
> BIT(2));
> +		MM_REG_WAIT(cmd, ISP_P2_2_DONE);
> +		break;
> +	case ISP_DRV_DIP_CQ_THRE3:
> +		MM_REG_WRITE_MASK(cmd, subsys_id, base, 0x2000, BIT(3),
> BIT(3));
> +		MM_REG_WAIT(cmd, ISP_P2_3_DONE);
> +		break;
> +	case ISP_DRV_DIP_CQ_THRE4:
> +		MM_REG_WRITE_MASK(cmd, subsys_id, base, 0x2000, BIT(4),
> BIT(4));
> +		MM_REG_WAIT(cmd, ISP_P2_4_DONE);
> +		break;
> +	case ISP_DRV_DIP_CQ_THRE5:
> +		MM_REG_WRITE_MASK(cmd, subsys_id, base, 0x2000, BIT(5),
> BIT(5));
> +		MM_REG_WAIT(cmd, ISP_P2_5_DONE);
> +		break;
> +	case ISP_DRV_DIP_CQ_THRE6:
> +		MM_REG_WRITE_MASK(cmd, subsys_id, base, 0x2000, BIT(6),
> BIT(6));
> +		MM_REG_WAIT(cmd, ISP_P2_6_DONE);
> +		break;
> +	case ISP_DRV_DIP_CQ_THRE7:
> +		MM_REG_WRITE_MASK(cmd, subsys_id, base, 0x2000, BIT(7),
> BIT(7));
> +		MM_REG_WAIT(cmd, ISP_P2_7_DONE);
> +		break;
> +	case ISP_DRV_DIP_CQ_THRE8:
> +		MM_REG_WRITE_MASK(cmd, subsys_id, base, 0x2000, BIT(8),
> BIT(8));
> +		MM_REG_WAIT(cmd, ISP_P2_8_DONE);
> +		break;
> +	case ISP_DRV_DIP_CQ_THRE9:
> +		MM_REG_WRITE_MASK(cmd, subsys_id, base, 0x2000, BIT(9),
> BIT(9));
> +		MM_REG_WAIT(cmd, ISP_P2_9_DONE);
> +		break;
> +	case ISP_DRV_DIP_CQ_THRE10:
> +		MM_REG_WRITE_MASK(cmd, subsys_id, base, 0x2000,
> BIT(10), BIT(10));
...
> +};
> +
> +static const struct mdp_comp_match
> mdp_comp_matches[MDP_MAX_COMP_COUNT] = {
...
> mdp_comp_dt_info[MDP_COMP_TYPE_COUNT] = {
...
> +}
> +
> +void mdp_comp_clocks_on(struct device *dev, struct mdp_comp *comps,
> int num)
> +{
> +	int i;
> +
...
> +}
> +
> +void mdp_comp_clocks_off(struct device *dev, struct mdp_comp *comps,
> int num)
> +{
> +	int i;
> +
...
> +}
> +
> +static int mdp_get_subsys_id(struct device *dev, struct device_node
> *node,
...
> +
> +	if (!comp_pdev) {
> +		dev_err(dev, "get comp_pdev fail! comp id=%d
> type=%d\n",
> +			comp->id, comp->type);
> +		return -ENODEV;
> +	}
...
> +	ret = cmdq_dev_get_client_reg(&comp_pdev->dev, &cmdq_reg,
> index);
> +	if (ret != 0) {
> +		dev_err(&comp_pdev->dev, "cmdq_dev_get_subsys
> fail!\n");
> +		return -EINVAL;
> +	}
> +
...
> +}
> +
> +static void __mdp_comp_init(struct mdp_dev *mdp, struct device_node
> *node,
> +			    struct mdp_comp *comp)
> +{
> +	struct resource res;
...
> +}
> +
> +static int mdp_comp_init(struct mdp_dev *mdp, struct device_node
> *node,
> +			 struct mdp_comp *comp, enum mtk_mdp_comp_id
> id)
> +{
> +	struct device *dev = &mdp->pdev->dev;
> +	int clk_num;
...
> +}
> +
> +static int mdp_sub_comps_create(struct mdp_dev *mdp, struct
> device_node *node)
> +{
> +	struct device *dev = &mdp->pdev->dev;
...
> +	int index = 0;
> +
> +	of_property_for_each_string(node, "mediatek,mdp3-comps", prop,
> name) {
...
> == 0) {
> +				type = (enum mdp_comp_type)matches-
> >data;
> +				break;
> +			}
> +		}
> +
> +		ret = of_property_read_u32_index(node, "mediatek,mdp3-
> comp-ids",
> +						 index, &alias_id);
> +		if (ret) {
...
> +		if (id < 0) {
> +			dev_err(dev, "Failed to get comp id: %s (%d,
> %d)\n",
> +				name, type, alias_id);
> +			return -ENODEV;
> +		}
...
> +		ret = of_property_read_u32(node, "mediatek,mdp3-id",
> &alias_id);
> +		if (ret) {
> +			dev_warn(dev, "Skipping unknown component
> %pOF\n", node);
...
> +	return 0;
> +}
> diff --git a/drivers/media/platform/mtk-mdp3/mtk-mdp3-comp.h
...
> +	int (*init_comp)(struct mdp_comp_ctx *ctx, struct
> mmsys_cmdq_cmd *cmd);
> +	int (*config_frame)(struct mdp_comp_ctx *ctx, struct
> mmsys_cmdq_cmd *cmd,
> +			    const struct v4l2_rect *compose);
...
> +			      struct mmsys_cmdq_cmd *cmd, u32 index);
> +	int (*post_process)(struct mdp_comp_ctx *ctx, struct
> mmsys_cmdq_cmd *cmd);
> +};
> +
> +struct mdp_dev;
...
> diff --git a/drivers/media/platform/mtk-mdp3/mtk-mdp3-core.c
> b/drivers/media/platform/mtk-mdp3/mtk-mdp3-core.c
> new file mode 100644
> index 000000000000..535a9b51ee88
> --- /dev/null
> +++ b/drivers/media/platform/mtk-mdp3/mtk-mdp3-core.c
> @@ -0,0 +1,338 @@
> +// SPDX-License-Identifier: GPL-2.0-only
...
> +
> +static struct platform_device *__get_pdev_by_name(struct
...
> +			goto err_reg_vpu;
> +		}
> +		ret = mdp_vpu_dev_init(&mdp->vpu, mdp->scp, &mdp-
> >vpu_lock);
> +		if (ret) {
> +			dev_err(&mdp->pdev->dev,
...
> +{
> +	struct mdp_dev *mdp = (struct mdp_dev
...
> +
> +	mdp_node = of_parse_phandle(pdev->dev.of_node, "mediatek,mm-
> mutex", 0);
> +	if (!mdp_node) {
> +		ret = -ENODEV;
> +		goto err_return;
...
> +
> +	for (i = 0; i < MDP_PIPE_MAX; i++) {
> +		mdp->mdp_mutex[i] = mtk_mutex_mdp_get(&mm_pdev->dev,
> i);
> +		if (!mdp->mdp_mutex[i]) {
> +			ret = -ENODEV;
> +			goto err_return;
...
> +	}
> +
> +	mdp->job_wq = alloc_workqueue(MDP_MODULE_NAME, WQ_FREEZABLE,
> 0);
> +	if (!mdp->job_wq) {
...
> +	dev_dbg(&pdev->dev, "MDP rproc_handle: %pK", mdp-
> >rproc_handle);
> +
...
> +	if (ret) {
> +		v4l2_err(&mdp->v4l2_dev, "Failed to register m2m
> device\n");
...
> diff --git a/drivers/media/platform/mtk-mdp3/mtk-mdp3-core.h
> b/drivers/media/platform/mtk-mdp3/mtk-mdp3-core.h
> new file mode 100644
> index 000000000000..d688071b0bf1
> --- /dev/null
> +++ b/drivers/media/platform/mtk-mdp3/mtk-mdp3-core.h
> @@ -0,0 +1,76 @@
> +/* SPDX-License-Identifier: GPL-2.0-only */
...
> +	struct mdp_comp				*comp[MDP_MAX_COMP_CO
> UNT];
> +	const struct mtk_mdp_driver_data	*mdp_data;
> +	s32					event[MDP_MAX_EVENT_C
> OUNT];
> +
...
> diff --git a/drivers/media/platform/mtk-mdp3/mtk-mdp3-m2m.c
> b/drivers/media/platform/mtk-mdp3/mtk-mdp3-m2m.c
> new file mode 100644
> index 000000000000..9b3d9a59ae48
> --- /dev/null
> +++ b/drivers/media/platform/mtk-mdp3/mtk-mdp3-m2m.c
> @@ -0,0 +1,789 @@
> +// SPDX-License-Identifier: GPL-2.0-only
...
> +{
> +	struct mdp_m2m_ctx *ctx = container_of(work, struct
> mdp_m2m_ctx, work);
...
> +}
> +
> +static int mdp_m2m_start_streaming(struct vb2_queue *q, unsigned int
> count)
> +{
...
> +}
> +
> +static struct vb2_v4l2_buffer *mdp_m2m_buf_remove(struct mdp_m2m_ctx
> *ctx,
> +						  unsigned int type)
> +{
> +	if (V4L2_TYPE_IS_OUTPUT(type))
...
> +
> +	dev_info(dev, "[%d] type:%d, frame:%ux%u colorspace=%d", ctx-
> >id, f->type,
...
> +	if (!fmt) {
> +		dev_info(dev, "[%d] try_fmt failed, type:%d", ctx->id,
> f->type);
> +		return -EINVAL;
> +	}
> +
...
> +
> +	if (!valid) {
> +		dev_err(dev, "[%s:%d] invalid type:%u target:%u",
> __func__, ctx->id,
> +			s->type, s->target);
> +		return -EINVAL;
> +	}
...
> +	snprintf(mdp->m2m_vdev->name, sizeof(mdp->m2m_vdev->name),
> "%s:m2m",
> +		 MDP_MODULE_NAME);
...
> +	mdp_m2m_process_done(ctx, vb_state);
> +}
> diff --git a/drivers/media/platform/mtk-mdp3/mtk-mdp3-m2m.h
> b/drivers/media/platform/mtk-mdp3/mtk-mdp3-m2m.h
> new file mode 100644
> index 000000000000..ffab59ac9dc7
> --- /dev/null
> +++ b/drivers/media/platform/mtk-mdp3/mtk-mdp3-m2m.h
> @@ -0,0 +1,49 @@
> +/* SPDX-License-Identifier: GPL-2.0-only */
...
> new file mode 100644
> index 000000000000..439711298867
> --- /dev/null
> +++ b/drivers/media/platform/mtk-mdp3/mtk-mdp3-regs.c
> @@ -0,0 +1,737 @@
> +// SPDX-License-Identifier: GPL-2.0-only
...
> +};
> +
> +static const struct mdp_format *mdp_find_fmt(u32 pixelformat, u32
> type)
> +{
> +	u32 i, flag;
> +
...
> +}
> +
> +static const struct mdp_format *mdp_find_fmt_by_index(u32 index, u32
> type)
> +{
> +	u32 i, flag, num = 0;
> +
...
> +}
> +
> +enum mdp_ycbcr_profile mdp_map_ycbcr_prof_mplane(struct v4l2_format
> *f,
> +						 u32 mdp_color)
> +{
...
> +	return 0;
> +}
> diff --git a/drivers/media/platform/mtk-mdp3/mtk-mdp3-regs.h
> b/drivers/media/platform/mtk-mdp3/mtk-mdp3-regs.h
> new file mode 100644
> index 000000000000..3ff784aa372b
> --- /dev/null
> +++ b/drivers/media/platform/mtk-mdp3/mtk-mdp3-regs.h
> @@ -0,0 +1,372 @@
> +/* SPDX-License-Identifier: GPL-2.0-only */
...
> + */
> +#define MDP_COLOR(PACKED, LOOSE, VIDEO, PLANE, HF, VF, BITS, GROUP,
> SWAP, ID)\
...
> +#define MDP_COLOR_IS_10BIT_LOOSE(c)	(((0x0c000000 & (c)) >> 26) ==
> 1)
...
> +	MDP_COLOR_RGBA8888	= MDP_COLOR(0, 0, 0, 1, 0, 0, 32,
> 0,  1, 2),
> +	MDP_COLOR_BGRA8888	= MDP_COLOR(0, 0, 0, 1, 0, 0, 32,
> 0,  0, 2),
> +	MDP_COLOR_ARGB8888	= MDP_COLOR(0, 0, 0, 1, 0, 0, 32,
> 0,  1, 3),
> +	MDP_COLOR_ABGR8888	= MDP_COLOR(0, 0, 0, 1, 0, 0, 32,
> 0,  0, 3),
> +
...
> +	MDP_COLOR_420_BLK_UFO	= MDP_COLOR(0, 0, 5, 2, 1, 1, 256, 1,
> 0, 12),
> +	/* Block mode */
> +	MDP_COLOR_420_BLK	= MDP_COLOR(0, 0, 1, 2, 1, 1, 256, 1, 0, 12),
> +	/* Block mode + field mode */
> +	MDP_COLOR_420_BLKI	= MDP_COLOR(0, 0, 3, 2, 1, 1, 256, 1,
> 0, 12),
> +	/* Block mode */
> +	MDP_COLOR_422_BLK	= MDP_COLOR(0, 0, 1, 1, 1, 0, 512, 1, 0, 4),
> +
> +	MDP_COLOR_IYU2		= MDP_COLOR(0, 0, 0, 1, 0, 0, 24,  1,
> 0, 25),
> +	MDP_COLOR_YUV444	= MDP_COLOR(0, 0, 0, 1, 0, 0, 24,  1, 0, 30),
> +
> +	/* Packed 10-bit formats */
> +	MDP_COLOR_RGBA1010102	= MDP_COLOR(1, 0, 0, 1, 0, 0, 32,  0,
> 1, 2),
> +	MDP_COLOR_BGRA1010102	= MDP_COLOR(1, 0, 0, 1, 0, 0, 32,  0,
> 0, 2),
> +	/* Packed 10-bit UYVY */
> +	MDP_COLOR_UYVY_10P	= MDP_COLOR(1, 0, 0, 1, 1, 0, 20,  1,
> 0, 4),
> +	/* Packed 10-bit NV21 */
> +	MDP_COLOR_NV21_10P	= MDP_COLOR(1, 0, 0, 2, 1, 1, 10,  1,
> 1, 12),
> +	/* 10-bit block mode */
> +	MDP_COLOR_420_BLK_10_H	= MDP_COLOR(1, 0, 1, 2, 1, 1, 320, 1,
> 0, 12),
> +	/* 10-bit HEVC tile mode */
> +	MDP_COLOR_420_BLK_10_V	= MDP_COLOR(1, 1, 1, 2, 1, 1, 320, 1,
> 0, 12),
> +	/* UFO encoded 10-bit block mode */
> +	MDP_COLOR_420_BLK_U10_H	= MDP_COLOR(1, 0, 5, 2, 1, 1, 320, 1,
> 0, 12),
> +	/* UFO encoded 10-bit HEVC tile mode */
> +	MDP_COLOR_420_BLK_U10_V	= MDP_COLOR(1, 1, 5, 2, 1, 1, 320, 1,
> 0, 12),
> +
> +	/* Loose 10-bit formats */
> +	MDP_COLOR_UYVY_10L	= MDP_COLOR(0, 1, 0, 1, 1, 0, 20,  1,
> 0, 4),
> +	MDP_COLOR_VYUY_10L	= MDP_COLOR(0, 1, 0, 1, 1, 0, 20,  1,
> 1, 4),
> +	MDP_COLOR_YUYV_10L	= MDP_COLOR(0, 1, 0, 1, 1, 0, 20,  1,
> 0, 5),
> +	MDP_COLOR_YVYU_10L	= MDP_COLOR(0, 1, 0, 1, 1, 0, 20,  1,
> 1, 5),
> +	MDP_COLOR_NV12_10L	= MDP_COLOR(0, 1, 0, 2, 1, 1, 10,  1,
> 0, 12),
> +	MDP_COLOR_NV21_10L	= MDP_COLOR(0, 1, 0, 2, 1, 1, 10,  1,
> 1, 12),
> +	MDP_COLOR_NV16_10L	= MDP_COLOR(0, 1, 0, 2, 1, 0, 10,  1,
> 0, 13),
> +	MDP_COLOR_NV61_10L	= MDP_COLOR(0, 1, 0, 2, 1, 0, 10,  1,
> 1, 13),
> +	MDP_COLOR_YV12_10L	= MDP_COLOR(0, 1, 0, 3, 1, 1, 10,  1,
> 1, 8),
> +	MDP_COLOR_I420_10L	= MDP_COLOR(0, 1, 0, 3, 1, 1, 10,  1,
> 0, 8),
> +};
> +
...
> +static inline u32 mdp_color_get_min_uv_stride(enum mdp_color c, u32
> width)
> +{
> +	u32 min_stride;
> +
...
> diff --git a/drivers/media/platform/mtk-mdp3/mtk-mdp3-vpu.c
...
> +}
> +
> +static int mdp_vpu_sendmsg(struct mdp_vpu_dev *vpu, enum scp_ipi_id
> id,
...
> +		&addr, sizeof(addr));
> +}
> diff --git a/drivers/media/platform/mtk-mdp3/mtk-mdp3-vpu.h
