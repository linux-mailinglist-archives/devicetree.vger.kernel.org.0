...
Received: from vger.kernel.org (vger.kernel.org [23.128.96.18])
	by mail.lfdr.de (Postfix) with ESMTP id F1755322912
	for <lists+devicetree@lfdr.de>; Tue, 23 Feb 2021 11:53:17 +0100 (CET)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S232297AbhBWKwh (ORCPT <rfc822;lists+devicetree@lfdr.de>);
        Tue, 23 Feb 2021 05:52:37 -0500
Received: from lindbergh.monkeyblade.net ([23.128.96.19]:38872 "EHLO
        lindbergh.monkeyblade.net" rhost-flags-OK-OK-OK-OK) by vger.kernel.org
        with ESMTP id S232274AbhBWKwS (ORCPT
        <rfc822;devicetree@vger.kernel.org>); Tue, 23 Feb 2021 05:52:18 -0500
Received: from mail-ej1-x636.google.com (mail-ej1-x636.google.com [IPv6:2a00:1450:4864:20::636])
        by lindbergh.monkeyblade.net (Postfix) with ESMTPS id 0E74BC06174A;
        Tue, 23 Feb 2021 02:51:37 -0800 (PST)
Received: by mail-ej1-x636.google.com with SMTP id jt13so33934976ejb.0;
        Tue, 23 Feb 2021 02:51:36 -0800 (PST)
...
X-Google-DKIM-Signature: v=1; a=rsa-sha256; c=relaxed/relaxed;
        ...
        bh=+swzN/yiXrod0OMg5exk+5CvJ9cZ3GvP+7gN8uE46mA=;
        b=
X-Gm-Message-State: AOAM530aCLpmZSiY/lBp/5mkZyS45aJavggvOWH4qYctA4Udo8JIbY6P
        3KCP9bjhAVB/UV/yM6wSG+kFiFFI3WW5+UY33xo=
X-Google-Smtp-Source: ABdhPJzfc7LaA1IsOYopZd61VMhZ2mhV9l87cQoMSdIIExVzSj0d6HlrM5TbG/JIGAAqJqs4Zumz1gLFAY5XzVbMkLU=
X-Received: by 2002:a17:906:11d5:: with SMTP id o21mr24547146eja.504.1614077495635;
...
References: <20210218210504.375752-1-adrien.grassein@gmail.com>
 ...
 <CAG3jFyv1HW_zKeL+wjZgmBvxAa__Y1X6-GgrC_W=0808hF-t5g@mail.gmail.com> <CABkfQAHuVtiy3PfyWhBxbA-BTSAsY6OAVYPiz942VvEb6k=gWw@mail.gmail.com>
...
>
> Le ven. 19 f=C3=A9vr. 2021 =C3=A0 22:28, Robert Foss <robert.foss@linaro.=
org> a =C3=A9crit :
> >
> > On Fri, 19 Feb 2021 at 16:01, Adrien Grassein <adrien.grassein@gmail.co=
m> wrote:
> > >
> > > Hey Robert,
> > >
> > > Le ven. 19 f=C3=A9vr. 2021 =C3=A0 14:27, Robert Foss <robert.foss@lin=
aro.org> a =C3=A9crit :
> > > >
> > > > Hey Adrien,
> > > >
...
> > > >
> > > > On Thu, 18 Feb 2021 at 22:05, Adrien Grassein <adrien.grassein@gmai=
l.com> wrote:
> > > > >
...
> > > > >  S:     Maintained
> > > > >  F:     Documentation/devicetree/bindings/display/bridge/lontium,=
lt8912.yaml
...
> > > > >           on ARM-based platforms. Saying Y here when this driver =
is not needed
...
> > > > > diff --git a/drivers/gpu/drm/bridge/Makefile b/drivers/gpu/drm/br=
idge/Makefile
...
> > > > > diff --git a/drivers/gpu/drm/bridge/lontium-lt8912.c b/drivers/gp=
u/drm/bridge/lontium-lt8912.c
...
> > > > > +       };
> > > > > +
> > > > > +       return regmap_multi_reg_write(lt->regmap[I2C_CEC_DSI], se=
q, ARRAY_SIZE(seq));
> > > > > +};
> > > > > +
...
> > > > > +};
> > > > > +
> > > > > +static inline struct lt8912 *bridge_to_lt8912(struct drm_bridge =
*b)
> > > > > +{
> > > > > +       return container_of(b, struct lt8912, bridge);
> > > > > +}
> > > > > +
> > > > > +static inline struct lt8912 *connector_to_lt8912(struct drm_conn=
ector *c)
> > > > > +{
...
> > > > > +       int ret;
> > > > > +       /*
> > > > > +        * At this time we only initialize 2 chips, but the lt891=
2 provides
> > > > > +        * a third interface for the audio over HDMI configuratio=
n.
> > > > > +        */
...
> > > > > +               /*
> > > > > +                * lt->i2c_client[0] was already allocated earlie=
r by the
> > > > > +                * kernel (via DT).
> > > > > +                */
> > > > > +               if (i > 0) {
> > > > > +                       lt->i2c_client[i] =3D i2c_new_dummy_devic=
e(lt->i2c_client[0]->adapter,
> > > > > +                                                                =
info[i].addr);
> > > > > +                       if (!lt->i2c_client[i])
> > > > > +                               return -ENODEV;
> > > > > +               }
> > > > > +               lt->regmap[i] =3D devm_regmap_init_i2c(lt->i2c_cl=
ient[i],
> > > > > +                                                    &lt8912_regm=
ap_config);
...
> > > > > +}
> > > >
> > > > I had a 2nd look at prior art for bridges with multiple I2C address=
es,
> > > > and I think I prefer the implementation in anx7625. Instead of havi=
ng
> > > > one I2C address provided by the DT and one by the driver, all are
> > > > provided by the driver. If you have any even better suggestions, I'=
d
> > > > be curious to hear them.
> > > >
> > > > https://github.com/torvalds/linux/blob/master/drivers/gpu/drm/bridg=
e/analogix/anx7625.c#L1646
> > > >
> > >
...
given by the DTB.
(I got a -EBUSY error when calling  "i2c_new_dummy_device"). When reading t=
he
...
;
> > > > > +       ret |=3D regmap_write(lt->regmap[I2C_CEC_DSI], 0x11, sett=
le);
> > > > > +       ret |=3D regmap_write(lt->regmap[I2C_CEC_DSI], 0x18, hpw)=
;
> > > > > +       ret |=3D regmap_write(lt->regmap[I2C_CEC_DSI], 0x19, vpw)=
;
> > > > > +       ret |=3D regmap_write(lt->regmap[I2C_CEC_DSI], 0x1c, hact=
ive & 0xff);
> > > > > +       ret |=3D regmap_write(lt->regmap[I2C_CEC_DSI], 0x1d, hact=
ive >> 8);
> > > > > +
> > > > > +       ret |=3D regmap_write(lt->regmap[I2C_CEC_DSI], 0x2f, 0x0c=
);
> > > > > +
> > > > > +       ret |=3D regmap_write(lt->regmap[I2C_CEC_DSI], 0x34, h_to=
tal & 0xff);
> > > > > +       ret |=3D regmap_write(lt->regmap[I2C_CEC_DSI], 0x35, h_to=
tal >> 8);
> > > > > +
> > > > > +       ret |=3D regmap_write(lt->regmap[I2C_CEC_DSI], 0x36, v_to=
tal & 0xff);
> > > > > +       ret |=3D regmap_write(lt->regmap[I2C_CEC_DSI], 0x37, v_to=
tal >> 8);
> > > > > +
...
> > > > > +lt8912_connector_detect(struct drm_connector *connector, bool fo=
rce)
> > > > > +{
...
> > > > > +}
> > > > > +
> > > > > +static int lt8912_connector_get_modes(struct drm_connector *conn=
ector)
> > > > > +{
> > > > > +       struct edid *edid;
> > > > > +       int ret, num =3D 0;
...
> > > > > +               return ret;
> > > > > +
> > > > > +       ret =3D drm_display_info_set_bus_formats(&connector->disp=
lay_info,
> > > > > +                                              &bus_format, 1);
> > > > > +       if (ret)
> > > > > +               num =3D ret;
...
> > > > > +}
> > > > > +
> > > > > +static const struct drm_connector_helper_funcs lt8912_connector_=
helper_funcs =3D {
...
> > > > > +       struct mipi_dsi_device *dsi;
> > > > > +       int ret =3D -1;
> > > > > +       const struct mipi_dsi_device_info info =3D { .type =3D "l=
t8912",
> > > > > +                                                  .channel =3D 0=
,
> > > > > +                                                  .node =3D NULL=
...
> > > > > +}
> > > > > +
> > > > > +static int lt8912_bridge_connector_init(struct drm_bridge *bridg=
e)
> > > > > +{
> > > > > +       int ret;
> > > > > +       struct lt8912 *lt =3D bridge_to_lt8912(bridge);
...
> > > > > +               goto exit;
> > > > > +
> > > > > +       drm_connector_helper_add(connector, &lt8912_connector_hel=
per_funcs);
> > > > > +
...
> > > > > +}
> > > > > +
> > > > > +static struct edid *lt8912_bridge_get_edid(struct drm_bridge *br=
idge,
> > > > > +                                              struct drm_connect=
or *connector)
> > > > > +{
...
> > > > > +
> > > > > +       gp_reset =3D devm_gpiod_get_optional(dev, "reset", GPIOD_=
OUT_HIGH);
> > > > > +       if (IS_ERR(gp_reset)) {
...
1);
> > > > > +       if (!port_node) {
> > > > > +               dev_err(lt->dev, "%s: Failed to get connector por=
t\n", __func__);
> > > > > +               ret =3D -ENODEV;
...
> > > > > +
> > > > > +       lt =3D devm_kzalloc(dev, sizeof(struct lt8912), GFP_KERNE=
L);
> > > > > +       if (!lt)
> > > > > +               return -ENOMEM;
> > > > > +
...
> > > >
> > > > I would expect DRM_BRIDGE_OP_HPD and possibly DRM_BRIDGE_OP_DETECT =
to
> > > > be enabled, since the hardware brief mentions supporting hot plug
> > > > detection.
> > > > drm_bridge_funcs.hpd_enable & drm_bridge_funcs.hpd_disable should b=
oth
...
> > > >
> > > > Have you seen the lt8912 driver in the Toradex tree? It mentions HP=
D
> > > > and the relevant registers, but doesn't implement support for it
> > > >
> > > > http://git.toradex.com/cgit/linux-toradex.git/tree/drivers/gpu/drm/=
bridge/lt8912.c?h=3Dcolibri&id=3D331ac1cf6e09d90e7d9ab39445bc8812ff33f178#n=
248
> > > >
> > >
> > > Yes I read the product brief and I know that this chip can do HPD. Th=
e
> > > problem is that it is not working (so far) on my board.
> > > Second point, my driver uses an "hdmi-connector" that does HPD and DD=
C
