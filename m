...
Received: from vger.kernel.org (vger.kernel.org [23.128.96.18])
	by mail.lfdr.de (Postfix) with ESMTP id D37AB2FF52F
	for <lists+devicetree@lfdr.de>; Thu, 21 Jan 2021 20:56:14 +0100 (CET)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S1726923AbhAUTzS (ORCPT <rfc822;lists+devicetree@lfdr.de>);
        Thu, 21 Jan 2021 14:55:18 -0500
Received: from lindbergh.monkeyblade.net ([23.128.96.19]:38196 "EHLO
        lindbergh.monkeyblade.net" rhost-flags-OK-OK-OK-OK) by vger.kernel.org
        with ESMTP id S1726898AbhAUTzC (ORCPT
        <rfc822;devicetree@vger.kernel.org>); Thu, 21 Jan 2021 14:55:02 -0500
Received: from relay07.th.seeweb.it (relay07.th.seeweb.it [IPv6:2001:4b7a:2000:18::168])
        by lindbergh.monkeyblade.net (Postfix) with ESMTPS id 7D399C061794
        for <devicetree@vger.kernel.org>; Thu, 21 Jan 2021 11:52:56 -0800 (PST)
Received: from IcarusMOD.eternityproject.eu (unknown [2.237.20.237])
        (using TLSv1.3 with cipher TLS_AES_256_GCM_SHA384 (256/256 bits)
         key-exchange X25519 server-signature RSA-PSS (2048 bits) server-digest SHA256)
        (No client certificate requested)
        by m-r2.th.seeweb.it (Postfix) with ESMTPSA id 4C7F03EEEC;
...
Cc:     bjorn.andersson@linaro.org, agross@kernel.org, rjw@rjwysocki.net,
        ...
        <angelogioacchino.delregno@somainline.org>
Subject: [PATCH v5 5/7] cpufreq: qcom-hw: Implement CPRh aware OSM programming
Date:   Thu, 21 Jan 2021 20:52:48 +0100
Message-Id: <20210121195250.492500-6-angelogioacchino.delregno@somainline.org>
...
index 9ed5341dc515..9a213a3046bb 100644
...
@@ -127,6 +389,648 @@ static unsigned int qcom_cpufreq_hw_fast_switch(struct cpufreq_policy *policy,
...
+	}
+
+	for (i = 0, rate = 0; i <= gpd_opp_cnt ; rate++, i++) {
...
+
+	ret = qcom_cpufreq_gen_params(cpu_dev, ddata, &hw_tbl,
...
+	if (num_entries < 2) {
+		dev_err(&pdev->dev,
+			"Not enough LUT entries found (%u)\n", num_entries);
+		return -EINVAL;
+	}
+
...
+ */
 static int qcom_cpufreq_hw_read_lut(struct device *cpu_dev,
 				    struct cpufreq_policy *policy)
 {
@@ -166,7 +1070,9 @@ static int qcom_cpufreq_hw_read_lut(struct device *cpu_dev,
 	for (i = 0; i < LUT_MAX_ENTRIES; i++) {
...
@@ -175,17 +1081,21 @@ static int qcom_cpufreq_hw_read_lut(struct device *cpu_dev,
...
@@ -205,18 +1115,19 @@ static int qcom_cpufreq_hw_read_lut(struct device *cpu_dev,
...
@@ -227,10 +1138,18 @@ static int qcom_cpufreq_hw_read_lut(struct device *cpu_dev,
 	return 0;
 }
...
@@ -245,34 +1164,237 @@ static void qcom_get_related_cpus(int index, struct cpumask *m)
 		if (ret < 0)
 			continue;
...
+
 ...
 {
 	struct platform_device *pdev = cpufreq_get_driver_data();
@@ -282,7 +1404,8 @@ static int qcom_cpufreq_hw_cpu_init(struct cpufreq_policy *policy)
...
+	int cpu_count, index, ret;
 	...
 	if (!cpu_dev) {
@@ -303,9 +1426,14 @@ static int qcom_cpufreq_hw_cpu_init(struct cpufreq_policy *policy)
...
+	kfree(fdom_resname);
 	...
 	if (!data) {
@@ -315,22 +1443,31 @@ static int qcom_cpufreq_hw_cpu_init(struct cpufreq_policy *policy)
...
+	}
 	...
 	if (ret) {
@@ -349,6 +1486,7 @@ static int qcom_cpufreq_hw_cpu_init(struct cpufreq_policy *policy)
...
@@ -389,9 +1527,51 @@ static struct cpufreq_driver cpufreq_qcom_hw_driver = {
...
+	}
 	...
 	if (IS_ERR(clk))
@@ -404,16 +1584,16 @@ static int qcom_cpufreq_hw_driver_probe(struct platform_device *pdev)
 	if (IS_ERR(clk))
 		return PTR_ERR(clk);
