...
Received: from vger.kernel.org (vger.kernel.org [209.132.180.67])
	by mail.lfdr.de (Postfix) with ESMTP id 6FB609D015
	for <lists+devicetree@lfdr.de>; Mon, 26 Aug 2019 15:09:47 +0200 (CEST)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S1731473AbfHZNJq (ORCPT <rfc822;lists+devicetree@lfdr.de>);
        Mon, 26 Aug 2019 09:09:46 -0400
Received: from retiisi.org.uk ([95.216.213.190]:47382 "EHLO
        hillosipuli.retiisi.org.uk" rhost-flags-OK-OK-OK-OK)
        by vger.kernel.org with ESMTP id S1731174AbfHZNJq (ORCPT
...
Received: from valkosipuli.localdomain (valkosipuli.retiisi.org.uk [IPv6:2a01:4f9:c010:4572::80:2])
        ...
        by hillosipuli.retiisi.org.uk (Postfix) with ESMTPS id B0E23634C87;
        Mon, 26 Aug 2019 16:09:30 +0300 (EEST)
Received: from sailus by valkosipuli.localdomain with local (Exim 4.92)
        (envelope-from <sakari.ailus@retiisi.org.uk>)
        id 1i2EkU-0000P7-Ii; Mon, 26 Aug 2019 16:09:30 +0300
Date:   Mon, 26 Aug 2019 16:09:30 +0300
...
Subject: Re: [RFC PATCH V3 1/4] media: platform: mtk-isp: Add Mediatek sensor
 interface driver
Message-ID: <20190826130930.GD826@valkosipuli.retiisi.org.uk>
...
I think you could omit this, as the check is present in the Makefile in the
subdirectory.
...
Please not wrap like this; instead you can do that between the arguments
and before the function name, but not elsewhere.

u32 would seem like a more appropriate return type.
...
Please document in DT bindings how the lane numbering goes.
...
Perhaps "mtk" prefix here? It's used by the rest of the driver. Or remove
the underscore.
...
What do these bits mean? Please add #defines for them in the header. The
same applies to similar numerical values used throughout the driver.

Could you use one of the macros you use to access these registers, or
writel() or such?
...
A for loop would seem more suitable for this; same below.
...
You appear to be missing the link_validate callback. Most likely
v4l2_subdev_link_validate_default will be fine.
...
Please align at the same level than the code.
...
This seems like a leftover from the development time. If you'd like to keep
it or something like that, use dev_dbg().
...
Please instead use media_entity_get_fwnode_pad().
...
In general, I'd suggest parsing the information from the firmware during
probe rather than here. See e.g. the omap3isp driver for an example. You
seem to have a function below for this purpose; it's just content that's
missing in it. :-)
...
Are the clocks needed by the seninf, or the sensors? If they're the
sensors, then please use the clock framework to expose them to the sensor
drivers. Otherwise they should go to the driver's runtime PM callbacks.
...
Please use names that look nice when printed.
...
Please indent this. If a rule is broken, I think in this case it'd be the
80 characters per line one.
...
How many of these can be used simultaneously?
...
If you have 12 pads on a single subdev with 4 sinks, what are the 8 source
pads for? I think this would necessitate some documentation under
Documentation/media/v4l-drivers/ . There may be a need to discuss the
design, too. Do you need the S_ROUTING IOCTL support to configure this, for
instance?
...
__maybe_unused; same for the other PM callbacks.
...
You can use .of_match_table = of_match_ptr(...),
...
Looks like you'll need bus-width and data-shift properties on the parallel
interface as well in DT bindings.
...
Does the HW block support CSI-2 output as well?
...
If the register named like this are simply not implemented by the hardware,
I think you could simply omit them.
