...
Received: from out1.vger.email (out1.vger.email [IPv6:2620:137:e000::1:20])
	by mail.lfdr.de (Postfix) with ESMTP id EA986793CD0
	for <lists+devicetree@lfdr.de>; Wed,  6 Sep 2023 14:38:22 +0200 (CEST)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S239981AbjIFMiY (ORCPT <rfc822;lists+devicetree@lfdr.de>);
        Wed, 6 Sep 2023 08:38:24 -0400
Received: from lindbergh.monkeyblade.net ([23.128.96.19]:47672 "EHLO
        lindbergh.monkeyblade.net" rhost-flags-OK-OK-OK-OK) by vger.kernel.org
        with ESMTP id S240649AbjIFMiX (ORCPT
        <rfc822;devicetree@vger.kernel.org>); Wed, 6 Sep 2023 08:38:23 -0400
Received: from smtp.kernel.org (relay.kernel.org [52.25.139.140])
        by lindbergh.monkeyblade.net (Postfix) with ESMTPS id 622C11987;
        Wed,  6 Sep 2023 05:38:08 -0700 (PDT)
Received: by smtp.kernel.org (Postfix) with ESMTPSA id C0774C433C7;
        Wed,  6 Sep 2023 12:38:03 +0000 (UTC)
Message-ID: <e425e50f-5a0a-168b-4f46-1c9b0b30bfe5@xs4all.nl>
Date:   Wed, 6 Sep 2023 14:38:01 +0200
MIME-Version: 1.0
...
In-Reply-To: <20230816152210.4080779-3-devarsht@ti.com>
...
You can just do:
	...
	return write_reg_field_not_busy(...
...
Just use return write_reg_field_not_busy(), same in the similar instances below.
...
I haven't read through the whole patch yet, but almost certainly get_queue is
only ever called with V4L2_BUF_TYPE_VIDEO_OUTPUT_MPLANE or V4L2_BUF_TYPE_VIDEO_CAPTURE_MPLANE.
...
Then the caller doesn't need to do any error checking, thus simplifying
the code further.
...
bus_info is filled in automatically for platform devices, just drop this.

> +	cap->device_caps = V4L2_CAP_VIDEO_M2M_MPLANE | V4L2_CAP_STREAMING;
> +	cap->capabilities = cap->device_caps | V4L2_CAP_DEVICE_CAPS;

No need for these two lines, the v4l2 framework will use device_caps in struct video_device
...
Hmm, this looks very much like the VIDIOC_TRY_FMT is actually updating
real values, instead of just 'trying' them.

I think this belongs in s_fmt below. Same for the other changes to
queue in this function.
...
No need for this, done by the v4l2 framework.
...
And here is where the actualy updates to e5010_q_data should take place, I think.
It certainly does not make sense that s_fmt is just the same as try_fmt. That
can really only happen if there is just a single hardcoded format, and that isn't
the case here.
...
Why is this needed? Either drop this or comment on why this is needed.
...
This should already ensure contiguous buffers.
...
This really can't happen, so I'd just drop this error message.
...
Ew, you are modifying a global struct.

Just embed video_device in struct e5010_dev do:
...
Drop these three NULL assignments: these fields are already NULL.
...
This must be done last. As soon as you register the video device it can be
used, so registering the video device should only be done once everything
...
This is where you can safely register the video device.
...
Can never be an error as mentioned in my get_queue comment.
...
If start_streaming fails, then it must return all queued buffers to
