...
Received: from out1.vger.email (out1.vger.email [IPv6:2620:137:e000::1:20])
	by mail.lfdr.de (Postfix) with ESMTP id B706654282E
	for <lists+devicetree@lfdr.de>; Wed,  8 Jun 2022 09:48:44 +0200 (CEST)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S234300AbiFHHIz (ORCPT <rfc822;lists+devicetree@lfdr.de>);
        Wed, 8 Jun 2022 03:08:55 -0400
Received: from lindbergh.monkeyblade.net ([23.128.96.19]:33114 "EHLO
        lindbergh.monkeyblade.net" rhost-flags-OK-OK-OK-OK) by vger.kernel.org
        with ESMTP id S1354775AbiFHGTp (ORCPT
        <rfc822;devicetree@vger.kernel.org>); Wed, 8 Jun 2022 02:19:45 -0400
Received: from mailgw02.mediatek.com (unknown [210.61.82.184])
        by lindbergh.monkeyblade.net (Postfix) with ESMTPS id B152019F9C;
        Tue,  7 Jun 2022 23:17:21 -0700 (PDT)
X-UUID: cad35b00221045709436f16bfc371d3d-20220608
X-CID-P-RULE: Release_Ham
X-CID-O-INFO: VERSION:1.1.5,REQID:2eebde3e-5db5-4251-aa72-e552d4a65b8e,OB:0,LO
        B:0,IP:0,URL:5,TC:0,Content:-5,EDM:0,RT:0,SF:100,FILE:0,RULE:Release_Ham,A
        CTION:release,TS:100
X-CID-INFO: VERSION:1.1.5,REQID:2eebde3e-5db5-4251-aa72-e552d4a65b8e,OB:0,LOB:
        0,IP:0,URL:5,TC:0,Content:-5,EDM:0,RT:0,SF:100,FILE:0,RULE:Spam_GS981B3D,A
        CTION:quarantine,TS:100
X-CID-META: VersionHash:2a19b09,CLOUDID:6f60987e-c8dc-403a-96e8-6237210dceee,C
        OID:1e5b5abe9d18,Recheck:0,SF:28|16|19|48,TC:nil,Content:0,EDM:-3,IP:nil,U
        RL:1,File:nil,QS:0,BEC:nil
X-UUID: cad35b00221045709436f16bfc371d3d-20220608
Received: from mtkcas11.mediatek.inc [(172.21.101.40)] by mailgw02.mediatek.com
        (envelope-from <xiaoyong.lu@mediatek.com>)
        (Generic MTA with TLSv1.2 ECDHE-RSA-AES256-SHA384 256/256)
        with ESMTP id 1400907338; Wed, 08 Jun 2022 14:17:16 +0800
Received: from mtkcas11.mediatek.inc (172.21.101.40) by
 mtkmbs10n1.mediatek.inc (172.21.101.34) with Microsoft SMTP Server
 (version=TLS1_2, cipher=TLS_ECDHE_RSA_WITH_AES_256_CBC_SHA384) id
 15.2.792.15; Wed, 8 Jun 2022 14:17:14 +0800
Received: from localhost.localdomain (10.17.3.154) by mtkcas11.mediatek.inc
 (172.21.101.73) with Microsoft SMTP Server id 15.0.1497.2 via Frontend
 Transport; Wed, 8 Jun 2022 14:17:12 +0800
...
Subject: [RFC PATCH v2] media: mediatek: vcodec: support stateless AV1 decoder
Date:   Wed, 8 Jun 2022 14:17:09 +0800
Message-ID: <20220608061709.2292-1-xiaoyong.lu@mediatek.com>
...
X-Spam-Status: No, score=1.1 required=5.0 tests=BAYES_00,HK_RANDOM_ENVFROM,
        HK_RANDOM_FROM,MAY_BE_FORGED,SPF_HELO_PASS,T_SCC_BODY_TEXT_LINE,
...
Reference series:
[1]: v2 of this series is presend by Daniel Almeida.
     message-id: 20220322160101.620748-1-daniel.almeida@collabora.com

 .../media/platform/mediatek/vcodec/Makefile   |    1 +
 .../vcodec/mtk_vcodec_dec_stateless.c         |   57 +-
 .../platform/mediatek/vcodec/mtk_vcodec_drv.h |    1 +
 .../vcodec/vdec/vdec_av1_req_lat_if.c         | 2251 +++++++++++++++++
 .../platform/mediatek/vcodec/vdec_drv_if.c    |    4 +
 .../platform/mediatek/vcodec/vdec_drv_if.h    |    1 +
 .../platform/mediatek/vcodec/vdec_msg_queue.c |   27 +
 .../platform/mediatek/vcodec/vdec_msg_queue.h |    4 +
 8 files changed, 2344 insertions(+), 2 deletions(-)
...
index 16d55785d84ba..12c25d49349eb 100644
--- a/drivers/media/platform/mediatek/vcodec/mtk_vcodec_dec_stateless.c
+++ b/drivers/media/platform/mediatek/vcodec/mtk_vcodec_dec_stateless.c
@@ -107,12 +107,60 @@ static const struct mtk_stateless_control mtk_stateless_controls[] = {
 		},
...
@@ -354,6 +402,7 @@ static void mtk_vcodec_add_formats(unsigned int fourcc,
...
@@ -405,6 +454,10 @@ static void mtk_vcodec_get_supported_formats(struct mtk_vcodec_ctx *ctx)
...
index eeee880eccdd5..a4e74fd4610ac 100644
...
index 0000000000000..3ee1604bd9685
--- /dev/null
...
+#define FH_FLAG(x, name)		(!!((x)->flags & V4L2_AV1_FRAME_HEADER_FLAG_##name))
+
+#define MINQ 0
+#define MAXQ 255
+
...
+	u32 num_tiles;
+	u8 last_tile_in_tile_group[V4L2_AV1_MAX_TILE_COUNT];
...
+	u32 tg_size, tge_size;
+	int i;
+
+	ctrl = v4l2_ctrl_find(&instance->ctx->ctrl_hdl, V4L2_CID_STATELESS_AV1_TILE_GROUP);
+	if (!ctrl)
+		return -EINVAL;
+
+	tg_size = ctrl->elems;
...
+			mtk_vcodec_err(instance, "Invalid tg_end %d, larger than tile_num %d\n",
...
+			((allow_update_cdf << 18) |
+			(tile_group->last_tile_in_tile_group[tile_num] << 16));
+
