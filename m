...
Received: from vger.kernel.org (vger.kernel.org [209.132.180.67])
	by mail.lfdr.de (Postfix) with ESMTP id DD41E1994A1
	for <lists+devicetree@lfdr.de>; Tue, 31 Mar 2020 13:02:42 +0200 (CEST)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S1730380AbgCaLCm (ORCPT <rfc822;lists+devicetree@lfdr.de>);
        Tue, 31 Mar 2020 07:02:42 -0400
Received: from gloria.sntech.de ([185.11.138.130]:59698 "EHLO gloria.sntech.de"
        rhost-flags-OK-OK-OK-OK) by vger.kernel.org with ESMTP
        id S1730378AbgCaLCm (ORCPT <rfc822;devicetree@vger.kernel.org>);
        Tue, 31 Mar 2020 07:02:42 -0400
Received: from ip5f5a5d2f.dynamic.kabel-deutschland.de ([95.90.93.47] helo=phil.sntech)
        by gloria.sntech.de with esmtpsa (TLS1.2:ECDHE_RSA_AES_128_GCM_SHA256:128)
        (Exim 4.89)
        (envelope-from <heiko@sntech.de>)
        id 1jJEfC-0004uz-9k; Tue, 31 Mar 2020 13:02:34 +0200
...
Subject: [PATCH 2/2] drm/panel: add panel driver for Leadtek LTK050H3146W
Date:   Tue, 31 Mar 2020 13:02:22 +0200
Message-Id: <20200331110222.1798093-2-heiko@sntech.de>
X-Mailer: git-send-email 2.24.1
In-Reply-To: <20200331110222.1798093-1-heiko@sntech.de>
References: <20200331110222.1798093-1-heiko@sntech.de>
MIME-Version: 1.0
...
index 000000000000..d7afcdf5538a
--- /dev/null
...
+}
+
+static int ltk050h3146w_select_page(struct ltk050h3146w *ctx, int page)
+{
...
+}
+
+static int ltk050h3146w_write_page(struct ltk050h3146w *ctx, int page,
+				   const struct ltk050h3146w_cmd *cmds, int num)
+{
+	struct mipi_dsi_device *dsi = to_mipi_dsi_device(ctx->dev);
+	int i, ret;
+
+	ret = ltk050h3146w_select_page(ctx, page);
+	if (ret < 0) {
...
+	 * documentation.
+	 */
+	ret = ltk050h3146w_write_page(ctx, 3, page3_cmds,
+					      ARRAY_SIZE(page3_cmds));
+	if (ret < 0)
+		return ret;
+
+	ret = ltk050h3146w_write_page(ctx, 4, page4_cmds,
+					      ARRAY_SIZE(page4_cmds));
+	if (ret < 0)
+		return ret;
+
+	ret = ltk050h3146w_write_page(ctx, 1, page1_cmds,
+					      ARRAY_SIZE(page1_cmds));
+	if (ret < 0)
+		return ret;
+
+	ret = ltk050h3146w_select_page(ctx, 0);
+	if (ret < 0) {
...
+	msleep(20);
+
+	ret = ltk050h3146w_init_sequence(ctx);
+	if (ret < 0) {
