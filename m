...
Received: from vger.kernel.org (vger.kernel.org [23.128.96.18])
	by mail.lfdr.de (Postfix) with ESMTP id F235E47F132
	for <lists+devicetree@lfdr.de>; Fri, 24 Dec 2021 22:17:07 +0100 (CET)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S1353603AbhLXVRF (ORCPT <rfc822;lists+devicetree@lfdr.de>);
        Fri, 24 Dec 2021 16:17:05 -0500
Received: from lindbergh.monkeyblade.net ([23.128.96.19]:55146 "EHLO
        lindbergh.monkeyblade.net" rhost-flags-OK-OK-OK-OK) by vger.kernel.org
        with ESMTP id S1353597AbhLXVRE (ORCPT
        <rfc822;devicetree@vger.kernel.org>); Fri, 24 Dec 2021 16:17:04 -0500
Received: from mail-ed1-x52a.google.com (mail-ed1-x52a.google.com [IPv6:2a00:1450:4864:20::52a])
        by lindbergh.monkeyblade.net (Postfix) with ESMTPS id AB970C06175B;
        Fri, 24 Dec 2021 13:17:03 -0800 (PST)
Received: by mail-ed1-x52a.google.com with SMTP id o6so37130888edc.4;
        Fri, 24 Dec 2021 13:17:03 -0800 (PST)
DKIM-Signature: v=1; a=rsa-sha256; c=relaxed/relaxed;
        d=gmail.com; s=20210112;
        h=mime-version:references:in-reply-to:from:date:message-id:subject:to
         :cc:content-transfer-encoding;
        bh=mzwrXx/fDbRw60Xb8bUyZC16aSBp54CKMHnyp3u3NiA=;
        b=
X-Google-DKIM-Signature: v=1; a=rsa-sha256; c=relaxed/relaxed;
        ...
        bh=mzwrXx/fDbRw60Xb8bUyZC16aSBp54CKMHnyp3u3NiA=;
        b=
X-Gm-Message-State: AOAM531Cai8AIxfgRvELKnKhBeqr2rUOttEyYt910ui4jvRCWo6oJAzH
        FpJO4gx4HTsjSKTP6yPiafznIgWQ2T8iaxUhMQ4=
X-Google-Smtp-Source: ABdhPJzBMdAQmdBn4Lk0SD1JVMRXJfu0AiksC55drGxiW/esJj+8jskdvGd/20JtYDpXAsVAIpT7EnkhrW1H+WCs1nc=
X-Received: by 2002:a05:6402:12c4:: with SMTP id k4mr7128214edx.218.1640380621895;
 Fri, 24 Dec 2021 13:17:01 -0800 (PST)
MIME-Version: 1.0
References: <20211224200935.93817-1-j.neuschaefer@gmx.net> <20211224200935.93817-6-j.neuschaefer@gmx.net>
In-Reply-To: <20211224200935.93817-6-j.neuschaefer@gmx.net>
From:   Andy Shevchenko <andy.shevchenko@gmail.com>
Date:   Fri, 24 Dec 2021 23:15:04 +0200
Message-ID: <CAHp75VdNd5q600qgfxZ+hy0NjpWXrLVycSYt=v6UP7wNd3-TJg@mail.gmail.com>
...
>
> v3:
> - make gc the first member of struct wpcm450_gpio, to simplify pointer ar=
ithmetic
...
> - I decided to keep using __assign_bit throughout the driver, because it =
keeps the code short
> - Remove nuvoton,interrupt-map again, to simplify the binding
> - Use gpio_chip->fwnode
> - I decided against using the bitmap_* API in(place of) wpcm450_gpio_irq_=
bitnum,
...
> diff --git a/drivers/pinctrl/nuvoton/Makefile b/drivers/pinctrl/nuvoton/M=
akefile
...
> +// - GPIO event (interrupt) registers, located centrally in the GPIO reg=
ister
...
> +
> +       /*
> +        * This spin lock protects registers and struct wpcm450_pinctrl f=
ields
...
> +};
> +
> +static int wpcm450_gpio_irq_bitnum(struct wpcm450_gpio *gpio, struct irq=
_data *d)
> +{
...
> +}
> +
> +static int wpcm450_irq_bitnum_to_gpio(struct wpcm450_gpio *gpio, int bit=
num)
> +{
> +       const struct wpcm450_bank *bank =3D gpio->bank;
> +
...
> +{
> +       struct wpcm450_gpio *gpio =3D gpiochip_get_data(irq_data_get_irq_=
chip_data(d));
...
> +
> +/*
> + * Since the GPIO controller does not support dual-edge triggered interr=
upts
> + * (IRQ_TYPE_EDGE_BOTH), they are emulated using rising/falling edge tri=
ggered
> + * interrupts. wpcm450_gpio_fix_evpol sets the interrupt polarity for th=
e
> + * specified emulated dual-edge triggered interrupts, so that the next e=
dge can
> + * be detected.
> + */
> +static void wpcm450_gpio_fix_evpol(struct wpcm450_gpio *gpio, unsigned l=
ong all)
> +{
> +       struct wpcm450_pinctrl *pctrl =3D gpio->pctrl;
> +       unsigned long flags;
...
> +{
> +       struct wpcm450_gpio *gpio =3D gpiochip_get_data(irq_desc_get_hand=
ler_data(desc));
> +       struct wpcm450_pinctrl *pctrl =3D gpio->pctrl;
...
> +       ours =3D GENMASK(gpio->bank->first_irq_bit + gpio->bank->num_irqs=
 - 1,
> +                      gpio->bank->first_irq_bit);

       ours =3D GENMASK(gpio->bank->num_irqs - 1, 0) << gpio->bank->first_i=
rq_bit;
...
> +       WPCM450_PINCFG(7,        none, NONE, 0,           sdio, MFSEL1, 3=
0),
...
> +       WPCM450_PINCFG(12,       gspi, MFSEL1, 24,        sspi, MFSEL1, 3=
1),
> +       WPCM450_PINCFG(13,       gspi, MFSEL1, 24,        sspi, MFSEL1, 3=
1),
> +       WPCM450_PINCFG(14,       gspi, MFSEL1, 24,        sspi, MFSEL1, 3=
1),
> +       WPCM450_PINCFG(15,       gspi, MFSEL1, 24,        sspi, MFSEL1, 3=
1),
> +       WPCM450_PINCFG(16,       none, NONE, 0,           pwm6, MFSEL2, 2=
2),
> +       WPCM450_PINCFG(17,       none, NONE, 0,           pwm7, MFSEL2, 2=
3),
> +       WPCM450_PINCFG(18,       none, NONE, 0,           none, NONE, 0),
> +       WPCM450_PINCFG(19,       uinc, MFSEL1, 23,        none, NONE, 0),
> +       WPCM450_PINCFG(20,        hg0, MFSEL2, 24,        pwm4, MFSEL2, 2=
0),
> +       WPCM450_PINCFG(21,        hg1, MFSEL2, 25,        pwm5, MFSEL2, 2=
1),
...
> +       WPCM450_PINCFG(43,       hsp1, MFSEL1, 10,        sdio, MFSEL1, 3=
0),
> +       WPCM450_PINCFG(44,       hsp1, MFSEL1, 10,        sdio, MFSEL1, 3=
0),
> +       WPCM450_PINCFG(45,       hsp1, MFSEL1, 10,        sdio, MFSEL1, 3=
0),
> +       WPCM450_PINCFG(46,       hsp1, MFSEL1, 10,        sdio, MFSEL1, 3=
0),
> +       WPCM450_PINCFG(47,       hsp1, MFSEL1, 10,        sdio, MFSEL1, 3=
0),
...
> +       WPCM450_PINCFG(60,        hg7, MFSEL2, 31,        sdio, MFSEL1, 3=
0),
...
> +       WPCM450_PIN(0),   WPCM450_PIN(1),   WPCM450_PIN(2),   WPCM450_PIN=
(3),
> +       WPCM450_PIN(4),   WPCM450_PIN(5),   WPCM450_PIN(6),   WPCM450_PIN=
(7),
> +       WPCM450_PIN(8),   WPCM450_PIN(9),   WPCM450_PIN(10),  WPCM450_PIN=
(11),
> +       WPCM450_PIN(12),  WPCM450_PIN(13),  WPCM450_PIN(14),  WPCM450_PIN=
(15),
> +       WPCM450_PIN(16),  WPCM450_PIN(17),  WPCM450_PIN(18),  WPCM450_PIN=
(19),
> +       WPCM450_PIN(20),  WPCM450_PIN(21),  WPCM450_PIN(22),  WPCM450_PIN=
(23),
> +       WPCM450_PIN(24),  WPCM450_PIN(25),  WPCM450_PIN(26),  WPCM450_PIN=
(27),
> +       WPCM450_PIN(28),  WPCM450_PIN(29),  WPCM450_PIN(30),  WPCM450_PIN=
(31),
> +       WPCM450_PIN(32),  WPCM450_PIN(33),  WPCM450_PIN(34),  WPCM450_PIN=
(35),
> +       WPCM450_PIN(36),  WPCM450_PIN(37),  WPCM450_PIN(38),  WPCM450_PIN=
(39),
> +       WPCM450_PIN(40),  WPCM450_PIN(41),  WPCM450_PIN(42),  WPCM450_PIN=
(43),
> +       WPCM450_PIN(44),  WPCM450_PIN(45),  WPCM450_PIN(46),  WPCM450_PIN=
(47),
> +       WPCM450_PIN(48),  WPCM450_PIN(49),  WPCM450_PIN(50),  WPCM450_PIN=
(51),
> +       WPCM450_PIN(52),  WPCM450_PIN(53),  WPCM450_PIN(54),  WPCM450_PIN=
(55),
> +       WPCM450_PIN(56),  WPCM450_PIN(57),  WPCM450_PIN(58),  WPCM450_PIN=
(59),
> +       WPCM450_PIN(60),  WPCM450_PIN(61),  WPCM450_PIN(62),  WPCM450_PIN=
(63),
> +       WPCM450_PIN(64),  WPCM450_PIN(65),  WPCM450_PIN(66),  WPCM450_PIN=
(67),
> +       WPCM450_PIN(68),  WPCM450_PIN(69),  WPCM450_PIN(70),  WPCM450_PIN=
(71),
> +       WPCM450_PIN(72),  WPCM450_PIN(73),  WPCM450_PIN(74),  WPCM450_PIN=
(75),
> +       WPCM450_PIN(76),  WPCM450_PIN(77),  WPCM450_PIN(78),  WPCM450_PIN=
(79),
> +       WPCM450_PIN(80),  WPCM450_PIN(81),  WPCM450_PIN(82),  WPCM450_PIN=
(83),
> +       WPCM450_PIN(84),  WPCM450_PIN(85),  WPCM450_PIN(86),  WPCM450_PIN=
(87),
> +       WPCM450_PIN(88),  WPCM450_PIN(89),  WPCM450_PIN(90),  WPCM450_PIN=
(91),
> +       WPCM450_PIN(92),  WPCM450_PIN(93),  WPCM450_PIN(94),  WPCM450_PIN=
(95),
> +       WPCM450_PIN(96),  WPCM450_PIN(97),  WPCM450_PIN(98),  WPCM450_PIN=
(99),
> +       WPCM450_PIN(100), WPCM450_PIN(101), WPCM450_PIN(102), WPCM450_PIN=
(103),
> +       WPCM450_PIN(104), WPCM450_PIN(105), WPCM450_PIN(106), WPCM450_PIN=
(107),
> +       WPCM450_PIN(108), WPCM450_PIN(109), WPCM450_PIN(110), WPCM450_PIN=
(111),
> +       WPCM450_PIN(112), WPCM450_PIN(113), WPCM450_PIN(114), WPCM450_PIN=
(115),
> +       WPCM450_PIN(116), WPCM450_PIN(117), WPCM450_PIN(118), WPCM450_PIN=
(119),
> +       WPCM450_PIN(120), WPCM450_PIN(121), WPCM450_PIN(122), WPCM450_PIN=
(123),
> +       WPCM450_PIN(124), WPCM450_PIN(125), WPCM450_PIN(126), WPCM450_PIN=
(127),
> +};
> +
...
> +               spin_unlock_irqrestore(&pctrl->lock, flags);
> +
> +               *config =3D pinconf_to_config_packed(param, !!(reg & BIT(=
bit)));
> +               break;
> +       default:
> +               return -ENOTSUPP;
...
> +                                  unsigned int offset, unsigned long con=
fig)
> +{
...
> +       const struct wpcm450_bank *bank =3D gpio->bank;
> +
> +       return gpiochip_add_pin_range(&gpio->gc, dev_name(gpio->pctrl->de=
v),
...
> +       if (!pctrl->gpio_base)
> +               return dev_err_probe(pctrl->dev, -ENOMEM, "Resource fail =
for GPIO controller\n");
> +
...
> +               if (ret < 0)
> +                       return dev_err_probe(pctrl->dev, ret, "GPIO initi=
alization failed\n");
> +
...
> +               girq->parents =3D devm_kcalloc(pctrl->dev, WPCM450_NUM_GP=
IO_IRQS,
...
o);
> +               if (ret)
> +                       return dev_err_probe(pctrl->dev, ret, "Failed to =
add GPIO chip\n");
> +       }
> +
> +       return 0;
...
> +               return dev_err_probe(pctrl->dev, PTR_ERR(pctrl->gcr_regma=
p),
