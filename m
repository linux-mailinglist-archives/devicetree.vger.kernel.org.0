...
Received: from vger.kernel.org (vger.kernel.org [209.132.180.67])
	by mail.lfdr.de (Postfix) with ESMTP id B6893192E26
	for <lists+devicetree@lfdr.de>; Wed, 25 Mar 2020 17:26:19 +0100 (CET)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S1727934AbgCYQ0H (ORCPT <rfc822;lists+devicetree@lfdr.de>);
        Wed, 25 Mar 2020 12:26:07 -0400
Received: from hqnvemgate25.nvidia.com ([216.228.121.64]:17459 "EHLO
        hqnvemgate25.nvidia.com" rhost-flags-OK-OK-OK-OK) by vger.kernel.org
        with ESMTP id S1727386AbgCYQ0G (ORCPT
        <rfc822;devicetree@vger.kernel.org>); Wed, 25 Mar 2020 12:26:06 -0400
Received: from hqpgpgate101.nvidia.com (Not Verified[216.228.121.13]) by hqnvemgate25.nvidia.com (using TLS: TLSv1.2, DES-CBC3-SHA)
        id <B5e7b85db0000>; Wed, 25 Mar 2020 09:24:59 -0700
Received: from hqmail.nvidia.com ([172.20.161.6])
  by hqpgpgate101.nvidia.com (PGP Universal service);
  Wed, 25 Mar 2020 09:25:44 -0700
X-PGP-Universal: processed;
        by hqpgpgate101.nvidia.com on Wed, 25 Mar 2020 09:25:44 -0700
...
Message-ID: <4ed299bd-1e29-70ad-0772-130173b0586e@nvidia.com>
Date:   Wed, 25 Mar 2020 09:25:44 -0700
...
DKIM-Signature: v=1; a=rsa-sha256; c=relaxed/relaxed; d=nvidia.com; s=n1;
        t=1585153499; bh=WYgWq1TsT2d1QCDWBaM2Pi+nz1egt1nPqa5AmllMbTs=;
        h=X-PGP-Universal:Subject:To:CC:References:From:Message-ID:Date:
         User-Agent:MIME-Version:In-Reply-To:X-Originating-IP:
         X-ClientProxiedBy:Content-Type:Content-Transfer-Encoding:
         Content-Language;
        b=
...
>> +                             return;
> This only returns something sensible if you have an exact size (or close to
> 1920x1080). Please use v4l2_find_nearest_size(), for instance.
>
>> +                     }
>> +             }
>> +     }
...
> As there's a static number of these, I'd try to allocate them as part of
> the main device context struct. That decreases the probability of leaking
> memory; error handling remains complex enough in any case.

Sensor support will be added later to the same driver and when in 
non-tpg, channel allocations are based on ports info in device tree 
where not all channels are needed to be created a head.
...
> There's a lot of repetition done for all these clocks. I wonder if using
> e.g. clk_bulk_prepare_enable() would help here.
>
>> +             if (ret) {
>> +                     dev_err(csi->dev,
...
> __maybe_unused for this and the above one.
>
>> +{
>> +     struct tegra_csi *csi = dev_get_drvdata(dev);
>> +
...
> The clock framework can tolerate NULL just fine, no need for these checks.
>
>> +
>> +     return 0;
>> +}
...
video entity is initialized in channel_init for all channels and 
subdevice entity initialization happens in CSI driver.
>
>> +             if (ret) {
>> +                     dev_err(vi->dev,
...
switched from devm_kzalloc to kzalloc based on earlier feedback to make 
sure of no crash during unbind as memory allocated with device managed 
resource gets immediately freed.
>
>> +     if (!vi)
>> +             return -ENOMEM;
>> +
...
Missed CONFIG_PM check. Will add.
>
>> +
...
We can't use threaded interrupt handler for VI/CSI host1x clients as all 
capture events in driver are synchronized through hardware sync point 
counters.

sync point driver is part of host1x driver and is used by all host1x 
clients to track events.
