...
Received: from out1.vger.email (out1.vger.email [IPv6:2620:137:e000::1:20])
	by mail.lfdr.de (Postfix) with ESMTP id 41C336179FC
	for <lists+devicetree@lfdr.de>; Thu,  3 Nov 2022 10:30:04 +0100 (CET)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S230315AbiKCJ36 (ORCPT <rfc822;lists+devicetree@lfdr.de>);
        Thu, 3 Nov 2022 05:29:58 -0400
Received: from lindbergh.monkeyblade.net ([23.128.96.19]:47172 "EHLO
        lindbergh.monkeyblade.net" rhost-flags-OK-OK-OK-OK) by vger.kernel.org
        with ESMTP id S229770AbiKCJ3W (ORCPT
        <rfc822;devicetree@vger.kernel.org>); Thu, 3 Nov 2022 05:29:22 -0400
Received: from dfw.source.kernel.org (dfw.source.kernel.org [IPv6:2604:1380:4641:c500::1])
        by lindbergh.monkeyblade.net (Postfix) with ESMTPS id DFD36F595;
        Thu,  3 Nov 2022 02:29:03 -0700 (PDT)
Received: from smtp.kernel.org (relay.kernel.org [52.25.139.140])
        (using TLSv1.2 with cipher ECDHE-RSA-AES256-GCM-SHA384 (256/256 bits))
        (No client certificate requested)
        by dfw.source.kernel.org (Postfix) with ESMTPS id 61F5261D17;
        Thu,  3 Nov 2022 09:29:03 +0000 (UTC)
Received: by smtp.kernel.org (Postfix) with ESMTPSA id 1DC4FC433C1;
        Thu,  3 Nov 2022 09:29:01 +0000 (UTC)
DKIM-Signature: v=1; a=rsa-sha256; c=relaxed/simple; d=kernel.org;
        s=k20201202; t=1667467742;
        bh=ljnI0+0oY9McuVMufWPSbtO2WvE+YljMtFhW38bH9GM=;
        h=From:To:Cc:Subject:Date:In-Reply-To:References:From;
        b=
...
Subject: [PATCH v3 net-next 6/8] net: ethernet: mtk_wed: add configure wed wo support
Date:   Thu,  3 Nov 2022 10:28:05 +0100
Message-Id: <db2741aacb187b20168dfc72c6501dd0b2f9d4ba.1667466887.git.lorenzo@kernel.org>
...
---
 drivers/net/ethernet/mediatek/mtk_wed.c      | 590 +++++++++++++++++--
 ...
 6 files changed, 832 insertions(+), 54 deletions(-)

diff --git a/drivers/net/ethernet/mediatek/mtk_wed.c b/drivers/net/ethernet/mediatek/mtk_wed.c
index e904596e67de..b254a611b902 100644
...
+
 ...
 {
@@ -116,7 +229,7 @@ mtk_wed_assign(struct mtk_wed_device *dev)
 }
...
@@ -203,10 +316,10 @@ mtk_wed_buffer_alloc(struct mtk_wed_device *dev)
 }
...
@@ -229,13 +343,59 @@ mtk_wed_free_buffer(struct mtk_wed_device *dev)
 		__free_page(page);
 	}
...
+
 ...
 {
@@ -246,6 +406,13 @@ mtk_wed_free_ring(struct mtk_wed_device *dev, struct mtk_wed_ring *ring)
...
+
 ...
 {
@@ -291,6 +458,38 @@ mtk_wed_set_512_support(struct mtk_wed_device *dev, bool enable)
 	}
 }
...
+
 ...
 {
@@ -304,20 +503,25 @@ mtk_wed_dma_disable(struct mtk_wed_device *dev)
...
@@ -338,6 +542,13 @@ mtk_wed_stop(struct mtk_wed_device *dev)
...
+	wed_clr(dev, MTK_WED_CTRL, MTK_WED_CTRL_WED_RX_BM_EN);
 ...
 static void
@@ -353,11 +564,21 @@ mtk_wed_detach(struct mtk_wed_device *dev)
...
@@ -434,10 +655,12 @@ mtk_wed_set_wpdma(struct mtk_wed_device *dev)
 	} else {
...
@@ -487,6 +710,127 @@ mtk_wed_hw_init_early(struct mtk_wed_device *dev)
 	}
 }
...
+
 ...
 {
@@ -498,11 +842,11 @@ mtk_wed_hw_init(struct mtk_wed_device *dev)
...
@@ -529,9 +873,9 @@ mtk_wed_hw_init(struct mtk_wed_device *dev)
...
@@ -539,18 +883,28 @@ mtk_wed_hw_init(struct mtk_wed_device *dev)
...
+mtk_wed_ring_reset(struct mtk_wed_ring *ring, int size, bool tx)
 {
 	...
 	int i;
@@ -560,7 +914,10 @@ mtk_wed_ring_reset(struct mtk_wed_ring *ring, int size)
...
@@ -616,7 +973,8 @@ mtk_wed_reset_dma(struct mtk_wed_device *dev)
...
@@ -634,6 +992,9 @@ mtk_wed_reset_dma(struct mtk_wed_device *dev)
...
@@ -668,12 +1029,11 @@ mtk_wed_reset_dma(struct mtk_wed_device *dev)
...
+		   int size, u32 desc_size, bool tx)
 {
 	ring->desc = dma_alloc_coherent(dev->hw->dev, size * desc_size,
 					&ring->desc_phys, GFP_KERNEL);
@@ -682,7 +1042,7 @@ mtk_wed_ring_alloc(struct mtk_wed_device *dev, struct mtk_wed_ring *ring,
...
@@ -710,6 +1075,60 @@ mtk_wed_wdma_rx_ring_setup(struct mtk_wed_device *dev, int idx, int size)
 	return 0;
 }
...
+
 ...
 {
@@ -732,6 +1151,8 @@ mtk_wed_configure_irq(struct mtk_wed_device *dev, u32 irq_mask)
...
@@ -750,6 +1171,16 @@ mtk_wed_configure_irq(struct mtk_wed_device *dev, u32 irq_mask)
...
@@ -787,9 +1218,15 @@ mtk_wed_dma_enable(struct mtk_wed_device *dev)
...
+
 		wed_set(dev, MTK_WED_WPDMA_GLO_CFG,
 			MTK_WED_WPDMA_GLO_CFG_RX_DRV_R0_PKT_PROC |
 			MTK_WED_WPDMA_GLO_CFG_RX_DRV_R0_CRX_SYNC);
@@ -797,6 +1234,15 @@ mtk_wed_dma_enable(struct mtk_wed_device *dev)
 		wed_clr(dev, MTK_WED_WPDMA_GLO_CFG,
...
@@ -822,7 +1268,19 @@ mtk_wed_start(struct mtk_wed_device *dev, u32 irq_mask)
...
@@ -856,7 +1314,7 @@ mtk_wed_attach(struct mtk_wed_device *dev)
 	if (!hw) {
...
@@ -869,15 +1327,24 @@ mtk_wed_attach(struct mtk_wed_device *dev)
 	dev->dev = hw->dev;
 	dev->irq = hw->irq;
...
@@ -886,8 +1353,10 @@ mtk_wed_attach(struct mtk_wed_device *dev)
 				   BIT(hw->index), 0);
 	else
...
+unlock:
 	...
 	return ret;
@@ -910,10 +1379,11 @@ mtk_wed_tx_ring_setup(struct mtk_wed_device *dev, int idx, void __iomem *regs)
 	 * WDMA RX.
 	 */
...
@@ -960,6 +1430,37 @@ mtk_wed_txfree_ring_setup(struct mtk_wed_device *dev, void __iomem *regs)
 	return 0;
 }
...
+
 ...
 {
@@ -1056,7 +1557,9 @@ void mtk_wed_add_hw(struct device_node *np, struct mtk_eth *eth,
...
@@ -1065,6 +1568,7 @@ void mtk_wed_add_hw(struct device_node *np, struct mtk_eth *eth,
...
index d792d91bce51..e1842fb6716d 100644
...
@@ -65,24 +66,37 @@ void mtk_wed_mcu_rx_event(struct mtk_wed_wo *wo, struct sk_buff *skb)
 	wake_up(&wo->mcu.wait);
 }
...
@@ -93,6 +107,7 @@ void mtk_wed_mcu_rx_unsolicited_event(struct mtk_wed_wo *wo,
 		break;
 	}
...
+		mtk_wed_update_rx_stats(wo->hw->wed_dev, skb);
 	...
 	default:
 		break;
@@ -149,6 +164,8 @@ mtk_wed_mcu_parse_response(struct mtk_wed_wo *wo, struct sk_buff *skb,
 	skb_pull(skb, sizeof(*hdr));
 	switch (cmd) {
...
@@ -187,6 +204,18 @@ int mtk_wed_mcu_send_msg(struct mtk_wed_wo *wo, int id, int cmd,
 	return ret;
 }
...
index eda32e25288b..ecf057c9f33f 100644
...
@@ -247,6 +273,8 @@ void mtk_wed_mcu_rx_unsolicited_event(struct mtk_wed_wo *wo,
 				      struct sk_buff *skb);
...
index 956978320f8b..642fe82b85ec 100644
