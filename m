...
Received: from out1.vger.email (out1.vger.email [IPv6:2620:137:e000::1:20])
	by mail.lfdr.de (Postfix) with ESMTP id 28AC56DDF70
	for <lists+devicetree@lfdr.de>; Tue, 11 Apr 2023 17:20:13 +0200 (CEST)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S230310AbjDKPUM (ORCPT <rfc822;lists+devicetree@lfdr.de>);
        Tue, 11 Apr 2023 11:20:12 -0400
Received: from lindbergh.monkeyblade.net ([23.128.96.19]:56850 "EHLO
        lindbergh.monkeyblade.net" rhost-flags-OK-OK-OK-OK) by vger.kernel.org
        with ESMTP id S231366AbjDKPTe (ORCPT
        <rfc822;devicetree@vger.kernel.org>); Tue, 11 Apr 2023 11:19:34 -0400
Received: from madras.collabora.co.uk (madras.collabora.co.uk [IPv6:2a00:1098:0:82:1000:25:2eeb:e5ab])
        by lindbergh.monkeyblade.net (Postfix) with ESMTPS id F2A0149D0;
        Tue, 11 Apr 2023 08:19:09 -0700 (PDT)
Received: from nicolas-tpx395.localdomain (unknown [IPv6:2606:6d00:15:199e::580])
        (using TLSv1.3 with cipher TLS_AES_256_GCM_SHA384 (256/256 bits)
         key-exchange X25519 server-signature RSA-PSS (4096 bits) server-digest SHA256)
        (No client certificate requested)
        (Authenticated sender: nicolas)
        by madras.collabora.co.uk (Postfix) with ESMTPSA id 0A54066031C2;
        Tue, 11 Apr 2023 16:17:35 +0100 (BST)
DKIM-Signature: v=1; a=rsa-sha256; c=relaxed/simple; d=collabora.com;
        s=mail; t=1681226257;
        bh=IF54v8M1hJ8g4ceqh54i5dMyY873CbLRpDF9efXKrfQ=;
        h=Subject:From:To:Cc:Date:In-Reply-To:References:From;
        b=
Message-ID: <5943b2bf5774ff70e732b2af4631d3f241f3f1b9.camel@collabora.com>
Subject: Re: [PATCH v5 10/13] media: verisilicon: Add film grain feature to
 AV1 driver
...
Date:   Tue, 11 Apr 2023 11:17:26 -0400
In-Reply-To: <20230330154043.1250736-11-benjamin.gaignard@collabora.com>
References: <20230330154043.1250736-1-benjamin.gaignard@collabora.com>
         <20230330154043.1250736-11-benjamin.gaignard@collabora.com>
...
> +#include "rockchip_av1_filmgrain.h"
> +
> +static const s32 gaussian_sequence[2048] =3D {
...
> +					       u16 *random_register)
> +{
> +	u16 random_reg =3D seed;
> +
> +	random_reg ^=3D ((luma_num * 37 + 178) & 255) << 8;
> +	random_reg ^=3D ((luma_num * 173 + 105) & 255);
> +	*random_register =3D random_reg;
> +}
> +
> +static inline void rockchip_av1_update_random_register(u16 *random_regis=
ter)
> +{
> +	u16 bit;
> +	u16 random_reg =3D *random_register;
> +
> +	bit =3D ((random_reg >> 0) ^ (random_reg >> 1) ^ (random_reg >> 3) ^
> +	       (random_reg >> 12)) & 1;
> +	*random_register =3D (random_reg >> 1) | (bit << 15);
> +}
> +
...
> +}
> +
> +void rockchip_av1_generate_luma_grain_block(s32 (*luma_grain_block)[73][=
82],
> +					    s32 bitdepth,
> +					    u8 num_y_points,
...
> +					    u16 random_seed)
> +{
> +	s32 gauss_sec_shift =3D 12 - bitdepth + grain_scale_shift;
> +	u16 grain_random_register =3D random_seed;
> +	s32 i, j;
> +
> +	for (i =3D 0; i < 73; i++) {
> +		for (j =3D 0; j < 82; j++) {
...
> +
> +			for (deltarow =3D -ar_coeff_lag; deltarow <=3D 0;
> +			     deltarow++) {
> +				for (deltacol =3D -ar_coeff_lag;
> +				     deltacol <=3D ar_coeff_lag; deltacol++) {
> +					if (deltarow =3D=3D 0 && deltacol =3D=3D 0)
> +						break;
> +					wsum =3D wsum + (*ar_coeffs_y)[pos] *
...
> +void rockchip_av1_generate_chroma_grain_block(s32 (*luma_grain_block)[73=
][82],
...
> +{
> +	s32 gauss_sec_shift =3D 12 - bitdepth + grain_scale_shift;
> +	u16 grain_random_register =3D 0;
> +	s32 i, j;
> +
...
> +				rockchip_av1_update_random_register
> +				    (&grain_random_register);
> +				(*cb_grain_block)[i][j] =3D
> +				    round_power_of_two(gaussian_sequence
> +					     [rockchip_av1_get_random_number
> +					      (grain_random_register)],
> +					     gauss_sec_shift);
> +			} else {
> +				(*cb_grain_block)[i][j] =3D 0;
> +			}
> +		}
> +	}
> +
...
> +				rockchip_av1_update_random_register
> +				    (&grain_random_register);
> +				(*cr_grain_block)[i][j] =3D
> +				    round_power_of_two(gaussian_sequence
> +					     [rockchip_av1_get_random_number
> +					      (grain_random_register)],
> +					     gauss_sec_shift);
> +			} else {
> +				(*cr_grain_block)[i][j] =3D 0;
> +			}
> +		}
> +	}
> +
...
> +					if (deltarow =3D=3D 0 && deltacol =3D=3D 0)
> +						break;
> +					wsum_cb =3D wsum_cb + (*ar_coeffs_cb)[pos] *
...
> +			if (num_y_points > 0) {
> +				s32 av_luma =3D 0;
> +				s32 luma_coord_y =3D (i << 1) - 3;
> +				s32 luma_coord_x =3D (j << 1) - 3;
> +
> +				av_luma +=3D
...
> +				av_luma =3D round_power_of_two(av_luma, 2);
> +
> +				wsum_cb =3D wsum_cb + (*ar_coeffs_cb)[pos] * av_luma;
...
> +	}
> +}
> diff --git a/drivers/media/platform/verisilicon/rockchip_av1_filmgrain.h =
...
> index a2c9bcebd8bb..d535b0e78f15 100644
> --- a/drivers/media/platform/verisilicon/rockchip_vpu981_hw_av1_dec.c
> +++ b/drivers/media/platform/verisilicon/rockchip_vpu981_hw_av1_dec.c
> @@ -334,6 +334,12 @@ void rockchip_vpu981_av1_dec_exit(struct hantro_ctx =
*ctx)
...
> +	av1_dec->film_grain.cpu =3D NULL;
> +
>  	if (av1_dec->prob_tbl.cpu)
>  		dma_free_coherent(vpu->dev, av1_dec->prob_tbl.size,
>  				  av1_dec->prob_tbl.cpu, av1_dec->prob_tbl.dma);
> @@ -374,6 +380,14 @@ int rockchip_vpu981_av1_dec_init(struct hantro_ctx *=
ctx)
>  		return -ENOMEM;
>  	av1_dec->tile_info.size =3D AV1_MAX_TILES;
> =20
> +	av1_dec->film_grain.cpu =3D dma_alloc_coherent(vpu->dev,
> +						     ALIGN(sizeof(struct rockchip_av1_film_grain), 2048),
> +						     &av1_dec->film_grain.dma,
> +						     GFP_KERNEL);
> +	if (!av1_dec->film_grain.cpu)
> +		return -ENOMEM;
> +	av1_dec->film_grain.size =3D ALIGN(sizeof(struct rockchip_av1_film_grai=
n), 2048);
> +
>  	av1_dec->prob_tbl.cpu =3D dma_alloc_coherent(vpu->dev,
>  						   ALIGN(sizeof(struct av1cdfs), 2048),
>  						   &av1_dec->prob_tbl.dma,
> @@ -1180,6 +1194,201 @@ static void rockchip_vpu981_av1_dec_set_prob(stru=
ct hantro_ctx *ctx)
>  	hantro_write_addr(vpu, AV1_PROP_TABLE, av1_dec->prob_tbl.dma);
>  }
> =20
> +static void
> +rockchip_vpu981_av1_dec_init_scaling_function(const u8 *values, const u8=
 *scaling,
> +					      u8 num_points, u8 *scaling_lut)
> +{
> +	int i, point;
> +
> +	if (num_points =3D=3D 0) {
> +		memset(scaling_lut, 0, 256);
> +		return;
> +	}
> +
> +	for (point =3D 0; point < num_points - 1; point++) {
> +		int x;
> +		s32 delta_y =3D scaling[point + 1] - scaling[point];
> +		s32 delta_x =3D values[point + 1] - values[point];
> +		s64 delta =3D
> +		    delta_x ? delta_y * ((65536 + (delta_x >> 1)) /
> +					 delta_x) : 0;
> +
> +		for (x =3D 0; x < delta_x; x++) {
> +			scaling_lut[values[point] + x] =3D
> +			    scaling[point] +
> +			    (s32)((x * delta + 32768) >> 16);
> +		}
> +	}
> +
> +	for (i =3D values[num_points - 1]; i < 256; i++)
> +		scaling_lut[i] =3D scaling[num_points - 1];
> +}
> +
...
> +	const struct v4l2_ctrl_av1_film_grain *film_grain =3D ctrls->film_grain=
;
> +	struct rockchip_av1_film_grain *fgmem =3D av1_dec->film_grain.cpu;
> +	struct hantro_dev *vpu =3D ctx->dev;
> +	bool scaling_from_luma =3D
> +		!!(film_grain->flags & V4L2_AV1_FILM_GRAIN_FLAG_CHROMA_SCALING_FROM_LU=
MA);
> +	s32 (*ar_coeffs_y)[24];
> +	s32 (*ar_coeffs_cb)[25];
> +	s32 (*ar_coeffs_cr)[25];
...
> +	}
> +
> +	ar_coeffs_y =3D kzalloc(sizeof(int32_t) * 24, GFP_KERNEL);
> +	ar_coeffs_cb =3D kzalloc(sizeof(int32_t) * 25, GFP_KERNEL);
> +	ar_coeffs_cr =3D kzalloc(sizeof(int32_t) * 25, GFP_KERNEL);
> +	luma_grain_block =3D kzalloc(sizeof(int32_t) * 73 * 82, GFP_KERNEL);
> +	cb_grain_block =3D kzalloc(sizeof(int32_t) * 38 * 44, GFP_KERNEL);
> +	cr_grain_block =3D kzalloc(sizeof(int32_t) * 38 * 44, GFP_KERNEL);
> +
...
> +		hantro_reg_write(vpu, &av1_cb_luma_mult, film_grain->cb_luma_mult - 12=
8);
> +		hantro_reg_write(vpu, &av1_cb_offset, film_grain->cb_offset - 256);
> +		hantro_reg_write(vpu, &av1_cr_mult, film_grain->cr_mult - 128);
> +		hantro_reg_write(vpu, &av1_cr_luma_mult, film_grain->cr_luma_mult - 12=
8);
...
> +			 !!(film_grain->flags & V4L2_AV1_FILM_GRAIN_FLAG_CLIP_TO_RESTRICTED_R=
ANGE));
...
> +	}
> +
> +	for (i =3D 0; i < V4L2_AV1_AR_COEFFS_SIZE; i++) {
> +		if (i < 24)
> +			(*ar_coeffs_y)[i] =3D film_grain->ar_coeffs_y_plus_128[i] - 128;
> +		(*ar_coeffs_cb)[i] =3D film_grain->ar_coeffs_cb_plus_128[i] - 128;
> +		(*ar_coeffs_cr)[i] =3D film_grain->ar_coeffs_cr_plus_128[i] - 128;
> +	}
> +
> +	ar_coeff_lag =3D film_grain->ar_coeff_lag;
> +	ar_coeff_shift =3D film_grain->ar_coeff_shift_minus_6 + 6;
> +	grain_scale_shift =3D film_grain->grain_scale_shift;
> +	bitdepth =3D ctx->bit_depth;
> +	grain_center =3D 128 << (bitdepth - 8);
> +	grain_min =3D 0 - grain_center;
> +	grain_max =3D (256 << (bitdepth - 8)) - 1 - grain_center;
> +
...
> +
> +	rockchip_av1_generate_chroma_grain_block(luma_grain_block, cb_grain_blo=
ck,
...
> +	for (i =3D 0; i < 64; i++) {
> +		for (j =3D 0; j < 64; j++)
> +			fgmem->cropped_luma_grain_block[i * 64 + j] =3D
> +				(*luma_grain_block)[i + 9][j + 9];
> +	}
> +
> +	for (i =3D 0; i < 32; i++) {
> +		for (j =3D 0; j < 32; j++) {
> +			fgmem->cropped_chroma_grain_block[i * 64 + 2 * j] =3D
> +				(*cb_grain_block)[i + 6][j + 6];
> +			fgmem->cropped_chroma_grain_block[i * 64 + 2 * j + 1] =3D
...
>  {
>  	struct hantro_av1_dec_hw_ctx *av1_dec =3D &ctx->av1_dec;
> @@ -1917,6 +2126,7 @@ int rockchip_vpu981_av1_dec_run(struct hantro_ctx *=
ctx)
